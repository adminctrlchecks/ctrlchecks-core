...
import {
  fetchMongoCollectionsForConnection,
  fetchMongoCollectionPreview,
  MongoCollectionPreview,
} from '@/lib/api/databaseExplorer';

interface MongoCollectionSelectProps {
  value: string;
  onChange: (value: string) => void;
  /** Saved Connection id from node.data.connectionRefs['mongodb_connection'], if linked */
  connectionId?: string;
}
...
/**
 * MongoDB Collection Name field with a "Browse Collections" helper. Lets the
 * user see real collection names and sample documents (which can vary in
 * shape) before typing a filter/document by hand.
 */
export default function MongoCollectionSelect({ value, onChange, connectionId }: MongoCollectionSelectProps) {
  ...
  const [previewCache, setPreviewCache] = useState<Record<string, MongoCollectionPreview>>({});
  ...
  const openBrowser = async () => {
    ...
    setError(null);
    try {
      const result = await fetchMongoCollectionsForConnection(connectionId);
  ...
  const toggleCollection = async (name: string) => {
    ...
    try {
      const preview = await fetchMongoCollectionPreview(connectionId, name);
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            title={connectionId ? 'Browse collections in the connected database' : 'Connect a MongoDB account first'}
          >
