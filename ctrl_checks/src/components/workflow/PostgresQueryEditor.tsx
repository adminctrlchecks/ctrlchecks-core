...
import { fetchPostgresTablesForConnection, fetchPostgresTablePreview, PostgresTablePreview } from '@/lib/api/databaseExplorer';

interface PostgresQueryEditorProps {
  value: string;
  onChange: (value: string) => void;
  /** Saved Connection id from node.data.connectionRefs['postgresql_connection'], if linked */
  connectionId?: string;
}

/**
 * PostgreSQL Query field with a "Browse Tables" helper. The Postgres node is raw
 * SQL against whichever database the linked connection points to, so this lists
 * the public tables and previews a few sample rows so the user can see real
 * column names/values before writing the query.
 */
export default function PostgresQueryEditor({ value, onChange, connectionId }: PostgresQueryEditorProps) {
  ...
  const [previewCache, setPreviewCache] = useState<Record<string, PostgresTablePreview>>({});
  ...
  const openBrowser = async () => {
    ...
    setError(null);
    try {
      const result = await fetchPostgresTablesForConnection(connectionId);
      setTables(result);
    } catch (err: any) {
  ...
  const toggleTable = async (table: string) => {
    ...
    try {
      const preview = await fetchPostgresTablePreview(connectionId, table);
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            title={connectionId ? 'Browse tables in the connected database' : 'Connect a PostgreSQL account first'}
          >
