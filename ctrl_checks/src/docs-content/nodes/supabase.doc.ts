...
function rich(label: string, meaning: string, enter: string, wrong: string, later: string, source = 'Use your Supabase project settings, a saved Supabase credential, or map safe workflow data such as {{$json.userId}}.'): string {
  return (
    ...
    `Common mistake: Choosing the panel's Raw SQL/query option. The supabase runtime does not execute raw SQL; use PostgreSQL or database_read/write for SQL text.`
  );
}
...
const fields: FieldDoc[] = [
  ...
  { name: 'Query', internalKey: 'query', type: 'textarea', required: false, description: 'Old frontend Raw SQL field. Not supported by supabase runtime.', placeholder: 'SELECT * FROM profiles', helpText: rich('Query', 'a stale Raw SQL field from the visual panel.', 'Do not use this field for supabase. Use operation select/insert/update/delete/rpc, or use PostgreSQL/database_read/database_write for raw SQL.', 'Choosing operation query returns operation must be one of: select, insert, update, delete, rpc. The query text itself is ignored by runSupabaseNode.', 'There is no SQL result path for this field; use real operation outputs such as {{$json.rows}} or {{$json.result}}.') },
