...
const apiKeyField: FieldDoc = {
  ...
  type: 'password',
  required: false,
  description: 'A custom APIKey header value, used only when Access Token is empty. This is a third, separate auth mode - not a fallback name for Access Token.',
  notes: 'Most SAP systems use OAuth2/Bearer (Access Token) or Basic Auth (Username/Password). This field exists specifically for systems that authenticate with a custom APIKey HTTP header instead - notably the SAP Business Accelerator Hub sandbox (api.sap.com) used to try out standard APIs before connecting to a real SAP tenant. Priority order at runtime is: Access Token first, then this API Key, then Basic Auth Username/Password last - only the first one that has a value is used.',
  helpText: "What this field is: A value sent in a custom APIKey request header, for SAP systems/sandboxes that authenticate that way instead of OAuth2 or Basic Auth.\nWhy it matters: SAP's own sandbox environment (the API Business Accelerator Hub at api.sap.com) issues this kind of key so developers can try real API shapes with sample data before ever connecting to a live SAP tenant - neither Access Token nor Basic Auth works there.\nWhen to fill it: Only when your target SAP system explicitly documents APIKey header authentication (most commonly the SAP Business Accelerator Hub sandbox). Leave blank for a real S/4HANA/ECC/Business One system using OAuth2 or Basic Auth - filling this in has no effect there since it is never checked unless Access Token is also empty, and a real SAP system would reject an APIKey header anyway.\nWhat to enter: The exact API key value SAP's sandbox (or your API-key-based SAP system) issued to you, with no prefix.\nWhere the value comes from: Sign in at api.sap.com, open the specific API you want to try (for example the Sales Order API), and copy the sandbox API key it provides for that API.\nHow to use it later: Never included in the node output.\nAccepted format: A single API key string.\nReal workplace example: Prototyping a sales-order lookup against the SAP Business Accelerator Hub sandbox before the real S/4HANA OAuth2 connection is ready, using the sandbox's own sample data.\nIf it is empty or wrong: If Access Token, this field, and Username/Password are all empty, the node returns {{$json._error}} = \"SAP node: authentication required — provide accessToken (OAuth2), apiKey, or username + password (Basic Auth)\"; a wrong key returns a SAP/sandbox authentication error.\nCommon mistake: Filling this in for a real production SAP tenant expecting it to work like an API token - production SAP systems use OAuth2 or Basic Auth, not this header; use Access Token or Username/Password instead.",
...
const usernameField: FieldDoc = {
  ...
  type: 'string',
  required: false,
  description: 'SAP Basic Auth username, used only when Access Token and API Key are both empty. Not confirmed to auto-fill from a saved connection - see notes.',
...
const sharedFields: FieldDoc[] = [operationField, baseUrlField, endpointField, accessTokenField, apiKeyField, usernameField, passwordField];
...
export const sapDoc: NodeDoc = {
  ...
  commonErrors: [
    ...
    {
      error: 'SAP node: authentication required — provide accessToken (OAuth2), apiKey, or username + password (Basic Auth)',
      cause: 'None of Access Token, API Key, or a complete Username + Password pair was supplied.',
      fix: 'Connect SAP in CtrlChecks -> Connections (Access Token), fill API Key for a sandbox/API-key-based system, or fill both Basic Auth Username and Basic Auth Password directly on the node.',
    },
    {
