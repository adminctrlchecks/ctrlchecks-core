...
export const odooDoc: NodeDoc = {
  ...
  credentialType: 'Odoo Credentials (Username/Password auto-fill from a saved connection; URL/Database are entered per-node)',
  credentialSetupSteps: [
    'Where to start: If your Odoo instance is version 14 or later, generate a dedicated API key instead of using a real password - go to your Odoo user avatar -> My Profile -> Security -> New API Key.',
    'How to connect: Save your Username and Password/API key as an "Odoo Credentials" connection in CtrlChecks Connections, then select it on this node - both fields auto-fill at execution time. Still type the Odoo instance URL and Database name directly into this node\'s own fields, since those identify a specific instance rather than a secret.',
    'Test it: Select your Odoo Credentials connection, fill URL/Database, set Operation to Get Records with Model set to res.partner, run the node, and confirm CtrlChecks returns real Odoo contact data instead of an authentication error.',
