...
| Shopify | `shopify`, `shopify_trigger` | API key | https://www.shopify.com/pricing (free dev store) — **Shopify action node manually tested 2026-08-06 and confirmed working end-to-end.** A real Shopify store (`bs6vxw-66.myshopify.com`) was activated on the lowest trial path, a custom app named CtrlChecks was created/released/installed from the Shopify Dev Dashboard, a real Admin API access token was generated via the client-credentials flow, and the CtrlChecks Shopify node successfully called the live Shopify Admin API. Verified the auth/list path and created a real draft product (`CtrlChecks Test Product`, product id `7502207975509`). Root-caused and fixed shared platform issues found during the pass: Shopify Admin API credentials were incorrectly frontend-gated as "Coming soon"; saved Shopify domains needed normalization; credential test requests needed shared template rendering; JSON payload fields were incorrectly treated as post-freeze structural drift; and empty optional runtime-owned fields could incorrectly fail handoff preflight. The `shopify_trigger` node is still separate and must receive a real signed Shopify webhook event before being marked verified working. |
