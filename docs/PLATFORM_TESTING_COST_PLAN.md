...
- **~40** platforms are testable at **$0** on a free tier or self-hosted
- **~13** platforms need a real payment to test properly
...
| Bitbucket | `bitbucket` | OAuth token | https://bitbucket.org/product/pricing |
...
| Salesforce | `salesforce` | OAuth | https://developer.salesforce.com/free-trials (free dev org) |
...
| Twitter / X | `twitter` | OAuth | https://developer.x.com/en/portal/products |
...
| Mailgun | `mailgun` | API key | https://www.mailgun.com/pricing | **Verified 2026-08-03: no free tier at signup** — checkout requires a card and the cheapest plan (Basic 10k) is **$15/month** (~$14.03 first prorated payment). Deferred; node not yet manually tested. |
...
| Zoom | `zoom_video` | OAuth 2.0 | https://zoom.us/pricing | **Verified 2026-08-03: signup itself is free** (Basic plan, no card), and a Zoom OAuth app already exists in the worker's env (`ZOOM_CLIENT_ID`/`ZOOM_CLIENT_SECRET`) — but connecting failed live with `errorCode 4700 "Invalid redirect url"`, meaning the currently-registered app's redirect URL doesn't match the worker's actual callback (`http://localhost:3001/api/credential-connections/oauth/callback` locally). This surfaced mid-way through migrating the Zoom OAuth app to the shared `adminctrlchecks@gmail.com` account (already in progress for other providers — GitHub/LinkedIn/Notion/Slack/HubSpot done). New app creation also needs phone number verification on the Zoom Marketplace developer account. Deferred until the new OAuth app is created and its redirect URLs + `worker/.env` are updated; node not yet manually tested. |
