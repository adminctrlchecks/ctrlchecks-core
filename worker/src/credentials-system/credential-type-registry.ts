...
const providerDocsUrls: Record<string, string> = {
  ...
  chargebee: 'https://apidocs.chargebee.com/docs/api?prod_cat_ver=2#api_authentication',
  ...
  google_cloud_storage: 'https://cloud.google.com/iam/docs/keys-create-delete',
  ...
  sql_server: 'https://learn.microsoft.com/sql/relational-databases/security/choose-an-authentication-mode',
  stripe: 'https://docs.stripe.com/keys',
  db: 'https://db.com/docs/guides/api/api-keys',
  tally: 'https://tally.so/help/tally-api',
  telegram: 'https://core.telegram.org/bots/features#botfather',
  timescaledb: 'https://docs.timescale.com/use-timescale/latest/security/',
  ...
  workday: 'https://community.workday.com/articles/1084547',
...
export const credentialTypeDefinitions: CredentialTypeDefinition[] = addCredentialGuides([
  ...
  // ─── Slack ──────────────────────────────────────────────────────────────────
  {
    ...
    inputFields: [
      {
        ...
        helpText: 'Required for Slack Trigger signature validation. Find it in Slack app settings -> Basic Information -> Signing Secret.',
        secret: true,
      },
    ],
    ...
    oauth2: {
      ...
      defaultScopes: ['chat:write', 'channels:read', 'users:read', 'app_mentions:read', 'channels:history', 'groups:history', 'im:history', 'mpim:history', 'commands'],
      scopeSeparator: ',',
    },
    refresh: { enabled: false, refreshBeforeSeconds: 300 },
    maskFields: ['access_token', 'refresh_token', 'authed_user', 'signingSecret', 'signing_secret'],
  },
  {
    id: 'slack_webhook',
    ...
    inputFields: [
      {
        ...
        required: true,
        secret: true,
        helpText: 'Create at gitlab.com/-/user_settings/personal_access_tokens. Select the "api" scope so CtrlChecks can also register/unregister GitLab Trigger webhooks on your behalf, not just read/write project data.',
      },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Token', testLabel: 'Test Token' },
    validation: { requiredFields: ['token'] },
  ...
  // ─── Trello ─────────────────────────────────────────────────────────────────
  {
    ...
    inputFields: [
      ...
      },
      {
        ...
        required: false,
        secret: true,
        helpText: 'Required for Trello Trigger webhook signature validation. Copy from the Trello API Key page.',
      },
    ],
    ...
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['apiKey', 'token', 'appSecret'],
  },
    ...
    inputFields: [
      {
        ...
        helpText: 'Azure Bot registration -> Configuration -> Microsoft App ID.',
      },
      {
        name: 'appPassword',
        ...
        required: false,
        secret: true,
        helpText: 'Azure app registration client secret. Required for Teams action replies through Bot Framework.',
      },
      {
        ...
        required: false,
        secret: true,
        helpText: 'Optional CtrlChecks shared secret for test/simulation requests. Production Teams Bot Framework requests use JWT validation.',
      },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Teams Bot', testLabel: 'Test Teams Bot' },
    validation: { requiredFields: ['appId'] },
    injection: [{ target: 'header', name: 'X-Microsoft-App-Id', valueTemplate: '{{appId}}' }],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['appPassword', 'validationSecret'],
  },
  {
    id: 'discord_webhook',
    ...
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['webhookUrl', 'headerName'],
  },
  {
    ...
    inputFields: [
      ...
      },
      {
        ...
        helpText: 'Get from discord.com/developers/applications -> your app -> General Information -> Public Key. Required for Discord Trigger signature validation.',
      },
      {
        name: 'applicationId',
        ...
        helpText: 'Get from discord.com/developers/applications -> your app -> General Information -> Application ID. Used for interaction follow-up replies.',
      },
    ],
  ...
  // ─── WhatsApp Business ───────────────────────────────────────────────────────
  {
    ...
    injection: [{ target: 'header', name: 'Authorization', valueTemplate: 'Bearer {{token}}' }],
    testRequest: { method: 'GET', url: 'https://graph.facebook.com/v19.0/{{phoneNumberId}}?fields=display_phone_number,verified_name', successStatus: [200] },
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['token', 'publicKey'],
  },
    ...
    inputFields: [
      {
        ...
        helpText: 'From your Supabase project → Settings → API → Project URL',
      },
      {
        name: 'token',
        ...
        required: true,
        secret: true,
        helpText: 'From your Supabase project → Settings → API → Project API keys → service_role',
      },
    ],
  ...
  // ─── Shopify ──────────────────────────────────────────────────────────────────
  {
    ...
    inputFields: [
      ...
      },
      {
        ...
        required: false,
        secret: true,
        helpText: 'Required for Shopify Trigger HMAC validation. Use the app client secret shown in Shopify app API credentials.',
      },
    ],
    ...
    inputFields: [
      {
        ...
        required: true,
        secret: true,
        helpText: 'Create at tally.so/settings/api under Personal Access Tokens.',
      },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Token', testLabel: 'Test Tally' },
    validation: { requiredFields: ['token'] },
    injection: [{ target: 'header', name: 'Authorization', valueTemplate: 'Bearer {{token}}' }],
    testRequest: { method: 'GET', url: 'https://api.tally.so/me', successStatus: [200] },
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['token'],
  },
  ...
  },
  {
    ...
    authType: 'oauth2',
    requiredScopes: ['public_profile', 'email', 'pages_show_list', 'pages_read_engagement', 'pages_manage_metadata', 'pages_manage_posts', 'pages_manage_engagement', 'pages_messaging', 'leads_retrieval'],
    inputFields: [],
    ...
    testRequest: { method: 'GET', url: 'https://graph.facebook.com/v19.0/me/accounts?fields=id,name,access_token&limit=5', successStatus: [200] },
    oauth2: {
      provider: 'facebook',
  ...
  },
  {
    ...
    authType: 'oauth2',
    requiredScopes: ['instagram_basic', 'instagram_content_publish', 'instagram_manage_messages', 'instagram_manage_comments', 'pages_show_list', 'pages_read_engagement', 'pages_manage_metadata', 'business_management'],
    inputFields: [],
    form: { layout: 'stacked', oauthButtonLabel: 'Connect Instagram', testLabel: 'Test Instagram' },
    validation: { requiredFields: [] },
    injection: [{ target: 'query', name: 'access_token', valueTemplate: '{{access_token}}' }],
    testRequest: { method: 'GET', url: 'https://graph.facebook.com/v19.0/me/accounts?fields=id,name,instagram_business_account', successStatus: [200] },
    oauth2: {
      provider: 'instagram',
      ...
      defaultScopes: ['instagram_basic', 'instagram_content_publish', 'instagram_manage_messages', 'instagram_manage_comments', 'pages_show_list', 'pages_read_engagement', 'pages_manage_metadata', 'business_management'],
      scopeSeparator: ',',
    },
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: [],
  ...
  {
    id: 'oracle_connection',
    provider: 'oracle_database',
    displayName: 'Oracle Database Connection',
    authType: 'basic_auth',
    inputFields: [
      { name: 'connectionString', label: 'Connection String', type: 'text', required: true, placeholder: 'localhost:1521/XEPDB1', helpText: 'Easy Connect string: host:port/service_name (e.g. mydb.example.com:1521/ORCLPDB1)' },
      { name: 'user', label: 'Username', type: 'text', required: true, placeholder: 'system' },
      { name: 'password', label: 'Password', type: 'password', required: true, secret: true },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Connection', testLabel: 'Test Oracle' },
    validation: { requiredFields: ['connectionString', 'user', 'password'] },
    injection: [],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['password'],
  },
  ...
  {
    id: 'contentful_cma_token',
    provider: 'contentful',
    displayName: 'Contentful CMA Personal Access Token',
    authType: 'bearer_token',
    inputFields: [
      {
        name: 'token',
        ...
        secret: true,
        placeholder: 'CFPAT-...',
        helpText: 'Create in Contentful under Settings -> CMA tokens, then click Authorize on the token row for the target organization/space. Use a token that starts with CFPAT-.',
      },
    ],
    guide: {
      summary: 'Use a Contentful Content Management API personal access token to manage entries, assets, and content models.',
      prerequisites: [
        'A Contentful account and space.',
        'Permission to create Content Management API personal access tokens.',
        'Permission to authorize the token for the target organization/space.',
      ],
      steps: [
        'Open Contentful and choose your space.',
        'Go to Settings -> CMA tokens.',
        'Create a personal access token and copy it immediately.',
        'In the CMA tokens list, click Authorize for the new token and grant access to the organization/space.',
        'Paste the CFPAT token here and save the connection.',
      ],
      fieldGuides: {
        token: {
          label: 'CMA Personal Access Token',
          description: 'The Contentful Management API token used for read/write content management calls.',
          whereToFind: 'Contentful -> Settings -> CMA tokens -> Create personal access token, then click Authorize on the token row.',
          example: 'CFPAT-...',
          notes: [
            'Content Delivery and Preview API tokens are not valid for this node.',
            'If Contentful shows an Authorize action for the CMA token, complete it before using the token in CtrlChecks.',
          ],
        },
      },
      securityNotes: [
        'Contentful shows this token only once.',
        'Keep the token private and revoke it from Contentful if it is exposed.',
      ],
      docsUrl: 'https://www.contentful.com/developers/docs/references/content-management-api/',
      troubleshooting: [
        'If Contentful returns 401 with OrganizationAccessGrantRequired, the token is real but has not been authorized for that organization/space. Go to Contentful -> Settings -> CMA tokens and click Authorize on the token row.',
        'If Contentful returns 401 AccessTokenInvalid, confirm you used a CMA personal access token, not a Delivery or Preview token.',
        'If Contentful returns 404, confirm the Space ID and environment name.',
      ],
    },
    form: { layout: 'stacked', submitLabel: 'Save Contentful Token', testLabel: 'Test Contentful' },
    validation: { requiredFields: ['token'] },
    injection: [{ target: 'header', name: 'Authorization', valueTemplate: 'Bearer {{token}}' }],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['token'],
    ...
    inputFields: [
      {
        ...
        required: true,
        placeholder: 'admin',
        helpText: 'The WordPress user the Application Password belongs to. Posts are created/updated as this user.',
      },
      {
        name: 'password',
        ...
        helpText: 'Generate in WP Admin -> Users -> Profile -> Application Passwords. This is NOT your login password.',
      },
    ],
    guide: {
      summary: 'Use a WordPress Application Password to create, read, update, and delete posts via the WordPress REST API.',
      prerequisites: [
        'A self-hosted WordPress site (5.6+) with the REST API reachable over HTTPS.',
        'A WordPress user with permission to manage posts (Author role or higher).',
        'Application Passwords enabled (on by default for HTTPS sites).',
      ],
      steps: [
        'Log in to your WordPress admin dashboard (https://yoursite.com/wp-admin).',
        'Go to Users -> Profile (or Users -> All Users -> click your user).',
        'Scroll down to the "Application Passwords" section.',
        'Enter a name like "CtrlChecks" and click "Add New Application Password".',
        'Copy the generated password immediately - WordPress shows it only once.',
        'Enter your WordPress username and the generated password here, then save.',
      ],
      fieldGuides: {
        username: {
          label: 'WordPress Username',
          description: 'The username of the WordPress account that owns the Application Password.',
          whereToFind: 'WP Admin -> Users -> your profile. Use the username (login name), not the display name or email.',
          example: 'admin',
          notes: ['Posts created through the API are attributed to this user.'],
        },
        password: {
          label: 'Application Password',
          description: 'A per-application password generated by WordPress for API access.',
          whereToFind: 'WP Admin -> Users -> Profile -> Application Passwords -> Add New Application Password.',
          example: 'abcd efgh ijkl mnop qrst uvwx',
          notes: [
            'This is different from the account login password - never use the login password here.',
            'Spaces in the generated password are OK; WordPress accepts it with or without spaces.',
          ],
        },
      },
      securityNotes: [
        'WordPress shows the Application Password only once at creation.',
        'Revoke it anytime from the same Application Passwords section without changing your login password.',
        'Application Passwords only work over HTTPS on production sites.',
      ],
  ...
  // ─── Workday ──────────────────────────────────────────────────────────────────
  {
    id: 'workday_connection',
    provider: 'workday',
    displayName: 'Workday',
    authType: 'bearer_token',
    inputFields: [
      { name: 'tenant', label: 'Tenant', type: 'text', required: false, placeholder: 'yourtenant', helpText: 'Your Workday tenant identifier.' },
      { name: 'baseUrl', label: 'REST API Base URL', type: 'text', required: false, placeholder: 'https://wd2-impl-services1.workday.com/ccx/api/v1/yourtenant', helpText: 'Leave blank to use the default Workday base URL built from your tenant.' },
      { name: 'accessToken', label: 'Access Token', type: 'password', required: false, secret: true, helpText: 'OAuth 2.0 Bearer token from a Workday Registered API Client. Provide this or a Username + Password.' },
      { name: 'username', label: 'Username', type: 'text', required: false, helpText: 'Workday integration system user (used when no Access Token is provided).' },
      { name: 'password', label: 'Password', type: 'password', required: false, secret: true, helpText: 'Password for the Workday integration system user.' },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Connection', testLabel: 'Test Workday' },
    validation: { requiredFields: [] },
    injection: [],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
  ...
  // ─── Chargebee ────────────────────────────────────────────────────────────────
  {
    id: 'chargebee_connection',
    provider: 'chargebee',
    displayName: 'Chargebee',
    authType: 'api_key',
    inputFields: [
      { name: 'site', label: 'Site', type: 'text', required: true, placeholder: 'mysite', helpText: 'Your Chargebee site subdomain, e.g. "mysite" for mysite.chargebee.com.' },
      { name: 'apiKey', label: 'API Key', type: 'password', required: true, secret: true, helpText: 'Chargebee full-access API key from Settings → API Keys.' },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Connection', testLabel: 'Test Chargebee' },
    validation: { requiredFields: ['site', 'apiKey'] },
    injection: [],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['apiKey'],
  },
  ...
  {
    id: 'google_cloud_storage_connection',
    provider: 'google_cloud_storage',
    displayName: 'Google Cloud Storage',
    authType: 'api_key',
    inputFields: [
      { name: 'projectId', label: 'Project ID', type: 'text', required: true, placeholder: 'my-gcp-project-id' },
      { name: 'clientEmail', label: 'Service Account Email', type: 'text', required: true, placeholder: 'workflow-service@project-id.iam.gserviceaccount.com' },
      { name: 'privateKey', label: 'Private Key', type: 'textarea', required: true, secret: true, helpText: 'Paste the full PEM private key from the service account JSON key file, including the -----BEGIN and -----END lines.' },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Connection', testLabel: 'Test GCS' },
    validation: { requiredFields: ['projectId', 'clientEmail', 'privateKey'] },
    injection: [],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['privateKey'],
  },
  ...
  // ─── Netlify ──────────────────────────────────────────────────────────────────
  {
    ...
    authType: 'bearer_token',
    inputFields: [
      { name: 'accessToken', label: 'Personal Access Token', type: 'password', required: true, secret: true, helpText: 'From Netlify → User settings → Applications → Personal access tokens.' },
    ],
    form: { layout: 'stacked', submitLabel: 'Save Connection', testLabel: 'Test Netlify' },
    validation: { requiredFields: ['accessToken'] },
    injection: [],
    refresh: { enabled: false, refreshBeforeSeconds: 0 },
    maskFields: ['accessToken'],
  },
  ...
  // ─── SQL Server ───────────────────────────────────────────────────────────────
  {
    ...
    inputFields: [
      { name: 'host', label: 'Host', type: 'text', required: true, placeholder: 'localhost' },
      { name: 'port', label: 'Port', type: 'number', required: false, defaultValue: 1433 },
      { name: 'database', label: 'Database', type: 'text', required: true },
      { name: 'username', label: 'Username', type: 'text', required: true },
      { name: 'password', label: 'Password', type: 'password', required: true, secret: true },
    ],
