...
import {
  ...
  getGithubDeliveryHeader,
  getGithubEventHeader,
...
} from '../services/github/github-trigger-service';
...
function findGithubTriggerNode(workflow: any, nodeId: string): any {
  const node = workflowNodes(workflow).find((candidate) => String(candidate?.id || '') === nodeId);
  if (!node || nodeTypeOf(node) !== 'github_trigger') {
    throw Object.assign(new Error('GitHub Trigger node not found in this workflow.'), { statusCode: 404 });
  }
  return node;
}
...
async function startWorkflowExecution(input: {
  ...
  const { data: execution, error } = await db
    ...
    .insert({
      ...
      trigger: normalizeExecutionTrigger('github', 'webhook'),
      input: input.executionInput,
      logs: [],
      started_at: new Date().toISOString(),
      metadata: {
        originalTrigger: 'github',
  ...
  if (error || !execution) {
    logger.error('[GitHub Trigger] Execution creation error:', error);
  ...
  if (!executeUrl) {
    const message = 'PUBLIC_BASE_URL is required to execute GitHub-triggered workflows.';
  ...
  fetch(executeUrl, {
    ...
    headers: {
      ...
      'X-Internal-Github-Execution': 'true',
    ...
    const message = `GitHub trigger handoff failed (${response.status}): ${details || response.statusText}`;
    logger.error('[GitHub Trigger] Error triggering workflow execution:', message);
    await markExecutionFailed(db, execution.id, message);
  }).catch(async (err) => {
    const message = err instanceof Error ? err.message : String(err);
    logger.error('[GitHub Trigger] Error triggering workflow execution:', err);
    await markExecutionFailed(db, execution.id, `GitHub trigger handoff failed: ${message}`);
  });

  return execution;
}

export async function githubWebhookInfoHandler(req: Request, res: Response) {
  try {
    ...
    await loadWorkflow(workflowId).then((workflow) => findGithubTriggerNode(workflow, nodeId));
    return res.json({
      success: true,
      webhookUrl: getGithubWebhookUrl(workflowId, nodeId),
      message: 'This URL is registered automatically as a GitHub repository webhook for the configured owner/repo when the workflow is active.',
    });
  } catch (error) {
...
export async function githubWebhookHandler(req: Request, res: Response) {
  try {
    ...
    if (!await validateGithubWebhookSecret(workflowId, nodeId, rawBody, signature)) {
      return res.status(401).json({ error: 'Invalid GitHub webhook signature.' });
    }

    const githubEvent = getGithubEventHeader(req);
    const deliveryId = getGithubDeliveryHeader(req);

    // GitHub sends a `ping` event when a webhook is first created. Acknowledge it
    // without starting a workflow execution.
    if (githubEvent.toLowerCase() === 'ping') {
      return res.status(200).json({ message: 'GitHub webhook ping received.' });
    }
    ...
    const node = findGithubTriggerNode(workflow, nodeId);
    const triggerConfig = nodeConfigOf(node) as GithubTriggerConfig;

    const normalized = normalizeGithubEvent(githubEvent, deliveryId, req.body || {});
    if (!normalized) {
      return res.status(200).json({ message: 'Ignored non-actionable GitHub payload.' });
    }

    if (!shouldAcceptGithubEvent(normalized, triggerConfig).accepted) {
      return res.status(200).json({ message: 'Ignored GitHub event not matching this trigger.' });
    }

    const executionInput = buildGithubExecutionInput({ workflowId, nodeId, normalized });
    ...
    const statusCode = (error as any)?.statusCode || 500;
    const message = error instanceof Error ? error.message : String(error);
    logger.error('[GitHub Trigger] Webhook error:', error);
    return res.status(statusCode).json({ error: message });
  }
}

export async function githubRegisterWebhookHandler(req: Request, res: Response) {
  try {
    ...
    const result = await registerGithubWebhook({
      ...
      workflowId,
      nodeId,
      owner: String(req.body?.owner || triggerConfig.owner || ''),
      repo: String(req.body?.repo || triggerConfig.repo || ''),
...
export async function githubUnregisterWebhookHandler(req: Request, res: Response) {
  try {
    const userId = getAuthenticatedUserId(req);
    const workflowId = String(req.body?.workflowId || '');
    const nodeId = String(req.body?.nodeId || '');
    ...
    const result = await unregisterGithubWebhook({ userId, workflowId, nodeId });
    return res.json(result);
  } catch (error) {
    const statusCode = (error as any)?.statusCode || 500;
    const message = error instanceof Error ? error.message : String(error);
