...
} from '../services/gitlab/gitlab-trigger-service';
...
function findGitlabTriggerNode(workflow: any, nodeId: string): any {
  const node = workflowNodes(workflow).find((candidate) => String(candidate?.id || '') === nodeId);
  if (!node || nodeTypeOf(node) !== 'gitlab_trigger') {
    throw Object.assign(new Error('GitLab Trigger node not found in this workflow.'), { statusCode: 404 });
  }
  return node;
}
...
async function startWorkflowExecution(input: {
  ...
  const { data: execution, error } = await db
    ...
    .insert({
      ...
      trigger: normalizeExecutionTrigger('gitlab', 'webhook'),
      input: input.executionInput,
      logs: [],
      started_at: new Date().toISOString(),
      metadata: {
        originalTrigger: 'gitlab',
  ...
  if (error || !execution) {
    logger.error('[GitLab Trigger] Execution creation error:', error);
  ...
  if (!executeUrl) {
    const message = 'PUBLIC_BASE_URL is required to execute GitLab-triggered workflows.';
  ...
  fetch(executeUrl, {
    ...
    headers: {
      ...
      'X-Internal-Gitlab-Execution': 'true',
    ...
    const message = `GitLab trigger handoff failed (${response.status}): ${details || response.statusText}`;
    logger.error('[GitLab Trigger] Error triggering workflow execution:', message);
    await markExecutionFailed(db, execution.id, message);
  }).catch(async (err) => {
    const message = err instanceof Error ? err.message : String(err);
    logger.error('[GitLab Trigger] Error triggering workflow execution:', err);
    await markExecutionFailed(db, execution.id, `GitLab trigger handoff failed: ${message}`);
  });

  return execution;
}

export async function gitlabWebhookInfoHandler(req: Request, res: Response) {
  try {
    ...
    await loadWorkflow(workflowId).then((workflow) => findGitlabTriggerNode(workflow, nodeId));
    return res.json({
      success: true,
      webhookUrl: getGitlabWebhookUrl(workflowId, nodeId),
      message: 'This URL is registered automatically as a GitLab project webhook for the configured project when the workflow is active. Validation uses the X-Gitlab-Token shared secret header, not an HMAC signature.',
    });
  } catch (error) {
...
export async function gitlabWebhookHandler(req: Request, res: Response) {
  try {
    ...
    const tokenHeader = getGitlabTokenHeader(req);
    if (!await validateGitlabWebhookSecret(workflowId, nodeId, tokenHeader)) {
      return res.status(401).json({ error: 'Invalid or missing X-Gitlab-Token secret.' });
    }
    ...
    const node = findGitlabTriggerNode(workflow, nodeId);
    const triggerConfig = nodeConfigOf(node) as GitlabTriggerConfig;

    const normalized = normalizeGitlabEvent(req.body || {});
    if (!normalized) {
      return res.status(200).json({ message: 'Ignored non-actionable GitLab payload.' });
    }

    if (!shouldAcceptGitlabEvent(normalized, triggerConfig).accepted) {
      return res.status(200).json({ message: 'Ignored GitLab event not matching this trigger.' });
    }

    const executionInput = buildGitlabExecutionInput({ workflowId, nodeId, normalized });
    ...
    const statusCode = (error as any)?.statusCode || 500;
    const message = error instanceof Error ? error.message : String(error);
    logger.error('[GitLab Trigger] Webhook error:', error);
    return res.status(statusCode).json({ error: message });
  }
}

export async function gitlabRegisterWebhookHandler(req: Request, res: Response) {
  try {
    ...
    const result = await registerGitlabWebhook({
      ...
      workflowId,
      nodeId,
      baseUrl: String(req.body?.baseUrl || triggerConfig.baseUrl || ''),
      projectId: String(req.body?.projectId || triggerConfig.projectId || ''),
...
export async function gitlabUnregisterWebhookHandler(req: Request, res: Response) {
  try {
    const userId = getAuthenticatedUserId(req);
    const workflowId = String(req.body?.workflowId || '');
    const nodeId = String(req.body?.nodeId || '');
    ...
    const result = await unregisterGitlabWebhook({ userId, workflowId, nodeId });
    return res.json(result);
  } catch (error) {
    const statusCode = (error as any)?.statusCode || 500;
    const message = error instanceof Error ? error.message : String(error);
