...
import {
  ...
  unregisterFacebookWebhook,
  validateFacebookSignature,
  validateFacebookVerifyToken,
  type FacebookTriggerConfig,
} from '../services/facebook/facebook-trigger-service';
...
function findFacebookTriggerNode(workflow: any, nodeId: string): any {
  const node = workflowNodes(workflow).find((candidate) => String(candidate?.id || '') === nodeId);
  if (!node || nodeTypeOf(node) !== 'facebook_trigger') {
    throw Object.assign(new Error('Facebook Trigger node not found in this workflow.'), { statusCode: 404 });
  }
  return node;
}
...
async function startWorkflowExecution(input: {
  ...
  const { data: execution, error } = await db
    ...
    .insert({
      ...
      trigger: normalizeExecutionTrigger('facebook', 'webhook'),
      input: input.executionInput,
      logs: [],
      started_at: startedAt,
      metadata: {
        originalTrigger: 'facebook',
  ...
  if (error || !execution) {
    logger.error('[Facebook Trigger] Execution creation error:', error);
  ...
  if (!executeUrl) {
    const message = 'PUBLIC_BASE_URL is required to execute Facebook-triggered workflows.';
  ...
  fetch(executeUrl, {
    ...
    headers: {
      ...
      'X-Internal-Facebook-Execution': 'true',
    ...
    const message = `Facebook trigger handoff failed (${response.status}): ${details || response.statusText}`;
    logger.error('[Facebook Trigger] Error triggering workflow execution:', message);
    await markExecutionFailed(db, execution.id, message);
  }).catch(async (err) => {
    const message = err instanceof Error ? err.message : String(err);
    logger.error('[Facebook Trigger] Error triggering workflow execution:', err);
    await markExecutionFailed(db, execution.id, `Facebook trigger handoff failed: ${message}`);
  });

  return execution;
}

export async function facebookWebhookVerifyHandler(req: Request, res: Response) {
  try {
    ...
    const node = findFacebookTriggerNode(workflow, nodeId);
    const triggerConfig = nodeConfigOf(node) as FacebookTriggerConfig;
    const challenge = validateFacebookVerifyToken(req, triggerConfig);
    if (!challenge) return res.status(403).send('Invalid verify token.');
    return res.status(200).send(challenge);
  } catch (error) {
    const statusCode = (error as any)?.statusCode || 500;
    const message = error instanceof Error ? error.message : String(error);
    logger.error('[Facebook Trigger] Verification error:', error);
...
export async function facebookWebhookHandler(req: Request, res: Response) {
  try {
    ...
    const node = findFacebookTriggerNode(workflow, nodeId);
    const triggerConfig = nodeConfigOf(node) as FacebookTriggerConfig;
    if (!validateFacebookSignature(req, triggerConfig)) {
      return res.status(401).json({ error: 'Invalid Facebook webhook signature.' });
    }

    const normalizedEvents = normalizeFacebookWebhookPayload(req.body || {});
    const acceptedEvents = normalizedEvents.filter((event) => shouldAcceptFacebookEvent(event, triggerConfig).accepted);
    ...
    if (acceptedEvents.length === 0) {
      return res.json({
        ...
        message: 'Facebook webhook received, no events matched this trigger.',
      });
    }

    const executions = [];
    for (const normalized of acceptedEvents) {
      const executionInput = buildFacebookExecutionInput({ workflowId, nodeId, normalized });
    ...
    return res.json({
      ...
      status: 'running',
      message: 'Facebook webhook received, workflow execution started.',
    });
  } catch (error) {
    const statusCode = (error as any)?.statusCode || 500;
    const message = error instanceof Error ? error.message : String(error);
    logger.error('[Facebook Trigger] Webhook error:', error);
    return res.status(statusCode).json({ error: message });
  }
}

export async function facebookRegisterWebhookHandler(req: Request, res: Response) {
  try {
    ...
    const result = await registerFacebookWebhook({
      ...
      pageId: String(req.body?.pageId || triggerConfig.pageId || ''),
...
export async function facebookUnregisterWebhookHandler(req: Request, res: Response) {
  try {
    const userId = getAuthenticatedUserId(req);
    const result = await unregisterFacebookWebhook({
      userId,
