    ...
    return {
      ...
      configSchema: {
        ...
        optional: {
          operation: {
            type: 'string',
            description: 'Operation: now, format, add, subtract, diff, convertTimezone, getTimezoneInfo',
            examples: ['now', 'format', 'add', 'subtract', 'diff', 'convertTimezone', 'getTimezoneInfo'],
            default: 'now',
            fillMode: { default: 'buildtime_ai_once', supportsRuntimeAI: false, supportsBuildtimeAI: true },
          },
          date: {
            type: 'string',
            description: 'Base date for format, add, subtract, diff, timezone conversion, or timezone info. Leave empty to use the current date/time.',
            examples: ['{{$json.timestamp}}', '2026-07-12T09:00:00Z'],
            visibleIf: { field: 'operation', equals: ['format', 'add', 'subtract', 'diff', 'convertTimezone', 'getTimezoneInfo'] },
          },
          format: {
            type: 'string',
            description: 'Output format for format operation: ISO, TIMESTAMP, LOCALE, or CUSTOM',
            examples: ['ISO', 'TIMESTAMP', 'LOCALE', 'CUSTOM'],
            default: 'ISO',
            visibleIf: { field: 'operation', equals: 'format' },
          },
          customFormat: {
            type: 'string',
            description: 'Custom format pattern used when format is CUSTOM. Supported tokens: YYYY, MM, DD, HH, mm, ss.',
            examples: ['YYYY-MM-DD HH:mm:ss'],
            visibleIf: { field: 'format', equals: 'CUSTOM' },
            requiredIf: { field: 'format', equals: 'CUSTOM' },
          },
          locale: {
            type: 'string',
            ...
            default: 'en-US',
            visibleIf: { field: 'format', equals: 'LOCALE' },
          },
          timezone: {
            type: 'string',
            description: 'IANA timezone used by now, format, convertTimezone, and getTimezoneInfo',
            examples: ['UTC', 'Asia/Kolkata', 'America/New_York'],
            visibleIf: { field: 'operation', equals: ['now', 'format', 'convertTimezone', 'getTimezoneInfo'] },
            requiredIf: { field: 'operation', equals: 'convertTimezone' },
          },
          value: {
            type: 'number',
            ...
            examples: [1, 7],
            default: 0,
            visibleIf: { field: 'operation', equals: ['add', 'subtract'] },
          },
          unit: {
            type: 'string',
            description: 'Unit for add, subtract, or diff: seconds, minutes, hours, days, weeks, months, years',
            examples: ['minutes', 'hours', 'days', 'weeks'],
            default: 'minutes',
            visibleIf: { field: 'operation', equals: ['add', 'subtract', 'diff'] },
          },
          endDate: {
            type: 'string',
            description: 'End date for diff operation',
            examples: ['{{$json.endDate}}', '2026-07-13T09:00:00Z'],
            visibleIf: { field: 'operation', equals: 'diff' },
            requiredIf: { field: 'operation', equals: 'diff' },
          },
        },
      },
      aiSelectionCriteria: {
    ...
    return {
      ...
      configSchema: {
        ...
        optional: {
          ...
          timeoutMs: { type: 'number', description: 'Global AI Agent timeout across model and tool iterations', default: 60000 },
          maxIterations: { type: 'number', description: 'Maximum reasoning/tool-call iterations before the agent stops', default: 10 },
          maxToolResultChars: { type: 'number', description: 'Maximum sanitized tool-result characters returned to the model per tool call', default: 8000 },
          maxRepeatedToolCalls: { type: 'number', description: 'Maximum identical consecutive tool calls before loop protection stops the agent', default: 3 },
          memoryScope: { type: 'string', description: 'Optional memory scope: none, conversation, or user', default: 'none' },
          requireApprovalFor: { type: 'string', description: 'Guardrail gate for side-effecting tools: none, destructive, or write_and_destructive', default: 'destructive' },
    ...
    return {
      ...
      configSchema: {
        ...
        optional: {
          ...
          operation: {
            type: 'string',
            description: 'Operation: parse, extract, toText',
            examples: ['parse', 'extract', 'toText'],
            default: 'parse',
            fillMode: { default: 'buildtime_ai_once', supportsRuntimeAI: false, supportsBuildtimeAI: true },
          },
          selector: {
      ...
      aiSelectionCriteria: {
        ...
        intentDescription: 'HTML node that parses and manipulates HTML content. Parses HTML strings into title/meta/body output, extracts selected element text, or converts HTML to plain text. Used for HTML parsing, web scraping, and extracting content from HTML documents.',
      ...
      configSchema: {
        ...
        optional: {
          ...
          csv: {
            ...
            examples: ['{{$json.csv}}'],
            visibleIf: { field: 'operation', equals: 'parse' },
          },
          data: {
            type: 'array',
            description: 'Data array (for generate)',
            examples: ['{{$json.data}}'],
            visibleIf: { field: 'operation', equals: 'generate' },
          },
          delimiter: {
            type: 'string',
            description: 'Delimiter used for parsing or generating CSV. Use \\t for tab-delimited data.',
            examples: [',', ';', '\\t', '|'],
            default: ',',
          },
          hasHeader: {
            type: 'boolean',
            description: 'Whether the first parsed row contains column names',
            examples: [true, false],
            default: true,
            visibleIf: { field: 'operation', equals: 'parse' },
          },
      ...
      configSchema: {
        ...
        optional: {
          ...
          delimiter: {
            ...
            examples: ['\\n', ', ', ' | '],
            default: '\n',
            visibleIf: { field: 'operation', equals: 'join' },
          },
        },
      },
      aiSelectionCriteria: {
      ...
      operationContracts: [
        {
          ...
          label: 'Send Email',
          requiredFields: ['from', 'to'],
          optionalFields: ['operation', 'subject', 'text', 'html'],
          forbiddenFields: [
            'apiKey',
            'credentialId',
            'templateId',
          ...
          emptyValuePolicy: {
            operation: 'optional',
            subject: 'provider_default',
            text: 'optional',
            html: 'optional',
          },
          fieldSourcePolicy: {
            from: { credentialForbidden: true },
            to: { credentialForbidden: true },
            subject: { credentialForbidden: true },
            text: { credentialForbidden: true },
            html: { credentialForbidden: true },
          ...
          providerDefaultFields: ['subject'],
          credentialProviders: ['sendgrid'],
          outputFields: ['success', 'status', 'messageId', '_error', '_errorDetails'],
          status: 'implemented',
        },
      ],
  ...
  private createZoomVideoSchema(): NodeSchema {
    return {
      ...
      configSchema: {
        ...
        optional: {
          ...
          meetingId: {
            ...
            examples: ['123456789'],
            requiredIf: { field: 'operation', equals: ['getMeeting', 'deleteMeeting', 'updateMeeting'] },
      ...
      operationContracts: [
        {
          ...
          optionalFields: ['topic', 'duration', 'startTime'],
          credentialProviders: ['zoom'],
          outputFields: ['default'],
          legacyAliases: [],
          status: 'implemented',
        },
        ...
        },
        {
          ...
          label: 'Get Meeting',
          requiredFields: ['operation', 'meetingId'],
          optionalFields: [],
          credentialProviders: ['zoom'],
          outputFields: ['default'],
          legacyAliases: [],
      ...
      aiSelectionCriteria: {
        ...
        intentDescription: 'Zoom Video integration node that creates and manages Zoom meetings using a selected Zoom OAuth connection. Supports creating, listing, getting, updating, and deleting meetings. Used for automating Zoom meeting management, scheduling video calls, and integrating Zoom into workflows.',
      ...
      schemaVersion: '1.0.0',
      capabilities: ['meeting.create', 'meeting.list', 'meeting.get', 'meeting.update', 'meeting.delete', 'zoom.api'],
      providers: ['zoom'],
      keywords: [
        'zoom', 'zoom meeting', 'zoom video', 'zoom call', 'video call',
        'zoom api', 'zoom integration', 'create meeting', 'schedule meeting',
