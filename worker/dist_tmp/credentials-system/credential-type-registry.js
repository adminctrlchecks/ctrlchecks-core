...
exports.credentialTypeDefinitions = void 0;
exports.getCredentialType = getCredentialType;
exports.getRedirectUri = getRedirectUri;
exports.logOAuthRedirectUris = logOAuthRedirectUris;
const credential_guides_1 = require("./credential-guides");
...
function optionalAuthParam(name, value) {
    return value ? { [name]: value } : {};
}
const facebookBusinessConfigId = process.env.META_FACEBOOK_CONFIG_ID ||
...
function inferFieldLocation(field, definition) {
    if (field.helpText)
        return field.helpText;
    const name = field.name.toLowerCase();
    const service = providerLabel(definition.provider);
    if (name.includes('host'))
        return `Find this in your ${service} server, database, or hosting provider connection details.`;
    if (name.includes('port'))
        return `Use the port shown in your ${service} connection details. If no custom port is listed, use the default shown in this form.`;
    if (name.includes('database'))
        return `Use the exact database name created in your ${service} server or cloud console.`;
    if (name.includes('username') || name === 'user')
        return `Use the ${service} account, database user, or integration username that has permission for this workflow.`;
    if (name.includes('password'))
        return `Use the password, app password, or generated credential for the selected ${service} user.`;
    if (name.includes('privatekey'))
        return `Use the PEM private key from your SSH key pair or server access settings.`;
    if (name.includes('serviceaccount'))
        return `Use the service account JSON downloaded from the project service account settings.`;
    if (name.includes('url') || name.includes('domain') || name.includes('subdomain'))
        return `Copy this from your ${service} account URL, API base URL, or project settings.`;
    if (name.includes('project'))
        return `Copy this from the ${service} project settings or project overview page.`;
    if (name.includes('region'))
        return `Choose the region where your ${service} account or resource is hosted.`;
    if (name.includes('secret'))
        return `Create or reveal this in the ${service} developer, API, or integration settings page.`;
    if (name.includes('token'))
        return `Create this in the ${service} developer, security, API, or personal access token settings page.`;
    if (name.includes('apikey') || name.includes('api_key') || name.includes('key'))
        return `Create or copy this from the ${service} API keys, developer, or integration settings page.`;
    if (name.includes('sid'))
        return `Copy this identifier from your ${service} project or account dashboard.`;
    if (name.includes('account') || name.includes('workspace'))
        return `Copy this from your ${service} account, workspace, or organization settings.`;
    if (name.includes('header'))
        return 'Use the exact HTTP header name and value required by the API you are connecting.';
    if (name.includes('query'))
        return 'Use the exact query parameter name and value required by the API you are connecting.';
    return `Find this value in your ${service} account settings, developer console, or integration setup page.`;
}
function buildFieldGuide(field, definition) {
    const example = !field.secret && field.placeholder
        ? field.placeholder
...
function buildGuide(definition) {
    const service = providerLabel(definition.provider);
    const isOAuth = definition.authType === 'oauth2';
    const specific = credential_guides_1.specificGuides[definition.id];
    // Per-field guide: priority = field.guide > inline fieldGuides[name] > specific fieldGuides[name] (merged) > auto
    const fieldGuides = Object.fromEntries(definition.inputFields.map((field) => {
        ...
        const resolved = field.guide ||
            inlineField ||
            (specificField ? { ...autoField, ...specificField } : null) ||
            autoField;
...
function addCredentialGuides(definitions) {
...
exports.credentialTypeDefinitions = addCredentialGuides([
    // ─── Google Suite ───────────────────────────────────────────────────────────
    {
        id: 'google_oauth2',
        provider: 'google',
    ...
    // ─── HubSpot Private App ────────────────────────────────────────────────────
    {
        ...
        form: { layout: 'stacked', submitLabel: 'Save Token', testLabel: 'Test HubSpot' },
        validation: { requiredFields: ['token'] },
        injection: [{ target: 'header', name: 'Authorization', valueTemplate: 'Bearer {{token}}' }],
        testRequest: { method: 'GET', url: 'https://api.hubapi.com/crm/v3/objects/contacts?limit=1', successStatus: [200] },
        refresh: { enabled: false, refreshBeforeSeconds: 0 },
        maskFields: ['token'],
    },
    // ─── Salesforce ─────────────────────────────────────────────────────────────
    ...
    // ─── Airtable ───────────────────────────────────────────────────────────────
    {
        ...
        guide: {
            summary: 'Create an Airtable Personal Access Token with record and schema access, then paste it here to connect Airtable to CtrlChecks.',
            prerequisites: [
            ...
            fieldGuides: {
                token: {
                    ...
                    whereToFind: 'Create it at https://airtable.com/create/tokens. The token is shown only once after creation and usually starts with pat.',
                    example: 'pat...',
                    notes: [
...
function getCredentialType(id) {
    return exports.credentialTypeDefinitions.find((definition) => definition.id === id);
}
function getRedirectUri(definition) {
...
function logOAuthRedirectUris() {
    const oauthProviders = exports.credentialTypeDefinitions.filter((d) => d.oauth2?.redirectUriEnv);
