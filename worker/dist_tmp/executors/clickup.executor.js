...
exports.executeClickUpNode = executeClickUpNode;
const clickupNode_1 = require("../services/clickup/clickupNode");
/**
 ...
 */
function convertConfigToParams(operation, config) {
    const params = {};
    // Map common fields
    if (config.listId)
        params.listId = config.listId;
    if (config.spaceId)
        params.spaceId = config.spaceId;
    if (config.folderId)
        params.folderId = config.folderId;
    if (config.taskId)
    ...
    if (operation === 'createTask' || operation === 'create_task') {
        ...
        if (typeof config.dueDate === 'number')
            params.due_date = config.dueDate;
        if (typeof config.startDate === 'number')
            params.start_date = config.startDate;
        if (typeof config.timeEstimate === 'number')
            params.time_estimate = config.timeEstimate;
    }
    // Handle update_task
    ...
    if (operation === 'getTasks' || operation === 'get_tasks_list' || operation === 'list_tasks') {
        ...
        if (config.reverse !== undefined)
            params.reverse = config.reverse;
        if (config.subtasks !== undefined)
            params.subtasks = config.subtasks;
        if (config.statuses)
            params.statuses = config.statuses;
        if (config.assignees)
            params.assignees = config.assignees;
        if (config.due_date_gt)
            params.due_date_gt = config.due_date_gt;
        if (config.due_date_lt)
            params.due_date_lt = config.due_date_lt;
        if (config.includeClosed !== undefined)
            params.include_closed = config.includeClosed;
        if (config.archived !== undefined)
            params.archived = config.archived;
    }
    ...
    if (operation === 'createComment' || operation === 'add_comment') {
        ...
        if (config.assignee !== undefined)
            params.assignee = config.assignee;
        if (config.notify_all !== undefined)
            params.notify_all = config.notify_all;
    }
    if (operation === 'getComments') {
        if (config.start !== undefined)
            params.start = config.start;
        if (config.start_id)
            params.start_id = config.start_id;
    }
    // Handle createList
    if (operation === 'createList') {
        ...
        if (config.due_date)
            params.due_date = config.due_date;
        if (config.priority)
            params.priority = config.priority;
        if (config.assignee)
            params.assignee = config.assignee;
    ...
    if (operation === 'getTimeEntries') {
        ...
        if (config.assignee)
            params.assignee = config.assignee;
        if (config.task_id)
            params.task_id = config.task_id;
        if (config.page !== undefined)
            params.page = config.page;
    }
    if (operation === 'createTimeEntry') {
        if (config.taskId || config.task_id)
            params.taskId = config.taskId || config.task_id;
        ...
        if (config.billable !== undefined)
            params.billable = config.billable;
        if (config.tags)
...
async function executeClickUpNode(node, input, credentials) {
    ...
    if (operation === 'get_tasks_space') {
        ...
        const clickupCredentials = {
            apiToken: apiKey,
            teamId: credentials?.teamId || config?.workspaceId || config?.teamId,
            baseUrl: credentials?.baseUrl,
        };
        const result = await (0, clickupNode_1.run)(clickupCredentials, {
            name: 'getTasks',
            params: spaceParams,
        });
        if (!result.success) {
    ...
    const params = convertConfigToParams(normalizedOperation, config);
    ...
    let result = await (0, clickupNode_1.run)(clickupCredentials, {
    ...
    if (!result.success &&
        ...
        result = await (0, clickupNode_1.run)(clickupCredentials, {
            name: normalizedOperation,
