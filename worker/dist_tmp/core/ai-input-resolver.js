...
exports.aiInputResolver = exports.AIInputResolver = void 0;
exports.compactForAiPrompt = compactForAiPrompt;
...
function inferRuntimeRole(fieldName, fieldDef) {
    if (fieldDef?.runtimeContract?.role)
        return fieldDef.runtimeContract.role;
    if (fieldDef?.role === 'recipient')
        return 'recipient';
    if (fieldDef?.role === 'title_like')
        return 'subject';
    if (fieldDef?.role === 'long_body' || fieldDef?.role === 'content')
        return 'body';
    const lower = fieldName.toLowerCase();
    if (lower.includes('recipient') || lower === 'to' || lower.includes('email'))
        return 'recipient';
    if (lower === 'subject' || lower === 'title' || lower === 'headline')
        return 'subject';
    if (lower.includes('body') || lower.includes('message') || lower.includes('content'))
        return 'body';
    if (lower === 'values' || lower.includes('rows'))
        return 'row_values';
    if (lower === 'conditions')
        return 'condition';
    if (lower === 'cases')
...
function inferRuntimeCardinality(fieldName, fieldDef) {
    if (fieldDef?.runtimeContract?.cardinality)
        return fieldDef.runtimeContract.cardinality;
    const lower = fieldName.toLowerCase();
    ...
    if (fieldDef?.type === 'object' || fieldDef?.type === 'json')
        return 'object';
    if (lower === 'code')
        return 'code_string';
    return 'single';
}
function truncateText(value, max = PREVIOUS_OUTPUT_PREVIEW_BUDGET) {
    return value.length > max ? `${value.slice(0, max)}... [truncated]` : value;
}
function compactValueForPrompt(value, depth = 0) {
    ...
    if (Array.isArray(value)) {
        ...
        const fieldNames = Array.from(new Set(objectItems.flatMap((item) => Object.keys(item)))).slice(0, 40);
        return {
            type: 'array',
            count: value.length,
            fieldNames,
...
function compactForAiPrompt(value, max = PREVIOUS_OUTPUT_PREVIEW_BUDGET) {
    if (value === undefined)
        return 'No previous output available';
    try {
...
class AIInputResolver {
        ...
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
            try {
                ...
                const retryContext = attempt > 0
                    ? {
                        ...context,
                        retryRequiredFields: context.retryRequiredFields && context.retryRequiredFields.length > 0
                ...
                lastError = err;
                console.warn(`[AIInputResolver] Attempt ${attempt + 1}/${MAX_ATTEMPTS} failed: ${err.message}`);
            }
        }
        throw lastError;
    }
     ...
     */
    determineResolutionMode(inputSchema, _nodeType, previousOutput) {
        ...
        const hasStructuredFields = fieldTypes.some(type => type === 'object' || type === 'array' || type === 'json');
     ...
     */
    buildResolutionPrompt(context, mode) {
        ...
        const fieldIntelligenceStr = JSON.stringify((0, node_field_intelligence_1.summarizeFieldIntelligenceForPrompt)(nodeInputSchema), null, 2);
        ...
        if (context.separatedUpstreamContext) {
            const { businessData, integrationMetadata, systemMetadata, errorData } = context.separatedUpstreamContext;
...
- Example: If previous output is {"response": "Hello"}, body should be {"message": "Hello"} or {"text": "Hello"} depending on API needs`;
        ...
        const upstreamObj = Array.isArray(previousOutput) &&
            previousOutput.length > 0 &&
            typeof previousOutput[0] === 'object'
            ? previousOutput[0]
            : previousOutput;
        const isTabularUpstream = upstreamObj !== null &&
            upstreamObj !== undefined &&
            typeof upstreamObj === 'object' &&
            !Array.isArray(upstreamObj) &&
            ('rows' in upstreamObj ||
                'count' in upstreamObj ||
                'headers' in upstreamObj ||
                'items' in upstreamObj ||
                'records' in upstreamObj);
...
- The text you produce will be used directly as input to this node. Make it concrete, specific, and ready-to-use.`;
        ...
        for (const [k, def] of Object.entries(nodeInputSchema || {})) {
            ...
            if (typeof ao === 'string' && ao.length > 0) {
                aliasHints.push(`Field "${ao}" is canonical; "${k}" duplicates the same value. In JSON mode you MUST set "${ao}" when delivering the main text.`);
            }
        }
                ...
                return `Output valid JSON in this format:
{
  ...
  "data": { ... structured data from previous output ... }
     ...
     */
    async callAIForInputResolution(prompt, mode, _inputSchema) {
        try {
            const messages = [
                {
                    role: 'system',
            ...
            if (mode === 'message') {
                ...
                if (content.includes('{{')) {
                    throw new Error(`[AIInputResolver] LLM returned a template reference instead of actual content. Retrying. Preview: ${content.substring(0, 120)}`);
                }
                return content;
            }
            else {
    ...
    assertRequestedFieldsPresent(value, requestedFields, mode) {
        if (mode !== 'json' || requestedFields.length === 0)
            return;
    ...
    isRuntimeEmptyValue(value) {
        ...
        if (typeof value === 'string') {
            ...
            if (lower === 'v' || lower === 'n/a' || lower === 'not configured')
                return true;
            if (trimmed.includes('{{'))
                return true;
     ...
     */
    validateAndNormalize(aiResponse, inputSchema, mode) {
     ...
     */
    normalizeToSchema(response, schema) {
        ...
        for (const [fieldName, fieldDef] of Object.entries(schema)) {
            ...
            const matchingKey = Object.keys(response).find(key => key.toLowerCase() === fieldName.toLowerCase() ||
        ...
        for (const [fieldName, fieldDef] of Object.entries(schema)) {
            ...
            const wantsNarrative = role === 'long_body' ||
                role === 'content' ||
                role === 'short_summary' ||
                fl.includes('message') ||
                fl.includes('body') ||
     ...
     */
    enforceMinimumAcceptance(candidate, schema) {
        ...
        for (const [fieldName, fieldDef] of Object.entries(schema)) {
            ...
            if (next !== undefined && next !== null) {
                if (expectedType === 'string' && typeof next !== 'string')
                    out[fieldName] = String(next);
                ...
                if (expectedType === 'boolean' && typeof next !== 'boolean') {
                    if (String(next).toLowerCase() === 'true')
                        out[fieldName] = true;
                    if (String(next).toLowerCase() === 'false')
                        out[fieldName] = false;
                }
            }
            const role = fieldDef.role;
            const isContentField = role === 'content' ||
                role === 'long_body' ||
                role === 'prompt' ||
    ...
    pickFirstNonEmptyString(obj, keys) {
        for (const k of keys) {
            const v = obj[k];
...
exports.AIInputResolver = AIInputResolver;
// Export singleton instance
exports.aiInputResolver = new AIInputResolver();
