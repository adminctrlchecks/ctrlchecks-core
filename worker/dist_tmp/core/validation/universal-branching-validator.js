...
exports.universalBranchingValidator = exports.UniversalBranchingValidator = void 0;
...
class UniversalBranchingValidator {
    constructor() { }
    static getInstance() {
     ...
     */
    nodeAllowsMultipleInputs(nodeType) {
        const normalizedType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(nodeType);
        const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(normalizedType);
        if (!nodeDef) {
            return false;
     ...
     */
    validateNoInvalidBranching(workflow, allEdges = []) {
        const errors = [];
        const warnings = [];
        ...
        for (const [sourceId, edges] of edgesBySource.entries()) {
            if (edges.length > 1) {
                ...
                if (sourceNode) {
                    const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(sourceNode.data.type);
                    const allowsBranching = this.nodeAllowsBranching(nodeType);
                    if (!allowsBranching) {
                        errors.push(`Node ${nodeType} (${sourceId}) has ${edges.length} outgoing edges but does not allow branching. ` +
                            `Only nodes with isBranching=true in registry can have multiple outgoing edges.`);
                    }
                    else {
                        // Valid branching - log for debugging
                        warnings.push(`Node ${nodeType} (${sourceId}) has ${edges.length} outgoing edges (branching allowed)`);
                    }
                }
                else {
                    errors.push(`Source node ${sourceId} not found in workflow nodes but has ${edges.length} outgoing edges`);
                }
            }
        }
        ...
        for (const [targetId, edges] of edgesByTarget.entries()) {
            if (edges.length > 1) {
                ...
                if (targetNode) {
                    const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(targetNode.data.type);
                    const allowsMultipleInputs = this.nodeAllowsMultipleInputs(nodeType);
                    if (!allowsMultipleInputs) {
                        errors.push(`Node ${nodeType} (${targetId}) has ${edges.length} incoming edges but does not allow multiple inputs. ` +
                            `Only nodes with multiple incomingPorts in registry can have multiple incoming edges.`);
                    }
                }
                else {
                    errors.push(`Target node ${targetId} not found in workflow nodes but has ${edges.length} incoming edges`);
                }
            }
        }
        return {
     ...
     */
    canCreateEdge(sourceNode, targetNode, existingEdges, allEdgesBeingCreated = []) {
...
exports.UniversalBranchingValidator = UniversalBranchingValidator;
// Export singleton instance
exports.universalBranchingValidator = UniversalBranchingValidator.getInstance();
