...
exports.validateSummaryV2 = validateSummaryV2;
function nonEmptyString(value) {
    return typeof value === 'string' && value.trim().length > 0;
}
function validateSummaryV2(summary) {
    const errors = [];
    ...
    const s = summary;
    if (!s.graphOverview)
        errors.push('summaryV2.graphOverview is required');
    ...
    if (!Array.isArray(s.branches))
        errors.push('summaryV2.branches must be an array');
    ...
    if (!Array.isArray(s.validationFindings))
        errors.push('summaryV2.validationFindings must be an array');
    if (s.graphOverview) {
        if (!Array.isArray(s.graphOverview.triggerNodeIds))
            errors.push('summaryV2.graphOverview.triggerNodeIds must be an array');
        if (!Array.isArray(s.graphOverview.terminalNodeIds))
            errors.push('summaryV2.graphOverview.terminalNodeIds must be an array');
    ...
    for (const [index, node] of (s.nodes || []).entries()) {
        if (!nonEmptyString(node.nodeId))
            errors.push(`summaryV2.nodes[${index}].nodeId is required`);
        if (!nonEmptyString(node.nodeType))
            errors.push(`summaryV2.nodes[${index}].nodeType is required`);
        if (!nonEmptyString(node.purpose))
            errors.push(`summaryV2.nodes[${index}].purpose is required`);
    }
    for (const [index, path] of (s.pathOutcomes || []).entries()) {
        if (!nonEmptyString(path.pathId))
            errors.push(`summaryV2.pathOutcomes[${index}].pathId is required`);
