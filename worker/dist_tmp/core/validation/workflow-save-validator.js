...
exports.validateStructuralReadiness = validateStructuralReadiness;
exports.validateWorkflowForSave = validateWorkflowForSave;
exports.normalizeWorkflowForSave = normalizeWorkflowForSave;
const workflow_build_manifest_utils_1 = require("../utils/workflow-build-manifest-utils");
const unified_graph_orchestrator_1 = require("../orchestration/unified-graph-orchestrator");
...
const form_field_identity_1 = require("../utils/form-field-identity");
const registry_field_contract_1 = require("./registry-field-contract");
const form_ifelse_binding_1 = require("../orchestration/form-ifelse-binding");
...
function validateStructuralReadiness(nodes, options) {
    const strict = !!options?.strict;
    const errors = [];
    const warnings = [];
    for (const node of nodes) {
        const nodeType = node.data?.type || node.type;
        const config = (nodeType === 'if_else'
            ? (0, if_else_conditions_1.normalizeIfElseConfig)((node.data?.config || {}))
            : (node.data?.config || {}));
 ...
 */
function validateWorkflowForSave(nodes, edges, metadata) {
    const errors = [];
    const warnings = [];
    ...
    const normalizedTypeById = new Map(nodes.map((n) => [n.id, String(n.data?.type || n.type || '')]));
    const branchingNodeIds = new Set(nodes
        ...
        .map((n) => n.id));
    const branchTargets = new Set(edges.filter((e) => branchingNodeIds.has(e.source)).map((e) => e.target));
    const logNodeIds = nodes
        .filter((n) => String(n.data?.type || n.type || '') === 'log_output')
        .map((n) => n.id);
    if (branchTargets.size > 1 && logNodeIds.length === 1) {
        errors.push(`Branching workflow has ${branchTargets.size} branch target(s) but only one log_output terminal. Use one log_output per branch path.`);
    }
    for (const logNodeId of logNodeIds) {
        ...
        if (uniqueSources.size > 1) {
            errors.push(`log_output node "${logNodeId}" has ${uniqueSources.size} incoming sources. log_output must be single-input (no branch fan-in).`);
        }
        ...
        for (const e of incoming) {
            ...
            if ((sourceDef?.tags || []).includes('merge')) {
                warnings.push(`log_output "${logNodeId}" is fed by merge "${e.source}". Prefer one branch-specific log_output per branch before merge when branch-level observability is required.`);
            }
        }
    }
    ...
    for (const node of nodes) {
        ...
        if (def) {
            ...
            const configValidation = unified_node_registry_1.unifiedNodeRegistry.validateConfig(nodeType, config);
            if (!configValidation.valid) {
                errors.push(...configValidation.errors.map((e) => `Node "${node.data?.label || node.id}" (${nodeType}) invalid config: ${e}`));
            }
        }
        if (nodeType === 'if_else') {
            const normalizedIfElse = (0, if_else_conditions_1.normalizeIfElseConfig)(config);
            const conditionErrors = (0, if_else_conditions_1.validateCanonicalIfElseConditions)(normalizedIfElse.conditions);
            if (conditionErrors.length > 0) {
                errors.push(...conditionErrors.map((e) => `Node "${node.data?.label || node.id}" (if_else) invalid canonical conditions: ${e}`));
            }
        }
        if (nodeType === 'switch') {
            const caseNames = (0, branching_node_ports_1.extractSwitchCasePortNames)(config);
            ...
            if (nCases >= 2) {
                ...
                if (branchOut.length > 0 && branchOut.length !== nCases) {
                    warnings.push(`Switch node "${node.data?.label || node.id}" defines ${nCases} case(s) in config but has ${branchOut.length} non-main outgoing edge(s). Align the graph with the switch cases.`);
                }
            }
        }
        if (nodeType === 'form') {
    ...
    const ifElseFormBinding = (0, form_ifelse_binding_1.validateIfElseConditionsAgainstUpstreamForm)({
        nodes,
        edges,
        metadata: {},
    });
    errors.push(...ifElseFormBinding.errors);
    const intelligenceIssues = (0, node_field_intelligence_1.validateWorkflowNodeIntelligence)({ nodes: nodes, edges: edges });
    ...
    const isolatedNodes = nodes.filter(n => !hasIncomingEdges.has(n.id) && !hasOutgoingEdges.has(n.id) &&
    ...
    if (metadata?.buildManifest && metadata.freezeBoundary?.frozen) {
        ...
        const match = (0, workflow_build_manifest_utils_1.workflowAuthorizedMultisetMatches)(wf, metadata.buildManifest);
        if (!match.ok) {
 ...
 */
function normalizeWorkflowForSave(nodes, edges, options) {
    ...
    const alreadyAppliedSet = new Set(options?.alreadyApplied ?? []);
    ...
    if (!configOnly && triggerNodes.length > 1) {
        ...
        normalizedNodes = normalizedNodes.filter(n => !isTriggerNode(n) || n.id === firstTriggerId);
        // Remove edges connected to removed triggers
        edges = edges.filter(e => !removedTriggerIds.includes(e.source) && !removedTriggerIds.includes(e.target));
    ...
    normalizedNodes = normalizedNodes.map(node => {
        ...
        if (nodeType === 'if_else' && !alreadyAppliedSet.has(ifelseMigKey)) {
            ...
            const normalized = (0, if_else_conditions_1.normalizeIfElseConfig)(config);
    ...
    const normalizedWorkflow = (0, form_field_identity_1.normalizeWorkflowFormFieldIdentities)({
        nodes: normalizedNodes,
        edges: edges,
    });
    normalizedNodes = (normalizedWorkflow.nodes || normalizedNodes);
    ...
    try {
        const nodeTypeById = new Map(normalizedNodes.map((n) => [n.id, String(n.data?.type || n.type || '')]));
        ...
        const patched = edges.map((e) => ({ ...e }));
        const findInPatched = (id, fallback) => {
            if (!id)
                return fallback;
        ...
        for (const [sourceId, outEdges] of outgoingBySource.entries()) {
            ...
            if (sourceType === 'switch') {
                ...
                const switchCasePorts = (0, branching_node_ports_1.extractSwitchCasePortNames)((sourceNode?.data?.config || {}));
                const normalizedOut = outEdges.map((e) => findInPatched(e.id, e));
                for (const e of normalizedOut) {
                    ...
                    const current = String((isGenericHandle ? '' : rawHandle) || e.type || rawHandle).trim();
                    if (!current)
                        continue;
    ...
    const outgoingEdgeCount = new Map();
    ...
    const switchNeedsReconcile = hasSwitchWithCases && finalNodes.some(n => {
        ...
        if (!Array.isArray(cases) || cases.length === 0)
            return false;
        const caseValues = cases.map((c) => typeof c === 'string' ? c.trim() : String(c?.value ?? '').trim()).filter(Boolean);
    ...
    if (!configOnly && switchNeedsReconcile && !alreadyAppliedSet.has(switchReconcileMigKey)) {
        try {
            const wf = { nodes: finalNodes, edges: finalEdges };
            const rec = unified_graph_orchestrator_1.unifiedGraphOrchestrator.reconcileWorkflow(wf);
            finalNodes = rec.workflow.nodes;
            finalEdges = rec.workflow.edges;
