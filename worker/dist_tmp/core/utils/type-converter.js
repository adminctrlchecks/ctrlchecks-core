 ...
 */
function convertToType(value, targetType, fieldName) {
...
function isCompatibleType(sourceType, targetType) {
    ...
    if (targetType === 'datetime' && sourceType === 'string')
        return true;
    // JSON is object
    if (targetType === 'json' && sourceType === 'object')
        return true;
    if (targetType === 'object' && sourceType === 'json')
        return true;
    return false;
}
/**
 ...
 */
function convertNullToType(targetType, fieldName) {
    const defaults = {
        string: '',
        number: 0,
        boolean: false,
...
function areTypesCompatible(sourceType, targetType) {
    ...
    if (targetType === 'boolean' && (sourceType === 'string' || sourceType === 'number'))
        return true;
