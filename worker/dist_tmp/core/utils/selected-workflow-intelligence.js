...
exports.evaluateSelectedFieldRelevance = evaluateSelectedFieldRelevance;
exports.analyzeSelectedWorkflowIntelligence = analyzeSelectedWorkflowIntelligence;
exports.getSelectedFieldRelevance = getSelectedFieldRelevance;
const unified_node_registry_1 = require("../registry/unified-node-registry");
const is_empty_value_1 = require("./is-empty-value");
...
function withSchemaDefaults(config, def) {
...
function getSelectedOperation(config, def) {
    return human(config.operation ||
        config.action ||
        config.method ||
...
function conditionApplies(condition, config) {
    ...
    if ('equals' in c)
        return valuesEqual(actual, c.equals);
    if ('notEquals' in c)
        return !valuesEqual(actual, c.notEquals);
    return true;
}
function firstSafeDefault(field) {
    return field.fieldIntelligence?.safeDefaults?.[0]?.value;
}
function missingFactsForField(field) {
    const missing = [];
    if (!field.fieldIntelligence?.purpose && !field.description)
        missing.push('purpose');
    if (!field.fieldIntelligence?.runtimeBehavior)
        missing.push('runtimeBehavior');
    if (!field.fieldIntelligence?.importance)
...
function riskForField(field) {
    if (field.fieldIntelligence?.importance?.dangerousIfEmpty)
        return 'high';
    if (field.required || field.ui?.requiredIf)
        return 'high';
    if (field.fieldIntelligence?.importance?.dangerousIfWrong)
        return 'medium';
    return 'none';
}
function classifyFieldRole(fieldName, field) {
    const text = normalize(`${fieldName} ${field.role || ''} ${field.helpCategory || ''} ${field.description || ''}`);
    if (field.ownership === 'credential' || /(credential|api_key|apikey|token|secret|password|oauth)/.test(text))
        return 'credential';
    if (/^(operation|action|method)$/.test(normalize(fieldName)) || /(operation_selector|method|action)/.test(text))
        return 'operation_selector';
    if (/(max|limit|count|timeout|tokens?|length|results?)/.test(text))
        return 'bound';
    if (/(recipient|email_address|\bto\b|cc|bcc)/.test(text))
        return 'recipient';
    if (/(subject|title)/.test(text))
        return 'title';
    if (/(body|message|content|text|prompt|summary)/.test(text))
        return 'content';
    if (/(query|search|filter_text)/.test(text))
        return 'query';
    if (/(condition|case|rule|expression)/.test(text))
        return 'condition';
    if (/(range|cell|row|column|record)/.test(text))
        return 'range';
    if (/(url|endpoint|uri|webhook)/.test(text))
        return 'endpoint';
    if (/(^|_)id$|identifier|resource|sheet|tab|table|document|file|folder|channel|calendar|repository/.test(text))
        return 'resource_identifier';
    if (/(values?|data|payload|fields?|mapping|json)/.test(text))
        return 'payload';
    if (/(format|mode|type|source|selector)/.test(text))
        return 'selector';
...
function connectedNodes(node, workflow, direction) {
    const ids = new Set((workflow.edges || [])
...
function summarizeConnectedDependency(nodes, direction) {
    if (!nodes.length)
        return undefined;
    const labels = nodes
...
function compactDownstream(node, workflow) {
    const text = connectedNodes(node, workflow, 'downstream')
        .map((n) => {
        const def = unified_node_registry_1.unifiedNodeRegistry.get(getNodeType(n));
...
function emptyBehaviorFor(field, relevance) {
    const runtime = field.fieldIntelligence?.runtimeBehavior;
    if (runtime?.whenEmpty && !/change behavior|produce incomplete output|optional setting|default behavior/i.test(runtime.whenEmpty))
        return runtime.whenEmpty;
    if (runtime?.whenMissing && !/backend\/default behavior|optional behavior|default behavior/i.test(runtime.whenMissing))
        return runtime.whenMissing;
    if (relevance === 'not_applicable')
        return 'This value is ignored for the selected operation or dependency state.';
    if (relevance === 'required')
        return 'The workflow cannot safely run this step without this exact value.';
    if (riskForField(field) === 'high')
        return 'If empty, this step may use the wrong data, miss the needed data, or stop before it finishes.';
    return 'If empty, this optional setting is skipped unless the selected operation needs it.';
}
function fieldRoleEmptyBehavior(fieldRole, nodeLabel, fieldName, field, operation) {
...
function wrongValueRiskFor(fieldRole, field, nodeLabel) {
    if (field.fieldIntelligence?.importance?.dangerousIfWrong) {
...
function userActionFor(args) {
    ...
    if (args.relevance === 'not_applicable')
        return `Leave this off for this workflow. ${args.reason}`;
    if (args.relevance === 'required')
        return `Turn this on and provide the ${args.fieldRole.replace(/_/g, ' ')} value before running.${valueHint}`;
    if (args.relevance === 'recommended')
        return `Review this ${args.fieldRole.replace(/_/g, ' ')} for this workflow. ${args.reason}${valueHint}`;
    if (args.relevance === 'advanced')
        return `Leave this in advanced settings unless you intentionally need to tune ${args.fieldName}.`;
...
function enrichResult(args) {
    ...
    return {
        ...
        emptyBehavior: args.result.relevance === 'not_applicable' || args.result.relevance === 'required' || riskForField(args.field) === 'high'
...
function operationAliases(operation) {
    ...
    if (['get_many', 'getmany'].includes(op))
        aliases.add('list');
    if (['send_email', 'sendemail'].includes(op))
        aliases.add('send');
...
function extractOperationMentions(description) {
...
function descriptionScopesFieldToOperations(fieldName, field, operation) {
    if (!operation)
        return null;
    ...
    const explicitlyNegativeForCurrent = currentAliases.some((alias) => text.includes(`not_for_${alias}`) || text.includes(`not_${alias}`));
    const operationScoped = text.includes('for_') ||
        text.includes('_for_') ||
        text.includes('_to_') ||
...
function readOnlyOperationMakesDeliveryFieldIrrelevant(fieldName, field, operation) {
    const op = normalize(operation);
    ...
    const deliveryField = /(recipient|recipients|subject|message|body|sender|from|to|email_address)/i.test(fieldName) ||
...
function operationContractRelevance(def, fieldName, field, operation) {
    if (!def || !operation || !def.operationContracts?.length)
        return null;
    ...
    if (!contract.optionalFields.includes(fieldName) &&
        fieldName !== 'operation' &&
...
function evaluateSelectedFieldRelevance(args) {
    const { workflow, node, fieldName, field } = args;
    const def = unified_node_registry_1.unifiedNodeRegistry.get(getNodeType(node));
    const config = withSchemaDefaults(getNodeConfig(node), def);
    const operation = getSelectedOperation(config, def);
    const done = (result) => enrichResult({ workflow, node, fieldName, field, operation, result });
    const fieldPolicy = def ? (0, field_policy_resolver_1.resolveFieldPolicyForNode)(def, config) : null;
    ...
    const mode = def?.inputSchema
        ? (0, fill_mode_resolver_1.resolveEffectiveFieldFillMode)(fieldName, def.inputSchema, config)
        : field.fillMode?.default;
    const hasCompactDownstream = compactDownstream(node, workflow);
    const boundedOutputForCompactDestination = hasCompactDownstream &&
        /max(length|tokens?)|maximum(length|tokens?)|limit/i.test(fieldName);
    const resourceIdentifier = /(id|name)$/i.test(fieldName) &&
    ...
    if (describedOperation)
        return done(describedOperation);
    return done({
        relevance: 'optional',
        shouldAskUser: false,
        shouldShowInOwnership: true,
        reason: (0, is_empty_value_1.isEmptyValue)(config[fieldName])
...
function analyzeSelectedWorkflowIntelligence(workflow) {
    const nodes = [];
    const fields = [];
...
function getSelectedFieldRelevance(workflow, nodeId, fieldName) {
