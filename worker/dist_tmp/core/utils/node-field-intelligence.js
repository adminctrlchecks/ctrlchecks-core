...
exports.buildFieldIntelligence = buildFieldIntelligence;
exports.summarizeFieldIntelligenceForPrompt = summarizeFieldIntelligenceForPrompt;
exports.validateWorkflowNodeIntelligence = validateWorkflowNodeIntelligence;
exports.buildFieldGuidanceDescription = buildFieldGuidanceDescription;
exports.mergeGuidanceWithDeterministic = mergeGuidanceWithDeterministic;
...
function emptyValueMeaning(value) {
    if (value === undefined)
        return 'unset';
    if (value === null)
        return 'null';
...
function fieldLooksLike(fieldName, patterns) {
    return patterns.some((p) => p.test(fieldName));
}
function boundedFieldSafeDefaults(fieldName) {
...
function mergeIntelligence(base, override) {
    if (!override)
        return base;
    return {
        ...base,
        ...
        importance: { ...(base.importance || {}), ...(override.importance || {}) },
...
function highRiskOverride(args) {
...
function buildFieldIntelligence(args) {
    ...
    const textLike = ['title_like', 'long_body', 'short_summary', 'prompt', 'content', 'query'].includes(role) ||
    ...
    const idUrlOrCredential = field.ownership === 'credential' ||
        role === 'id' ||
...
function summarizeFieldIntelligenceForPrompt(inputSchema) {
    const out = {};
...
function firstSafeDefault(intel) {
...
function composeNeeded(args) {
    ...
    if (relevance?.relevance === 'not_applicable')
        return `Leave this off for this workflow. ${relevance.reason}`;
    if (relevance?.relevance === 'required')
        return `Turn this on and provide a value before running.${args.safeText}`;
    ...
    if (args.importance?.base === 'required')
        return `Turn this on and provide a value before running.${args.safeText}`;
    if (args.importance?.base === 'conditionally_required')
        return `Use this when the selected operation needs it. ${args.emptyBehavior}${args.safeText}`;
...
function composeDataImpact(args) {
    const relevance = args.relevance;
    if (relevance?.relevance === 'not_applicable')
        return 'This field is not used by the current operation or dependency state.';
    const role = relevance?.fieldRole;
    if (role === 'operation_selector')
        return `Changing this changes which action ${args.nodeLabel} performs before connected steps run.`;
    if (role === 'resource_identifier')
        return `Changing this points ${args.nodeLabel} at a different resource, so later steps receive or update different data.`;
    if (role === 'range')
        return `Changing this changes exactly which records, rows, columns, or cells this step reads or writes.`;
    if (role === 'bound')
        return `Changing this changes how much data or text flows from ${args.nodeLabel} into the next step.`;
    if (role === 'recipient')
        return 'Changing this changes who receives the workflow result.';
    if (role === 'title')
        return 'Changing this changes how the delivered result is labeled for the recipient or downstream system.';
    if (role === 'content')
        return `Changing this changes the main content ${args.nodeLabel} passes forward or sends out.`;
    if (role === 'query')
        return `Changing this changes which matching records ${args.nodeLabel} finds before later steps use them.`;
    if (role === 'condition')
        return 'Changing this changes how the workflow filters, branches, or routes the incoming data.';
    if (role === 'endpoint')
        return `Changing this changes the external route or service ${args.nodeLabel} calls.`;
    if (role === 'payload')
        return `Changing this changes the data ${args.nodeLabel} writes, transforms, or sends.`;
    if (role === 'selector')
        return `Changing this changes the mode, format, or source ${args.nodeLabel} uses.`;
    if (args.useCaseGuidance)
        return args.useCaseGuidance;
    if (args.importance?.dangerousIfWrong)
        return `Changing this value can change what ${args.nodeLabel} reads, writes, routes, or produces.`;
...
function stripConditionLead(value) {
...
function composeNeedSummary(args) {
    if (args.relevance?.relevance === 'not_applicable')
        return 'It is not needed for the selected operation.';
    ...
    if (args.importance?.base === 'conditionally_required')
        return 'It is needed only when this selected operation depends on it.';
...
function isUsefulSetupSummary(value, deterministic) {
    const summary = normalizeGuidanceText(value);
    if (summary.length < 40)
        return false;
    if (/configured default behavior|vague default behavior/i.test(summary))
        return false;
    if (/if\s+this\s+is\s+off[\s\S]*if\s+this\s+is\s+empty/i.test(summary))
        return false;
    if (/required|must be set|provide a value/i.test(deterministic.needed) &&
...
function composeYouGuidance(args) {
    ...
    if (args.relevance?.relevance === 'not_applicable')
        return 'Leave this disabled unless you change the selected operation or dependency.';
    if (role === 'credential')
        return 'Connect or select the correct credential from the Credentials step.';
    ...
    if (role === 'resource_identifier')
        return `Paste the exact identifier or name from the source system. ${args.example}`;
    if (role === 'endpoint')
        return `Paste the exact URL or route from the system that should receive the request. ${args.example}`;
    if (role === 'recipient')
        return `Enter the exact destination recipient value. ${args.example}`;
    if (role === 'condition')
        return `Enter the exact rule that decides which data should continue. ${args.example}`;
    if (role === 'bound')
        return `Enter a clear numeric limit that matches the destination and expected output size. ${args.example}`;
    if (args.importance?.dangerousIfWrong)
        return `Provide the exact value from the source system or workflow requirement. Use You when it must stay stable.`;
    return `Enter a fixed value yourself when ${args.label} should stay the same on every run.`;
}
function requiredIfApplies(field, config) {
    const requiredIf = field.ui?.requiredIf;
    if (!requiredIf)
        return true;
    const current = config[String(requiredIf.field)];
    if ('equals' in requiredIf)
        return current === requiredIf.equals;
    return true;
}
function fieldHasExplicitDefault(field) {
...
function displayActionableValue(value) {
...
function normalizeActionableOptions(field) {
    const options = field.options || field.ui?.options || field.ui?.options;
...
function isModelField(fieldName, label) {
    return /\bmodel\b/i.test(`${fieldName} ${label || ''}`);
}
function isCredentialLikeField(field, fieldName) {
    const role = normalizeSemanticRole(field.role || field.helpCategory || field.ownership);
    const searchable = `${fieldName || ''} ${field.label || ''} ${field.description || ''} ${role}`.toLowerCase();
    return (field.ownership === 'credential' ||
...
function selectActionableValue(field, rawValue) {
...
function firstJsonLikeSubstring(value) {
...
function normalizeExampleCandidate(rawValue, field) {
    ...
    if (['json', 'object', 'array'].includes(fieldType)) {
        ...
        try {
            ...
            if (fieldType === 'object' && (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)))
                return undefined;
    ...
    if (fieldType === 'boolean') {
        if (/^(true|yes|on|enabled)$/i.test(cleaned))
            return true;
        if (/^(false|no|off|disabled)$/i.test(cleaned))
            return false;
        return undefined;
    }
    if (/^sample[-_\s]/i.test(cleaned) ||
...
function actionableCandidateValues(field, safe) {
    ...
    if (Array.isArray(anyField.examples)) {
        for (const example of anyField.examples)
            push(example);
    }
...
function selectUniversalActionableValue(field, safe) {
    const options = normalizeActionableOptions(field);
    ...
    for (const candidate of candidates) {
        ...
        if (normalized === undefined || normalized === null)
            continue;
        if (String(displayActionableValue(normalized)).trim() === '')
            continue;
        return normalized;
    }
    return undefined;
...
function deterministicActionableValue(args) {
    ...
    return {
        ...
        applyMode: 'buildtime_ai_once',
        reason: args.recommendedOwner === 'AI Runtime'
...
function isGenericEmptyBehavior(text) {
...
function normalizeSemanticRole(value) {
    return String(value || '').trim().toLowerCase().replace(/[\s-]+/g, '_');
}
function inferPlainFieldRole(field, relevance) {
    const semanticRole = normalizeSemanticRole(relevance?.fieldRole || field.role || field.helpCategory || field.ownership);
    if (field.ownership === 'credential' || ['credential', 'api_key', 'oauth', 'token', 'secret'].includes(semanticRole))
        return 'credential';
    if (['operation_selector', 'operation', 'action', 'method'].includes(semanticRole))
        return 'operation';
    if (['resource_identifier', 'id', 'resource', 'field_name'].includes(semanticRole))
        return 'resource';
    if (['range'].includes(semanticRole))
        return 'range';
    if (['selector', 'type_selector', 'format', 'mode', 'source'].includes(semanticRole))
        return 'format';
    if (['recipient'].includes(semanticRole))
        return 'recipient';
    if (['title', 'title_like'].includes(semanticRole))
        return 'title';
    if (['content', 'long_body', 'short_summary', 'prompt'].includes(semanticRole))
        return 'content';
    if (['bound', 'limit', 'count'].includes(semanticRole))
        return 'bound';
    if (['query'].includes(semanticRole))
        return 'query';
    if (['condition'].includes(semanticRole))
        return 'condition';
    if (['endpoint', 'base_url', 'webhook_url', 'url'].includes(semanticRole))
        return 'endpoint';
    if (['payload', 'raw_json', 'value'].includes(semanticRole))
        return 'payload';
...
function validationConfidenceFor(args) {
    if (args.relevance?.guidanceQualitySignals?.usesStructuredMetadata || args.field.fieldIntelligence?.runtimeBehavior)
        return 'high';
    if (args.importance || args.field.description)
        return 'medium';
    return 'low';
}
function warningsFor(args) {
    const warnings = [];
    if (args.importance?.dangerousIfEmpty || args.relevance?.riskIfEmpty === 'high')
        warnings.push(args.emptyBehavior);
    if (args.field.supportsBuildtimeAI !== false)
        warnings.push('Review AI Build values before running; AI suggestions can be wrong for account-specific or business-specific settings.');
    if (args.field.supportsRuntimeAI === false)
        warnings.push('AI Runtime is not available for this field.');
    return Array.from(new Set(warnings.filter(Boolean)));
}
function validateWorkflowNodeIntelligence(workflow) {
    const issues = [];
    const { unifiedNodeRegistry } = require('../registry/unified-node-registry');
    const { evaluateSelectedFieldRelevance } = require('./selected-workflow-intelligence');
    for (const node of workflow.nodes || []) {
        ...
        for (const [fieldName, field] of Object.entries(def.inputSchema)) {
            const mode = (0, fill_mode_resolver_1.resolveEffectiveFieldFillMode)(fieldName, def.inputSchema, config);
            ...
            if (relevance.relevance === 'not_applicable')
                continue;
            const conditionalApplies = requiredIfApplies(field, config);
            if (mode !== 'runtime_ai' &&
            ...
            for (const hint of intel.validationHints || []) {
                if (!conditionalApplies && (hint.when === 'empty' || hint.when === 'missing'))
                    continue;
                if (!matchHint(hint.when, value))
                    continue;
                if (mode === 'runtime_ai' && (hint.when === 'empty' || hint.when === 'missing'))
                    continue;
                issues.push({
                    nodeId: String(node.id || ''),
                    nodeType,
...
function buildFieldGuidanceDescription(args) {
    const field = args.field;
    const intel = field.fieldIntelligence;
    const label = String(field.label || args.fieldName);
    ...
    const aiBuild = field.supportsBuildtimeAI === false
    ...
    const aiRun = field.supportsRuntimeAI === false
        ? 'Not available for this field.'
        : relevance?.relevance === 'not_applicable'
            ? 'Not needed for this selected operation.'
...
function mergeGuidanceWithDeterministic(deterministic, ai) {
    if (!ai || !isPlainObject(ai))
        return deterministic;
    const deterministicReason = String(deterministic.actionableExample?.reason || '');
    const deterministicSecurityBlocked = deterministic.actionableExample?.canApply === false &&
    ...
    const deterministicHasValue = deterministic.actionableExample?.value !== undefined &&
        deterministic.actionableExample?.value !== null &&
        String(displayActionableValue(deterministic.actionableExample.value)).trim() !== '';
    const aiActionable = ai.actionableExample && isPlainObject(ai.actionableExample)
        ? ai.actionableExample
        : undefined;
    const aiCanApply = aiActionable ? aiActionable.canApply !== false : false;
    const aiHasValue = aiActionable &&
        Object.prototype.hasOwnProperty.call(aiActionable, 'value') &&
        aiActionable.value !== undefined &&
        aiActionable.value !== null &&
        String(displayActionableValue(aiActionable.value)).trim() !== '';
    const useDeterministicActionable = !!aiActionable &&
    ...
    const mergedActionable = aiActionable
        ? useDeterministicActionable
            ...
            : {
                value: Object.prototype.hasOwnProperty.call(aiActionable, 'value')
                    ? aiActionable.value
                    : deterministic.actionableExample?.value,
                displayValue: String(aiActionable.displayValue ||
                    displayActionableValue(aiActionable.value) ||
                    deterministic.actionableExample?.displayValue ||
                    ''),
                canApply: deterministicSecurityBlocked
                    ? false
                    : aiActionable.canApply !== false,
                applyMode: 'buildtime_ai_once',
                reason: String(aiActionable.reason || deterministic.actionableExample?.reason || ''),
