...
exports.filterPlaceholderValues = filterPlaceholderValues;
exports.cleanOutputFromConfig = cleanOutputFromConfig;
/**
 ...
 */
function looksLikeInstructionalPlaceholder(trimmed) {
    // "Enter your …", "Paste your …", etc. (must start with verb + your/the)
    if (/^(enter|type|add|paste|insert|provide|select|fill|choose)\s+(your|the|a)\b/i.test(trimmed)) {
        return true;
    }
 ...
 */
function filterPlaceholderValues(config) {
    const filtered = {};
 ...
 */
function cleanOutputFromConfig(output, config) {
