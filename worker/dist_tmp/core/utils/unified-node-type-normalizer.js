...
exports.unifiedNormalizeNodeType = unifiedNormalizeNodeType;
exports.unifiedNormalizeNodeTypeString = unifiedNormalizeNodeTypeString;
exports.unifiedNormalizeNodeTypeWithInfo = unifiedNormalizeNodeTypeWithInfo;
const pipeline_observability_1 = require("../../services/ai/pipeline-observability");
const startupUnknownTypeCounts = new Map();
...
let startupUnknownFlushTimer = null;
function scheduleStartupUnknownFlush() {
    if (process.env.NODE_ENV === 'test') {
        const snapshot = [...startupUnknownTypeCounts.entries()]
            ...
            .join(', ');
        if (snapshot) {
            console.warn(`[UnifiedNodeTypeNormalizer] Aggregated startup unknown aliases: ${snapshot}`);
        }
        startupUnknownTypeCounts.clear();
        return;
    }
    if (startupUnknownFlushTimer)
        return;
...
function normalizeViaServiceSafely(nodeType) {
    try {
 ...
 */
function unifiedNormalizeNodeType(input) {
 ...
 * @returns Normalized node type string
 */
function unifiedNormalizeNodeTypeString(nodeType, context) {
    ...
    if (serviceResult.method === 'unrecognized_type') {
        ...
        if (phase === 'startup' || phase === 'equivalence') {
            ...
            (0, pipeline_observability_1.incrementPipelineCounter)('normalizer_startup_unknown_aggregated');
            scheduleStartupUnknownFlush();
        }
        else {
            (0, pipeline_observability_1.incrementPipelineCounter)('normalizer_runtime_unknown_total');
            if (!suppressUnknownWarning) {
                console.warn(`[UnifiedNodeTypeNormalizer] ⚠️  Runtime unknown node type: "${nodeType}" (method: ${serviceResult.method})`);
            }
        }
        return nodeType;
    }
    // Registry unavailable is a hard infra/init issue and should stay visible.
    console.error(`[UnifiedNodeTypeNormalizer] ❌ Could not normalize node type: "${nodeType}" (method: ${serviceResult.method})`);
    return nodeType;
}
/**
 ...
 */
function unifiedNormalizeNodeTypeWithInfo(nodeType) {
    if (!nodeType || typeof nodeType !== 'string') {
