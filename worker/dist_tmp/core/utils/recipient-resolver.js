...
exports.parseRecipientEmails = parseRecipientEmails;
exports.extractEmailsFromText = extractEmailsFromText;
exports.intentImpliesMultipleRecipients = intentImpliesMultipleRecipients;
...
function extractEmailsFromText(text) {
    ...
    return dedupe(matches.map(m => normalizeEmail(m)).filter(Boolean));
}
function intentImpliesMultipleRecipients(intent) {
...
function detectEmailsInArrayOfObjects(rows, maxRows) {
    const emails = [];
    const fieldNames = [];
    ...
    for (const row of limited) {
        ...
        for (const [k, v] of Object.entries(row)) {
            ...
            const keyLooksLikeEmail = keyLower === 'email' ||
...
function detectEmailsInAllCellsOfRows(rows, maxRows) {
    const emails = [];
    const fieldNames = [];
    const limited = rows.slice(0, Math.max(0, maxRows));
    for (const row of limited) {
...
function extractCandidateDatasets(upstream) {
    if (!upstream)
        return [];
