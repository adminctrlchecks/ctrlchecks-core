...
exports.normalizeIfElseConditions = normalizeIfElseConditions;
exports.normalizeIfElseConfig = normalizeIfElseConfig;
exports.validateCanonicalIfElseConditions = validateCanonicalIfElseConditions;
...
function sanitizeFieldPath(raw) {
...
function tryParseJson(raw) {
    ...
    if (!trimmed)
        return undefined;
    const likelyJson = trimmed.startsWith('{') ||
        trimmed.startsWith('[') ||
        (trimmed.startsWith('"') && trimmed.endsWith('"'));
    if (!likelyJson)
        return undefined;
...
function normalizeSingleCondition(input) {
...
function normalizeIfElseConditions(input) {
...
function normalizeIfElseConfig(config) {
    ...
    const combineUpper = typeof combineRaw === 'string' ? combineRaw.toUpperCase() : 'AND';
    normalizedConfig.combineOperation = combineUpper === 'OR' ? 'OR' : 'AND';
    return normalizedConfig;
}
function validateCanonicalIfElseConditions(conditions) {
    const errors = [];
