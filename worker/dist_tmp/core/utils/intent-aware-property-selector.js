...
exports.intentAwarePropertySelect = intentAwarePropertySelect;
const DEFAULT_OPTIONS = {
    maxRecordsToScan: 10,
    minMatchScore: 0.84,
};
function intentAwarePropertySelect(userPrompt, upstreamJson, options = {}) {
    ...
    if (intentTerms.length === 0 || availableProps.length === 0) {
        return {
            ...
            explanation: intentTerms.length === 0
...
function pickDatasetRoot(upstreamJson) {
    if (!upstreamJson || typeof upstreamJson !== 'object')
        return upstreamJson;
    const obj = upstreamJson;
...
function extractRecordKeys(records, maxScan) {
    const keys = new Set();
...
function extractCandidatePropertyTerms(prompt) {
    if (!prompt)
        return [];
...
function scoreMatches(intentTerms, properties) {
    const results = [];
...
function filterDatasetByProperties(dataset, properties, maxScan) {
    if (!dataset)
        return dataset;
    if (Array.isArray(dataset)) {
        ...
        if (properties.length === 1) {
            ...
            return dataset.map((r) => (r && typeof r === 'object' ? r[key] : undefined));
        }
