...
exports.LRUNodeOutputsCache = void 0;
class LRUNodeOutputsCache {
    /**
     ...
     */
    constructor(maxSize = 100, cloneOnGet = false) {
        this.hits = 0;
        this.misses = 0;
        this.evictions = 0;
        if (maxSize < 1) {
     ...
     */
    set(nodeId, output, persistent = false) {
    ...
    getAll() {
        ...
        this.cache.forEach((entry, key) => {
            if (key.startsWith('__'))
                return; // skip internal audit/observability entries
     ...
     */
    getMostRecentOutput(excludeKeys = []) {
     ...
     */
    getMostRecentOutputEntry(excludeKeys = []) {
        ...
        for (const [key, entry] of this.cache.entries()) {
            if (exclude.has(key))
                continue;
            if (key.startsWith('__'))
                continue; // skip internal audit/observability entries
...
exports.LRUNodeOutputsCache = LRUNodeOutputsCache;
