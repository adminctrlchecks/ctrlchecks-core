...
exports.getProviderCircuitKeyFromNodeType = getProviderCircuitKeyFromNodeType;
function getProviderCircuitKeyFromNodeType(nodeType) {
    const type = (nodeType || '').toLowerCase();
    if (!type)
        return 'provider:unknown';
    if (type.includes('gmail') || type.includes('google_'))
    ...
    if (type.includes('zoho'))
        return 'provider:zoho';
    if (type.includes('pipedrive'))
    ...
    if (type.includes('ai_') || type.includes('ollama') || type.includes('gemini'))
        return 'provider:ai';
