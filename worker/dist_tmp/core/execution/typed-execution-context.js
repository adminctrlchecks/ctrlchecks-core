...
exports.setNodeOutput = setNodeOutput;
exports.getAllNodeOutputs = getAllNodeOutputs;
const object_utils_1 = require("../utils/object-utils");
/**
 * Create a new execution context
 */
function createExecutionContext(initialInput) {
 ...
 */
function extractInputObject(input) {
...
function getContextValue(context, path) {
    ...
    if (path.startsWith('$json.') || path.startsWith('json.')) {
        ...
        if (context.variables.$json) {
            const value = (0, object_utils_1.getNestedValue)(context.variables.$json, jsonPath);
            if (value !== undefined)
                return value;
        }
        if (context.variables.json) {
        ...
        const directValue = (0, object_utils_1.getNestedValue)(context.variables, jsonPath);
        if (directValue !== undefined)
            return directValue;
        // ✅ FIX: Try lastOutput as fallback (for backward compatibility)
        const lastOutputValue = (0, object_utils_1.getNestedValue)(context.lastOutput, jsonPath);
        ...
        if (!jsonPath.includes('.')) {
            ...
            if (context.lastOutput && typeof context.lastOutput === 'object' && !Array.isArray(context.lastOutput)) {
                const lastOutputObj = context.lastOutput;
    ...
    if (dotIdx > 0) {
        ...
        if (nodeOut !== undefined) {
            if (nodeOut && typeof nodeOut === 'object') {
                const nested = (0, object_utils_1.getNestedValue)(nodeOut, nestedPath);
                if (nested !== undefined)
                    return nested;
            }
        ...
        if (directNodeOut !== undefined)
            return directNodeOut;
    }
    // Try nested access
    return (0, object_utils_1.getNestedValue)(context.variables, path)
        ?? (0, object_utils_1.getNestedValue)(context.lastOutput, path);
}
/**
 ...
 */
function getAllNodeOutputs(context) {
    const result = {};
