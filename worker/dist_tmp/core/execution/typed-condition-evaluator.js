...
function evaluateCondition(condition, context) {
    ...
    const leftRaw = typeof condition.leftValue === 'string'
        ? (0, typed_value_resolver_1.resolveTypedValue)(condition.leftValue, context)
        : condition.leftValue;
    const rightRaw = typeof condition.rightValue === 'string'
        ? (0, typed_value_resolver_1.resolveTypedValue)(condition.rightValue, context)
    ...
    if (leftType === 'number' && rightType === 'number') {
        return compareNumbers(leftCoerced, rightCoerced, condition.operation);
    }
    ...
    if (leftType === 'string' && rightType === 'string') {
        ...
        return compareStrings(leftCoerced, rightCoerced, condition.operation);
    }
    // If both are booleans, compare as booleans
    if (leftType === 'boolean' && rightType === 'boolean') {
        return compareBooleans(leftCoerced, rightCoerced, condition.operation);
    }
...
function evaluateExpression(expression, context) {
    ...
    if (hasTemplate && hasOperator) {
        ...
        evalExpr = evalExpr.replace(/\{\{([^}]+)\}\}/g, (match, path) => {
            ...
            const value = (0, typed_execution_context_1.getContextValue)(context, variablePath);
                ...
                if (process.env.DEBUG_DATA_FLOW === 'true') {
                    console.log('[evaluateExpression] ⚠️ Value undefined for path:', variablePath, {
                        ...
                        $jsonKeys: context.variables.$json && typeof context.variables.$json === 'object'
                            ? Object.keys(context.variables.$json)
                            : [],
                    });
                }
                return 'undefined';
...
function evaluateStringExpression(expression, context) {
    ...
    if (comparison) {
        const left = coerceLiteralScalar((0, typed_value_resolver_1.resolveTypedValue)(comparison.left, context));
        const right = coerceLiteralScalar((0, typed_value_resolver_1.resolveTypedValue)(comparison.right, context));
        const leftType = inferType(left);
        const rightType = inferType(right);
        // Numeric comparison
        if (leftType === 'number' && rightType === 'number') {
            return compareNumbers(left, right, comparison.op);
        }
        // String comparison
        if (leftType === 'string' && rightType === 'string') {
            // ✅ TYPE SAFETY (CORE CONTRACT)
            ...
            return compareStrings(left, right, comparison.op);
        }
    ...
    try {
        ...
        evalExpr = evalExpr.replace(/\{\{([^}]+)\}\}/g, (match, path) => {
            const trimmedPath = path.trim();
            const value = (0, typed_execution_context_1.getContextValue)(context, trimmedPath);
            if (value === undefined || value === null) {
                return 'undefined';
            }
 ...
 */
function coerceLiteralScalar(value) {
    if (typeof value !== 'string')
        return value;
 ...
 */
function evaluateModuloExpression(expr, context) {
    ...
    if (match) {
        ...
        const leftRaw = leftStr.includes('{{')
            ? (0, typed_value_resolver_1.resolveTypedValue)(leftStr, context)
            : parseFloat(leftStr);
 ...
 */
function compareNumbers(left, right, operation) {
    switch (operation) {
        case 'equals':
            return left === right;
        case 'not_equals':
 ...
 */
function compareStrings(left, right, operation) {
    switch (operation) {
        case 'equals':
            return left === right;
        case 'not_equals':
 ...
 * Compare booleans
 */
function compareBooleans(left, right, operation) {
    switch (operation) {
        case 'equals':
            return left === right;
        case 'not_equals':
