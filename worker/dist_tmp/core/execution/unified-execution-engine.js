...
exports.createUnifiedExecutionContext = createUnifiedExecutionContext;
exports.updateExecutionContext = updateExecutionContext;
exports.shouldSkipNode = shouldSkipNode;
exports.buildNodeInput = buildNodeInput;
const switch_branch_router_1 = require("./switch-branch-router");
 ...
 */
function isTriggerNodeInline(node) {
 ...
 */
function createUnifiedExecutionContext(initialInput, nodeOutputs) {
    const context = (0, typed_execution_context_1.createExecutionContext)(initialInput);
    const outputs = nodeOutputs || new lru_node_outputs_cache_1.LRUNodeOutputsCache(100, false);
 ...
 */
function updateExecutionContext(execCtx, nodeId, output, mergedInput) {
    // ✅ Store output in isolated storage (nodeOutputs map)
    execCtx.nodeOutputs.set(nodeId, output);
    (0, typed_execution_context_1.setNodeOutput)(execCtx.context, nodeId, output);
 ...
 */
function shouldSkipNode(node, incomingEdges, nodes, edges, ifElseResults, switchResults, skippedNodeIds = new Set(), // Track skipped nodes to prevent infinite loops
switchExpressionValues) {
    ...
    for (const edge of incomingEdges) {
        ...
        if (sourceNode.data?.type === 'switch' && switchResults[edge.source] !== undefined) {
            ...
            if ((0, switch_branch_router_1.shouldSkipForSwitchIncomingEdge)(edge, sourceNode, edges, matchedCase, exprVal)) {
                if (skippedNodeIds) {
                    skippedNodeIds.add(node.id);
                }
                return true;
 ...
 */
function buildNodeInput(node, edges, nodeOutputs, defaultInput) {
