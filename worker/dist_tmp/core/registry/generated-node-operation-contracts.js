...
exports.GENERATED_NODE_OPERATION_VALUES = void 0;
exports.applyGeneratedOperationContracts = applyGeneratedOperationContracts;
exports.GENERATED_NODE_OPERATION_VALUES = {
...
function applyGeneratedOperationContracts(def) {
    const operations = exports.GENERATED_NODE_OPERATION_VALUES[def.type];
    ...
    const { options: _oldOptions, enum: _oldEnum, examples: _oldExamples, ...operationFieldBase } = existingOperationField;
    const existingUiOptions = (existingOperationField.ui?.options || []);
    const existingOptionLabels = new Map(existingUiOptions
        .map((option) => {
        if (typeof option === 'string')
            return [option, labelForOperation(option)];
        if (option && typeof option.value === 'string')
            return [option.value, option.label || labelForOperation(option.value)];
        return null;
    })
    ...
    return {
        ...
        inputSchema: {
            ...
            operation: {
                ...operationFieldBase,
                default: operations.includes(String(def.inputSchema.operation?.default || ''))
