...
exports.semanticEquivalenceAutoGenerator = exports.SemanticEquivalenceAutoGenerator = void 0;
...
class SemanticEquivalenceAutoGenerator {
     ...
     */
    generateComprehensiveEquivalences() {
        const equivalences = [];
     ...
     */
    generateCategoryBasedEquivalences(schemas) {
        const equivalences = [];
        ...
        schemas.forEach(schema => {
            ...
            if (!category || EXCLUDED_CATEGORIES.has(category))
                return; // Skip excluded categories
        ...
        byCategory.forEach((categorySchemas, category) => {
            ...
            if (!hasSimilarCapabilities) {
                console.log(`[SemanticEquivalenceAutoGenerator] ⚠️  Skipping category-based equivalence for "${category}": ` +
     ...
     */
    checkSimilarCapabilities(schemas) {
        if (schemas.length < 2)
            return false;
        ...
        for (const cap of allCapabilities) {
            const hasCap = schemas.every(schema => {
                const capabilities = (schema.capabilities || []).map((c) => c.toLowerCase());
     ...
     */
    generateCapabilityBasedEquivalences(schemas) {
        const equivalences = [];
        ...
        byCapability.forEach((capSchemas, capability) => {
            if (capSchemas.length < 2)
                return;
            // Remove duplicates
            const uniqueSchemas = Array.from(new Map(capSchemas.map(s => [s.type, s])).values());
     ...
     */
    generateAliasBasedEquivalences(schemas) {
        const equivalences = [];
        ...
        schemas.forEach(schema => {
            ...
            aliases.forEach((alias) => {
                const normalizedAlias = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(alias, {
                    phase: 'equivalence',
                    suppressUnknownWarning: true,
                }).toLowerCase();
                const normalizedCanonical = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(canonical, {
                    phase: 'equivalence',
                    suppressUnknownWarning: true,
                }).toLowerCase();
     ...
     */
    generatePatternBasedEquivalences(schemas) {
        const equivalences = [];
        ...
        const googleServices = schemas.filter(s => s.type.startsWith('google_') ||
            s.keywords?.some((k) => k.includes('google')));
        if (googleServices.length > 1) {
            ...
            byBase.forEach((group, base) => {
                if (group.length >= 2) {
                    ...
                    if (!hasSimilarCapabilities) {
                        console.log(`[SemanticEquivalenceAutoGenerator] ⚠️  Skipping pattern-based equivalence for "google_${base}": ` +
        ...
        const apiNodes = schemas.filter(s => s.type.endsWith('_api') ||
            s.type.includes('api'));
     ...
     */
    isServiceSpecificCapability(capability) {
    ...
    selectCanonicalType(schemas) {
        ...
        const scored = schemas.map(schema => {
            ...
            if (!type.endsWith('_api') && !type.endsWith('api'))
                score += 15;
...
exports.SemanticEquivalenceAutoGenerator = SemanticEquivalenceAutoGenerator;
// Export singleton instance
exports.semanticEquivalenceAutoGenerator = new SemanticEquivalenceAutoGenerator();
