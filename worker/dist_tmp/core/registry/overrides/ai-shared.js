...
exports.overrideAiNodeWithIntentAwareSelection = overrideAiNodeWithIntentAwareSelection;
const unified_node_registry_legacy_adapter_1 = require("../unified-node-registry-legacy-adapter");
const intent_aware_property_selector_1 = require("../../utils/intent-aware-property-selector");
/**
 ...
 */
function overrideAiNodeWithIntentAwareSelection(def, schema) {
    return {
        ...def,
            ...
            return await (0, unified_node_registry_legacy_adapter_1.executeViaLegacyExecutor)({
                ...
                hooks: {
                    beforeExecute: (prepared) => {
                        const userPrompt = String(global.currentWorkflowIntent || '');
                        if (!userPrompt)
                            return;
                        const mostRecentUpstream = prepared.nodeOutputs && typeof prepared.nodeOutputs.getMostRecentOutput === 'function'
                            ? prepared.nodeOutputs.getMostRecentOutput(['$json', 'json', 'trigger', 'input'])
                            : null;
                        if (!mostRecentUpstream)
                            return;
                        const selection = (0, intent_aware_property_selector_1.intentAwarePropertySelect)(userPrompt, mostRecentUpstream);
