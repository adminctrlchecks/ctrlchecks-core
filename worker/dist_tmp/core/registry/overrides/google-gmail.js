...
exports.overrideGoogleGmail = overrideGoogleGmail;
...
const google_sheets_read_range_1 = require("../../../shared/google-sheets-read-range");
function ensureRecipientEmailsField(inputSchema) {
    if (inputSchema.recipientEmails)
        return inputSchema;
    return {
        ...inputSchema,
        recipientEmails: {
            type: 'string',
            description: 'Recipient email address(es). Supports comma-separated list or array input. If provided, overrides automatic detection.',
            required: false,
            examples: ['john@example.com', 'john@example.com, jane@example.com', '{{$json.email}}'],
        },
    };
}
async function executeGmailSend(context, schema) {
    ...
    const { filterPlaceholderValues } = await Promise.resolve().then(() => __importStar(require('../../utils/placeholder-filter')));
    ...
    const finalResolvedInputs = (0, runtime_input_handoff_1.getAuthoritativeInputs)(context);
    const configWithResolvedInputs = {
        ...filteredBaseConfig,
        ...filteredConfig,
        ...finalResolvedInputs,
    };
    ...
    if (op !== 'send') {
        ...
        return await (0, unified_node_registry_legacy_adapter_1.executeViaLegacyExecutor)({ context: { ...context, config: configWithResolvedInputs }, schema });
    }
    const userIntent = String(global.currentWorkflowIntent || '');
    ...
    if (recipientUrl && typeof recipientUrl === 'string' && recipientUrl.trim()) {
        try {
            const { parseRecipientEmails, extractEmailsFromText } = await Promise.resolve().then(() => __importStar(require('../../utils/recipient-resolver')));
    ...
    if (!recipientEmails && recipientSource) {
        const { parseRecipientEmails } = await Promise.resolve().then(() => __importStar(require('../../utils/recipient-resolver')));
    ...
    const useAggressive = configWithResolvedInputs.useAiRecipientMapping === true ||
        configWithResolvedInputs.useAiRecipientMapping === 'true';
    let resolution = (0, recipient_resolver_1.resolveRecipients)({
    ...
    if (resolution.recipientList.length === 0 &&
        ...
        if (inlineSpreadsheetId) {
            ...
            const accessToken = userIdsToTry.length > 0 ? await (0, google_sheets_1.getGoogleAccessToken)(context.db, userIdsToTry) : null;
            if (!accessToken) {
                return {
                    success: true,
                    output: {
                        ...finalResolvedInputs,
                        _error: 'Gmail: inline Google Sheets fallback requires a connected Google account (same OAuth as Gmail, with Sheets API access).',
            ...
            const range = typeof rangeRaw === 'string' && rangeRaw.trim() !== '' ? rangeRaw.trim() : undefined;
            const fetched = await (0, google_sheets_read_range_1.fetchGoogleSheetReadRange)({
            ...
            resolution = (0, recipient_resolver_1.resolveRecipients)({
                credentialInputRecipientEmails: recipientEmails,
                recipientSource,
                userIntent,
    ...
    if (resolution.recipientList.length === 0) {
        return {
            ...
            output: {
                ...finalResolvedInputs,
                _error: 'Gmail: missing recipient email(s). For Extract from sheet: ensure a Google Sheets node upstream supplies rows with email-like columns, or set optional Spreadsheet ID + sheet on this node for fallback, or use manual recipients / prompt.',
                _adapterDiagnostics: {
    ...
    const coerceString = (v) => (typeof v === 'string' ? v.trim() : '');
    const inputObjAny = typeof context.rawInput === 'object' && context.rawInput !== null ? context.rawInput : null;
    const inputSubject = coerceString(inputObjAny?.subject) ||
    ...
    const inputBody = coerceString(inputObjAny?.body) ||
    ...
    const { resolveUniversalTemplate } = await Promise.resolve().then(() => __importStar(require('../../utils/universal-template-resolver')));
    // Resolve subject template if it contains template syntax
    let subjectFinal = String((configWithResolvedInputs.subject ?? '')).trim();
    ...
    let bodyFinal = String((configWithResolvedInputs.body ?? '')).trim();
    ...
    if (!subjectFinal && inputSubject)
        subjectFinal = inputSubject;
    if (!bodyFinal && inputBody)
        bodyFinal = inputBody;
    const mostRecentUpstream = nodeOutputs && typeof nodeOutputs.getMostRecentOutput === 'function'
        ? nodeOutputs.getMostRecentOutput()
        : null;
    if (mostRecentUpstream) {
        const upstreamObj = mostRecentUpstream;
        const upstreamSubject = coerceString(upstreamObj?.subject) ||
            (typeof upstreamObj?.response === 'string'
                ? coerceString(upstreamObj.response.split(/\r?\n/)[0] || '')
                : '') ||
        ...
        const upstreamBody = coerceString(upstreamObj?.body) ||
        ...
        if (!subjectFinal && upstreamSubject)
            subjectFinal = upstreamSubject;
        if (!bodyFinal && upstreamBody)
            bodyFinal = upstreamBody;
    }
    ...
    const { resolveGmailCredentials, sendGmailEmail } = await Promise.resolve().then(() => __importStar(require('../../../shared/gmail-executor')));
    const credential = await resolveGmailCredentials(context.db, context.workflowId, context.nodeId, context.userId, context.currentUserId);
    if (!credential) {
        return {
            success: true,
            output: {
...
function overrideGoogleGmail(def, schema) {
