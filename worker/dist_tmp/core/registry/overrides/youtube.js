...
exports.overrideYoutube = overrideYoutube;
...
async function getYouTubeAccessToken(context) {
    ...
    if (deprecatedRawToken)
        return deprecatedRawToken;
...
async function youtubeApiRequest(url, accessToken, init = {}) {
...
async function youtubeRawRequest(url, accessToken, init = {}) {
    const response = await fetch(url, {
        ...init,
        headers: {
            Authorization: `Bearer ${accessToken}`,
...
function flattenChannelResponse(operation, data) {
...
function flattenVideoResponse(operation, data) {
...
async function resolveUploadVideoBytes(inputs) {
    ...
    if (Buffer.isBuffer(rawBinary))
        return { bytes: rawBinary, mimeType: configuredMimeType };
    if (rawBinary instanceof Uint8Array)
        return { bytes: Buffer.from(rawBinary), mimeType: configuredMimeType };
...
async function uploadVideo(accessToken, inputs) {
    const title = stringValue(inputs.title);
    if (!title)
        throw new Error('title is required for upload_video');
    const { bytes, mimeType } = await resolveUploadVideoBytes(inputs);
    if (bytes.length === 0)
        throw new Error('Video upload data is empty');
    ...
    const sessionResponse = await youtubeRawRequest(`https://www.googleapis.com/upload/youtube/v3/videos?${new URLSearchParams({
...
function overrideYoutube(def, _schema) {
    const baseInputSchema = { ...def.inputSchema };
    delete baseInputSchema.accessToken;
    delete baseInputSchema.credentialId;
    delete baseInputSchema.apiKey;
    delete baseInputSchema.videoUrl;
    delete baseInputSchema.commentText;
    delete baseInputSchema.commentId;
    return {
        ...def,
        inputSchema: {
            ...baseInputSchema,
        ...
        execute: async (context) => {
            ...
            try {
                ...
                if (operation === 'list_my_channels') {
                    ...
                    if (!query)
                        throw new Error('query is required for search_videos');
                    const params = new URLSearchParams({
                        part: 'snippet',
                        type: 'video',
                        q: query,
                    ...
                    if (!videoId)
                        throw new Error('videoId is required for get_video_stats');
                    const params = new URLSearchParams({
                        part: 'snippet,contentDetails,statistics',
                        id: videoId,
                    });
                    ...
                    const videoId = stringValue(inputs.videoId);
                    if (!videoId)
                        throw new Error('videoId is required for update_video_metadata');
                    ...
                    const existing = await youtubeApiRequest(`https://www.googleapis.com/youtube/v3/videos?${new URLSearchParams({ part: 'snippet', id: videoId }).toString()}`, accessToken);
                    ...
                    const data = await youtubeApiRequest(`https://www.googleapis.com/youtube/v3/videos?${new URLSearchParams({ part: 'snippet' }).toString()}`, accessToken, {
                        method: 'PUT',
                    ...
                    const videoId = stringValue(inputs.videoId);
                    if (!videoId)
                        throw new Error('videoId is required for delete_video');
                    await youtubeApiRequest(`https://www.googleapis.com/youtube/v3/videos?${new URLSearchParams({ id: videoId }).toString()}`, accessToken, { method: 'DELETE' });
