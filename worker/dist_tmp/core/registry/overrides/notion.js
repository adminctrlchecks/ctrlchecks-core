...
exports.overrideNotion = overrideNotion;
const runtime_input_handoff_1 = require("../../execution/runtime-input-handoff");
const credential_resolver_1 = require("../../../shared/credential-resolver");
...
function toBlock(type, content, language = 'plain text') {
    ...
    if (type === 'code')
        return { type: 'code', code: { rich_text: toRichText(content), language } };
...
function guessPropertyType(value) {
...
function buildNotionFilter(property, type, condition, value) {
    const prop = property;
    switch (type) {
        case 'text':
        case 'rich_text':
...
// ─── Token resolver ───────────────────────────────────────────────────────────
async function getNotionToken(context) {
    ...
    if (context.currentUserId && context.currentUserId !== context.userId)
        ids.push(context.currentUserId);
    if (ids.length > 0) {
        const token = await (0, credential_resolver_1.resolveOAuthTokenString)('notion', ids);
        if (token)
            return token;
    }
...
// ─── Override ─────────────────────────────────────────────────────────────────
function overrideNotion(def, _schema) {
    const runtimeAI = { default: 'manual_static', supportsRuntimeAI: true, supportsBuildtimeAI: true };
    const buildAI = { default: 'buildtime_ai_once', supportsRuntimeAI: false, supportsBuildtimeAI: true };
            ...
            try {
                ...
                if (resource === 'page') {
                    if (operation === 'get') {
                        ...
                        if (!pageId)
                            throw new Error('pageId is required for page › get');
                        ...
                        if (databaseId) {
                            ...
                            const blockType = (str(cfg.blockType, 'paragraph') || 'paragraph');
                        ...
                        const pageId = notionId(cfg.pageId);
                        if (!pageId)
                            throw new Error('pageId is required for page › update');
                        const simpleProps = parseJsonSafe(cfg.properties);
                        const titleText = str(cfg.title);
                        const props = buildPropertiesFromSimpleJson(simpleProps, titleText || undefined);
                        if (Object.keys(props).length === 0) {
                        ...
                        result = await notion.pages.update({ page_id: pageId, properties: props });
                    }
                    else if (operation === 'archive') {
                        const pageId = notionId(cfg.pageId);
                        if (!pageId)
                            throw new Error('pageId is required for page › archive');
                        result = await notion.pages.update({ page_id: pageId, archived: true });
                    }
                    else if (operation === 'restore') {
                        const pageId = notionId(cfg.pageId);
                        if (!pageId)
                            throw new Error('pageId is required for page › restore');
                ...
                // ════════════════════════════════════════════════════════════════════
                    ...
                    if (operation === 'get') {
                        const databaseId = notionId(cfg.databaseId);
                        if (!databaseId)
                            throw new Error('databaseId is required for database › get');
                        result = await notion.databases.retrieve({ database_id: databaseId });
                    }
                    else if (operation === 'list') {
                        const ps = clampedPageSize();
                        const all = returnAll();
                        const fn = (cursor) => notion.search({
                            filter: { property: 'object', value: 'database' },
                            start_cursor: cursor,
                            page_size: ps,
                        });
                        result = all
                            ? { results: await collectAll(fn), object: 'list' }
                            : await fn();
                    }
                    else if (operation === 'query') {
                        const databaseId = notionId(cfg.databaseId);
                        if (!databaseId)
                            throw new Error('databaseId is required for database › query');
                        const ps = clampedPageSize();
                        const all = returnAll();
                        ...
                        const fn = (cursor) => notion.databases.query({
                        ...
                        const parentPageId = notionId(cfg.parentPageId) || notionId(cfg.parentId);
                        if (!parentPageId)
                            throw new Error('parentPageId is required for database › create');
                        ...
                        result = await notion.databases.create({
                            parent: { page_id: parentPageId },
                            title: toRichText(titleText),
                            properties: schemaRaw,
                            is_inline: isInline,
                        });
                    }
                    else if (operation === 'update') {
                        const databaseId = notionId(cfg.databaseId);
                        if (!databaseId)
                            throw new Error('databaseId is required for database › update');
                        const titleText = str(cfg.title) || str(cfg.databaseTitle);
                        const schemaRaw = parseJsonSafe(cfg.schemaJson);
                        const updatePayload = { database_id: databaseId };
                        if (titleText)
                            updatePayload.title = toRichText(titleText);
                        if (schemaRaw)
                            updatePayload.properties = schemaRaw;
                ...
                // ════════════════════════════════════════════════════════════════════
                    ...
                    if (operation === 'get') {
                        ...
                        if (!blockId)
                            throw new Error('blockId is required for block › get');
                        result = await notion.blocks.retrieve({ block_id: blockId });
                    }
                    else if (operation === 'listChildren') {
                        const blockId = notionId(cfg.blockId);
                        if (!blockId)
                            throw new Error('blockId is required for block › listChildren');
                        const ps = clampedPageSize();
                        const all = returnAll();
                        const fn = (cursor) => notion.blocks.children.list({ block_id: blockId, start_cursor: cursor, page_size: ps });
                        result = all
                            ? { results: await collectAll(fn), object: 'list' }
                            : await fn();
                    }
                        ...
                        const blockId = notionId(cfg.blockId);
                        if (!blockId)
                            throw new Error('blockId is required for block › appendChildren');
                        const content = str(cfg.content);
                        const blockType = (str(cfg.blockType, 'paragraph') || 'paragraph');
                        const codeLanguage = str(cfg.codeLanguage, 'plain text');
                        ...
                        result = await notion.blocks.children.append({ block_id: blockId, children: children });
                    }
                    else if (operation === 'update') {
                        const blockId = notionId(cfg.blockId);
                        if (!blockId)
                            throw new Error('blockId is required for block › update');
                        const content = str(cfg.content);
                        const blockType = (str(cfg.blockType, 'paragraph') || 'paragraph');
                        const codeLanguage = str(cfg.codeLanguage, 'plain text');
                        if (!content && blockType !== 'divider') {
                        ...
                        result = await notion.blocks.update({ block_id: blockId, ...blockPayload });
                    }
                    else if (operation === 'delete') {
                        const blockId = notionId(cfg.blockId);
                        if (!blockId)
                            throw new Error('blockId is required for block › delete');
                ...
                // ════════════════════════════════════════════════════════════════════
                    ...
                    if (operation === 'get') {
                        ...
                        if (!userId)
                            throw new Error('userId is required for user › get');
                        result = await notion.users.retrieve({ user_id: userId });
                    }
                    else if (operation === 'list') {
                        const ps = clampedPageSize();
                        const all = returnAll();
                        const fn = (cursor) => notion.users.list({ start_cursor: cursor, page_size: ps });
                        result = all
                            ? { results: await collectAll(fn), object: 'list' }
                            : await fn();
                    }
                ...
                // ════════════════════════════════════════════════════════════════════
                    ...
                    if (operation === 'list') {
                        ...
                        const blockId = notionId(cfg.blockId);
                        if (!pageId && !blockId)
                            throw new Error('Provide pageId or blockId for comment › list');
                        const ps = clampedPageSize();
                        const all = returnAll();
                        const fn = (cursor) => {
                            const params = { start_cursor: cursor, page_size: ps };
                            // Notion comments API always uses block_id — pages are blocks too
                            params.block_id = pageId || blockId;
                            return notion.comments.list(params);
                        };
                        result = all
                            ? { results: await collectAll(fn), object: 'list' }
                            : await fn();
                        ...
                        if (!commentText)
                            throw new Error('comment text is required for comment › create');
                        const commentData = { rich_text: toRichText(commentText) };
                    ...
                    if (operation === 'search' || operation === 'get' || operation === 'list') {
                        ...
                        const fn = (cursor) => {
                            ...
                            if (query)
                                params.query = query;
                            if (filterType)
                                params.filter = { property: 'object', value: filterType };
                            if (sortDir)
                                params.sort = { direction: sortDir, timestamp: 'last_edited_time' };
