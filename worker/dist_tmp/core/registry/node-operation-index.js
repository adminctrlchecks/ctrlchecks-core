...
exports.nodeOperationIndex = exports.NodeOperationIndexService = void 0;
const unified_node_registry_1 = require("./unified-node-registry");
/**
 * ✅ UNIVERSAL: Extract tokens from operation name
 * Handles camelCase, snake_case, kebab-case, etc.
 */
function extractTokensFromOperation(operation) {
    const tokens = [];
 ...
 */
function generateVerbVariations(verb) {
    const variations = [];
 ...
 */
function extractOperationsFromNode(nodeDef) {
    const operations = [];
 ...
 */
class NodeOperationIndexService {
    constructor() {
        this.index = new Map();
    ...
    initialize() {
        ...
        const registry = unified_node_registry_1.unifiedNodeRegistry;
        const allNodeTypes = registry.getAllTypes();
        for (const nodeType of allNodeTypes) {
            try {
                const nodeTypeStr = nodeType; // ✅ Type assertion for registry compatibility
                const nodeDef = registry.get(nodeTypeStr);
                if (!nodeDef)
                    continue;
                const operations = extractOperationsFromNode(nodeDef);
                if (operations.length > 0) {
                    const operationTokens = operations.map(op => ({
                        operation: op,
     ...
     */
    findBestOperation(nodeType, verbTokens) {
        if (!this.initialized) {
            this.initialize();
        }
...
exports.NodeOperationIndexService = NodeOperationIndexService;
// Export singleton instance
exports.nodeOperationIndex = NodeOperationIndexService.getInstance();
