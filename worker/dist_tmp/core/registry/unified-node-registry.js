...
exports.unifiedNodeRegistry = exports.UnifiedNodeRegistry = void 0;
const node_library_1 = require("../../services/nodes/node-library");
const unified_node_registry_overrides_1 = require("./unified-node-registry-overrides");
...
const field_help_metadata_1 = require("../utils/field-help-metadata");
const field_ownership_1 = require("../utils/field-ownership");
const node_field_intelligence_1 = require("../utils/node-field-intelligence");
const credential_requirements_1 = require("../../credentials-system/credential-requirements");
class UnifiedNodeRegistry {
    ...
    validateIntegrity() {
        const missingTypes = [];
        for (const canonicalType of node_library_1.CANONICAL_NODE_TYPES) {
            if (!this.definitions.has(canonicalType)) {
                missingTypes.push(canonicalType);
            }
        }
        if (missingTypes.length > 0) {
            const error = new Error(`[UnifiedNodeRegistry] ❌ Integrity check failed: ` +
        ...
        console.log(`[UnifiedNodeRegistry] ✅ Integrity check passed: All ${node_library_1.CANONICAL_NODE_TYPES.length} canonical types have definitions`);
    }
    static getInstance() {
    ...
    initializeFromNodeLibrary() {
        ...
        for (const schema of allSchemas) {
            try {
                ...
                const overridden = (0, unified_node_registry_overrides_1.applyNodeDefinitionOverrides)(baseDefinition, schema);
        ...
        if (!this.definitions.has('log_output')) {
            ...
            const logOutputSchema = node_library_1.nodeLibrary.getSchema('log_output');
            if (logOutputSchema) {
                try {
                    ...
                    const overridden = (0, unified_node_registry_overrides_1.applyNodeDefinitionOverrides)(baseDefinition, logOutputSchema);
                    const reconciled = this.applyDeclarativeRuntimeMetadata(overridden, baseDefinition);
    ...
    applyDeclarativeRuntimeMetadata(definition, declarativeDefinition) {
    ...
    convertNodeLibrarySchemaToUnified(schema) {
        ...
        const getDefaultFillMode = (fieldName, fieldType) => {
            ...
            const isRuntimeSemanticText = field.includes('subject') ||
            ...
            const isStructureSemanticField = field === 'fields' ||
            ...
            const isDeterministicConfig = field.includes('id') ||
                field.includes('key') ||
        ...
        const inferRole = (fieldName, fieldType) => {
            const f = (fieldName || '').toLowerCase();
            const t = (fieldType || '').toLowerCase();
            if (f.includes('subject') || f.includes('title') || f.includes('heading'))
                return 'title_like';
            if (f.includes('body') || f.includes('message') || f.includes('content'))
                return 'long_body';
            if (f.includes('summary'))
                return 'short_summary';
            if (f.includes('prompt') || f.includes('query'))
                return 'prompt';
            if (f.includes('recipient') || f === 'to' || f.includes('email'))
                return 'recipient';
            if (f.endsWith('id') || f.includes('id'))
                return 'id';
        ...
        const inferEssentialForExecution = (required, fieldName) => {
            if (required)
                return true;
            const f = (fieldName || '').toLowerCase();
        ...
        const libraryFieldUi = (fieldName, raw) => {
            ...
            if (reqIf && typeof reqIf === 'object' && reqIf !== null && 'field' in reqIf) {
                out.requiredIf = reqIf;
            }
            const visIf = fd.visibleIf;
            if (visIf && typeof visIf === 'object' && visIf !== null && 'field' in visIf) {
                out.visibleIf = visIf;
            }
        ...
        if (schema.configSchema?.required) {
            for (const fieldName of schema.configSchema.required) {
                ...
                const ui = libraryFieldUi(fieldName, optionalField);
                inputSchema[fieldName] = {
                    type,
                    ...
                    fillMode: optionalField?.fillMode ?? getDefaultFillMode(fieldName, type),
                    role: inferRole(fieldName, type),
                    essentialForExecution: inferEssentialForExecution(true, fieldName),
                    fieldIntelligence: optionalField?.fieldIntelligence,
                    runtimeContract: optionalField?.runtimeContract,
                    ...(ui ? { ui } : {}),
                };
            }
        }
        ...
        if (schema.configSchema?.optional) {
            for (const [fieldName, fieldDef] of Object.entries(schema.configSchema.optional)) {
                if (!inputSchema[fieldName]) {
                    ...
                    const ui = libraryFieldUi(fieldName, fieldDef);
                    inputSchema[fieldName] = {
                        type,
        ...
        for (const fieldName of Object.keys(inputSchema)) {
            ...
            const meta = (0, field_help_metadata_1.inferFieldHelpMetadata)(schema.type, fieldName, fd.type);
            fd.helpCategory = meta.helpCategory;
            fd.ownership = (0, field_ownership_1.classifyFieldOwnership)(fieldName, fd);
            if (meta.docsUrl) {
            ...
            fd.fieldIntelligence = (0, node_field_intelligence_1.buildFieldIntelligence)({
        ...
        const rawOutputType = (schema.outputType || schema.outputSchema?.type || 'object');
        const outputSchema = {
            default: {
                name: 'default',
        ...
        const validateConfig = (config) => {
            ...
            for (const fieldName of requiredInputs) {
                const effectiveMode = (0, fill_mode_resolver_1.resolveEffectiveFieldFillMode)(fieldName, inputSchema, config);
            ...
            function isTypeCompatible(actualType, expectedType) {
                ...
                if (expectedType === 'string')
                    return true; // String can accept most types
                if (expectedType === 'email' && actualType === 'string')
                    return true;
                if (expectedType === 'datetime' && actualType === 'string')
                    return true;
                if (expectedType === 'number' && actualType === 'string')
                    return true;
                if (expectedType === 'boolean' && (actualType === 'string' || actualType === 'number'))
                    return true;
                if (expectedType === 'array' && actualType === 'object')
                    return true;
                if (expectedType === 'object' && actualType === 'array')
                    return true;
                if (expectedType === 'json' && actualType === 'object')
                    return true;
                return false;
            }
            return {
    ...
    normalizeNodeCategory(schema) {
        ...
        if (originalCategory && directMap[originalCategory] !== undefined) {
            ...
            if (isSocialMedia)
                return 'social_media';
            if (isCommunication && (originalCategory === 'google' || originalCategory === 'microsoft'))
                return 'output';
        ...
        const socialMediaTypes = ['linkedin', 'twitter', 'instagram', 'facebook', 'youtube'];
        const isSocialMedia = socialMediaTypes.some(s => nodeType.includes(s));
        if (isSocialMedia || originalCategory === 'social')
            return 'social_media';
        ...
        if (triggerTypes.some(t => nodeType.includes(t)))
            return 'trigger';
        ...
        if (dataTypes.some(d => nodeType.includes(d)))
            return 'data';
        const utilityTypes = ['wait', 'delay', 'timeout', 'return', 'execute_workflow', 'code', 'function_item'];
        if (utilityTypes.some(u => nodeType.includes(u)))
            return 'utility';
        // Default fallback
        return 'utility';
    }
    enrichRequirementsWithCredentialType(requirements, provider) {
        const mapping = UnifiedNodeRegistry.PROVIDER_CREDENTIAL_MAP[provider.toLowerCase()];
        if (!mapping)
            return requirements;
        return requirements.map((req) => {
            if (req.credentialTypeId)
                return req; // already set by override — don't overwrite
            return {
                ...req,
     ...
     */
    extractCredentialSchema(schema, inputSchema) {
        const requirements = [];
        const credentialFieldsSet = new Set();
        const provider = Array.isArray(schema.providers) && typeof schema.providers[0] === 'string'
            ? schema.providers[0]
        ...
        for (const [fieldName, fd] of Object.entries(inputSchema)) {
            if (!(0, field_ownership_1.isCredentialOwnership)(fieldName, fd))
                continue;
            if (credentialFieldsSet.has(fieldName))
                continue;
        ...
        if (requirements.length === 0 && credentialFields.length === 0) {
            ...
            if (Array.isArray(schema.providers) && schema.providers.length > 0) {
                ...
                if (mapping) {
                    ...
                    return (0, credential_requirements_1.enrichCredentialSchema)({ requirements: [syntheticReq], credentialFields: [] });
                }
            }
            return undefined;
        }
        ...
        return (0, credential_requirements_1.enrichCredentialSchema)({
            requirements: enrichedRequirements,
            credentialFields,
        });
    }
    mergeCredentialSchema(explicitSchema, extractedSchema) {
        if (!explicitSchema)
            return (0, credential_requirements_1.enrichCredentialSchema)(extractedSchema);
        if (!extractedSchema)
            return (0, credential_requirements_1.enrichCredentialSchema)(explicitSchema);
        ...
        const seenKeys = new Set(requirements.map((req) => req.credentialTypeId || `${req.provider}:${req.category}:${req.required}`));
        const seenProviders = new Set(requirements.map((req) => req.provider).filter((p) => Boolean(p)));
        for (const req of extractedSchema.requirements || []) {
            const key = req.credentialTypeId || `${req.provider}:${req.category}:${req.required}`;
            if (seenKeys.has(key))
                continue;
            if (req.provider && seenProviders.has(req.provider))
                continue;
            seenKeys.add(key);
            if (req.provider)
        ...
        return (0, credential_requirements_1.enrichCredentialSchema)({ requirements, credentialFields });
    }
    inferProviderFromNodeType(nodeType) {
        const typeLower = nodeType.toLowerCase();
        // Gemini uses an API key, not Google OAuth — must come before the generic 'google' check
        if (typeLower === 'google_gemini')
            return 'gemini';
        if (typeLower.includes('google'))
            return 'google';
        if (typeLower.includes('slack'))
            return 'slack';
        if (typeLower === 'discord_webhook')
            return 'discord_webhook';
        if (typeLower.includes('discord'))
            return 'discord';
        if (typeLower.includes('openai') || typeLower.includes('gpt'))
            return 'openai';
        if (typeLower.includes('anthropic') || typeLower.includes('claude'))
            return 'anthropic';
        if (typeLower.includes('notion'))
            return 'notion';
        if (typeLower.includes('airtable'))
            return 'airtable';
        if (typeLower.includes('vercel'))
            return 'vercel';
        if (typeLower.includes('salesforce'))
            return 'salesforce';
        if (typeLower.includes('hubspot'))
            return 'hubspot';
        if (typeLower.includes('pipedrive'))
            return 'pipedrive';
        if (typeLower.includes('zoho'))
            return 'zoho';
        if (typeLower.includes('odoo'))
            return 'odoo';
        if (typeLower.includes('mailchimp'))
            return 'mailchimp';
        if (typeLower.includes('activecampaign'))
            return 'activecampaign';
        if (typeLower.includes('intercom'))
            return 'intercom';
        if (typeLower.includes('freshdesk'))
            return 'freshdesk';
        if (typeLower.includes('postgres') || typeLower === 'database_read' || typeLower === 'database_write')
            return 'postgresql';
        if (typeLower.includes('mysql'))
            return 'mysql';
        if (typeLower.includes('mongo'))
            return 'mongodb';
        if (typeLower.includes('redis'))
            return 'redis';
        if (typeLower.includes('github'))
            return 'github';
        if (typeLower.includes('gitlab'))
            return 'gitlab';
        if (typeLower.includes('bitbucket'))
            return 'bitbucket';
        if (typeLower.includes('jira'))
            return 'jira';
        if (typeLower.includes('jenkins'))
            return 'jenkins';
        if (typeLower.includes('clickup'))
            return 'clickup';
        if (typeLower.includes('twitter'))
            return 'twitter';
        if (typeLower.includes('webhook'))
            return 'webhook';
        if (typeLower.includes('ftp'))
            return 'ftp';
        if (typeLower.includes('sftp'))
            return 'sftp';
        if (typeLower.includes('stripe'))
            return 'stripe';
        if (typeLower.includes('paypal'))
            return 'paypal';
        if (typeLower.includes('shopify'))
            return 'shopify';
        if (typeLower.includes('woocommerce'))
            return 'woocommerce';
        return undefined;
    }
    inferCredentialCategory(fieldName, helpCategory) {
        // URL-type categories are config values, not secrets — never treat them as credentials
        const URL_CONFIG_CATEGORIES = new Set([
            'webhook_url',
            'base_url',
            'api_endpoint',
            'callback_url',
        ...
        if (nameLower === 'apikey' || nameLower.includes('api_key'))
            return 'api_key';
    ...
    migrateConfig(nodeType, oldConfig) {
    ...
    validateConfig(nodeType, config) {
        ...
        if (!(0, node_library_1.isValidCanonicalNodeType)(nodeType)) {
            const sampleTypes = node_library_1.CANONICAL_NODE_TYPES.slice(0, 10).join(', ');
            throw new Error(`[NodeAuthority] ❌ Invalid node type: "${nodeType}". ` +
        ...
        if (!definition) {
            ...
            throw new Error(`[NodeAuthority] ❌ Integrity error: Canonical node type '${nodeType}' not found in registry. ` +
                `This indicates a system initialization failure. All canonical types must have UnifiedNodeDefinitions.`);
        }
     ...
     */
    getCredentialPreflightDescriptor(nodeType) {
        const def = this.get(nodeType);
        const reqs = def?.credentialSchema?.requirements ?? [];
        const scopesFromSchema = reqs.flatMap((r) => r.scopes || []);
        const hasSchemaCreds = reqs.length > 0 || (def?.credentialSchema?.credentialFields?.length ?? 0) > 0;
        ...
        const oauthish = mappedAuthType === 'oauth2' ||
        ...
        const credentialType = oauthish ? 'OAuth' : 'API_KEY';
    ...
    inferCredentialProviderForPreflight(nodeType) {
        const t = nodeType.toLowerCase();
        const fromBase = this.inferProviderFromNodeType(nodeType);
        if (fromBase)
            return fromBase;
        if (t === 'http_request' || t === 'http_post' || t === 'graphql' || t === 'webhook_response')
            return 'http';
        if (t === 'email')
            return 'email';
        if (t.includes('database_') || t === 'postgresql' || t === 'db' || t === 'mysql' || t === 'mongodb' || t === 'redis')
            return 'database';
        if (t === 'linkedin' || t === 'twitter' || t === 'instagram' || t === 'facebook')
            return t.split('_')[0];
        if (t === 'vercel')
            return 'vercel';
        return undefined;
    }
    inferDefaultOAuthScopes(nodeType, provider) {
        const t = nodeType.toLowerCase();
        ...
        if (provider === 'vercel' || t === 'vercel')
            return ['deployments:read', 'deployments:write'];
        return [];
    }
    buildCredentialLookupKeys(nodeType, provider, credentialFields) {
        const keys = new Set([nodeType]);
        if (provider)
            keys.add(provider);
        ...
        if (nodeType === 'openai_gpt' || nodeType === 'chat_model')
            keys.add('openai');
    ...
    getOutputSchema(nodeType) {
        const definition = this.get(nodeType);
        return definition?.outputSchema;
    }
    getInputSchema(nodeType) {
        const definition = this.get(nodeType);
        return definition?.inputSchema;
    }
    /**
     ...
     */
    getEffectiveOutputSchema(nodeType, config) {
        const def = this.get(nodeType);
        if (!def?.outputSchema?.default) {
            return undefined;
        }
        const port = def.outputSchema.default;
        const baseType = port.schema?.type || 'object';
        const baseProperties = port.schema?.properties;
    ...
    getOutgoingPortsForWorkflowNode(node) {
        ...
        return (0, branching_node_ports_1.getBranchOutgoingPortsForNode)(nodeType, config, def?.outgoingPorts || ['output']);
    }
    allowsBranching(nodeType) {
        const definition = this.get(nodeType);
    ...
    normalizeRegistryAliasPhrase(raw) {
        return String(raw || '')
            .toLowerCase()
            .replace(/[_-]+/g, ' ')
            .replace(/[^a-z0-9]+/g, ' ')
    ...
    addRegistryAlias(index, phrase, nodeType) {
    ...
    buildRegistryAliasIndex() {
        ...
        for (const def of this.definitions.values()) {
            ...
            for (const tag of def.tags || [])
                this.addRegistryAlias(index, String(tag), def.type);
            for (const capability of def.capabilities || [])
                this.addRegistryAlias(index, String(capability), def.type);
            const criteria = def.aiSelectionCriteria;
            for (const keyword of criteria?.keywords || [])
                this.addRegistryAlias(index, String(keyword), def.type);
            for (const useCase of criteria?.useCases || [])
                this.addRegistryAlias(index, String(useCase), def.type);
            for (const whenToUse of criteria?.whenToUse || [])
                this.addRegistryAlias(index, String(whenToUse), def.type);
            const operation = (def.inputSchema || {}).operation;
            const operationValues = Array.isArray(operation?.enum)
                ? operation.enum
                : Array.isArray(operation?.oneOf)
                    ? operation.oneOf.map((x) => x?.const || x?.enum?.[0]).filter(Boolean)
                    : [];
    ...
    resolveRegistryDerivedAlias(alias) {
    ...
    getCommunicationCategoryDefinitions() {
    ...
    collectPlannerConfigKeys(config) {
        const keys = new Set();
    ...
    scoreConfigAgainstDefinition(def, configKeys) {
        let score = 0;
    ...
    definitionSendsOAuthEmail(def) {
        if (!def)
            return false;
        const caps = def.capabilities || [];
        return caps.some((c) => {
            const x = String(c).toLowerCase();
            return (x.startsWith('email.send') ||
                x.startsWith('gmail.') ||
     ...
     */
    getPreferredGmailSendCanonicalType() {
    ...
    ensureEmailChannelIntentKeywordCache() {
        ...
        for (const def of this.definitions.values()) {
            if (!this.definitionSendsOAuthEmail(def))
                continue;
    ...
    workflowTextSuggestsRegistryEmailChannel(intentText) {
     ...
     */
    shouldRunCommunicationDisambiguation(fromDef, role, configKeys) {
     ...
     */
    resolvePlannedStepCanonicalType(rawType, role, config, context) {
        const normalized = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(rawType);
        ...
        if (this.shouldRunCommunicationDisambiguation(fromDef, role, configKeys)) {
            ...
            if (bestType !== normalized && bestScore >= 2) {
                console.log(`[UnifiedNodeRegistry] Planned-step disambiguation: "${rawType}" (${normalized}) → "${bestType}" (schema overlap score ${bestScore}, role=${role || 'n/a'})`);
                resolved = bestType;
            }
        }
        const r = (role || '').toLowerCase().trim();
        ...
        if (resolvedDef?.category === 'ai' &&
            ...
            if (gmailType !== resolved) {
                console.log(`[UnifiedNodeRegistry] Email intent → Gmail: "${rawType}" (${resolved}) → "${gmailType}" (output step; text matched email-channel registry keywords)`);
                resolved = gmailType;
            }
        }
        return resolved;
     ...
     */
    reconcileMisroutedAiCommunicationNodes(workflow) {
        ...
        const nodes = workflow.nodes.map((node) => {
            ...
            const role = data.metadata?.aiRole ||
                data.stepType ||
                data.metadata?.stepType;
            const wfIntent = [
                workflow?.metadata?.summary,
                workflow?.metadata?.generatedFrom,
                workflow?.metadata?.userPrompt,
            ]
                .filter(Boolean)
                .join(' ');
            const resolved = this.resolvePlannedStepCanonicalType(semanticType, role, data.config, {
                workflowIntentText: wfIntent,
                stepLabel: typeof data.label === 'string' ? data.label : undefined,
            });
            const normalizedBefore = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(semanticType);
     ...
     */
    hasWorkflowBehavior(nodeType, behavior) {
        const def = this.get(nodeType);
    ...
    isUtilityNode(nodeType) {
        ...
        if ((def.tags || []).some(t => utilityTags.includes(t.toLowerCase())))
            return true;
        return false;
    }
    /**
     ...
     */
    getBuildValueContext(targetNodeType, upstreamNodeType) {
        ...
        if (upstreamNodeType) {
            ...
            if (props && typeof props === 'object') {
                upstreamFields = Object.entries(props).map(([key, val]) => ({
                    name: key,
        ...
        for (const [name, field] of Object.entries(targetDef.inputSchema)) {
            ...
            const isBuildtimeEligible = fillMode?.default === 'buildtime_ai_once' ||
                fillMode?.supportsBuildtimeAI === true;
            if (!isBuildtimeEligible)
                continue;
            if ((0, field_ownership_1.isCredentialOwnership)(name, field))
                continue;
            targetFields.push({
                name,
...
exports.UnifiedNodeRegistry = UnifiedNodeRegistry;
/**
 ...
 */
UnifiedNodeRegistry.PROVIDER_CREDENTIAL_MAP = {
    // AI
...
exports.unifiedNodeRegistry = UnifiedNodeRegistry.getInstance();
