...
exports.NodeSchemaRegistry = void 0;
...
class NodeSchemaRegistry {
    ...
    registerFromNodeSchema(schema) {
        ...
        const contract = {
            ...
            category: schema.category, // schema.category is string, NodeCategory is union type
            inputs,
            outputs,
     ...
     */
    validateEdge(sourceNode, targetNode, edge) {
        ...
        if (sourceType === 'manual_trigger' && sourceOutput === 'data') {
            ...
            errors.push(`Source node "${sourceType}" uses incorrect output port "${sourceOutput}". ` +
            ...
            errors.push(`Source node "${sourceType}" does not have output port "${sourceOutput}". ` +
        ...
        if (targetType === 'slack_message' && targetInput === 'input') {
            ...
            if (!targetSchema.inputs.includes(targetInput)) {
                errors.push(`Target node "${targetType}" does not have input port "${targetInput}". ` +
    ...
    inferCredentialType(schema) {
        ...
        if (credentialNodes.includes(schema.type)) {
            ...
            if (schema.type === 'slack_message')
                return 'slackOAuth';
            if (schema.type === 'linkedin')
                return 'linkedinOAuth';
            if (schema.type === 'twitter')
                return 'twitterOAuth';
            if (schema.type === 'instagram')
                return 'instagramOAuth';
            if (schema.type.startsWith('google_'))
                return 'googleOAuth';
...
exports.NodeSchemaRegistry = NodeSchemaRegistry;
