...
exports.unifiedGraphOrchestrator = void 0;
...
const node_injection_coordinator_1 = require("./node-injection-coordinator");
...
const terminal_mode_policy_1 = require("./terminal-mode-policy");
class UnifiedGraphOrchestratorImpl {
    getNodeType(node) {
     ...
     */
    initializeWorkflow(nodes, initialExecutionOrder, dslExecutionOrder, switchContext) {
        ...
        const executionOrder = initialExecutionOrder || execution_order_manager_1.executionOrderManager.initialize(workflow, dslExecutionOrder);
        ...
        if (switchContext) {
            ...
            for (const ctx of contexts) {
                seedWorkflow = this.wireSwitchCaseEdges(seedWorkflow, ctx);
            }
        }
        ...
        const logOutputNodes = seedWorkflow.nodes.filter((n) => {
            const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(n.data?.type || n.type || '');
        ...
        for (const logOutputNode of logOutputNodes) {
            ...
            if (incomingEdgeCount > 1) {
                throw new Error(`Invalid workflow structure: log_output node "${logOutputNode.id}" has ${incomingEdgeCount} incoming edges. ` +
        ...
        const reconciliationResult = edge_reconciliation_engine_1.edgeReconciliationEngine.reconcileEdges(seedWorkflow, executionOrder);
        ...
        if (!postWireValidation.valid && shouldEnforceBranchTerminalLineage) {
            const terminalLineageErrors = postWireValidation.errors.filter(e => e.toLowerCase().includes('orphan') ||
     ...
     */
    wireSwitchCaseEdges(workflow, switchContext) {
        ...
        const switchDef = unified_node_registry_1.unifiedNodeRegistry.get(switchNodeType);
        const outgoingPorts = switchDef?.outgoingPorts ?? [];
        ...
        caseEntries.forEach(([caseValue, targetSpec], index) => {
            const explicitSlot = targetSpec && typeof targetSpec === 'object' && !Array.isArray(targetSpec)
                ? targetSpec.slot
                : undefined;
            const semanticPort = typeof caseValue === 'string' && caseValue.trim().length > 0 ? caseValue.trim() : undefined;
            const portLabel = semanticPort || explicitSlot || outgoingPorts[index] || `case_${index + 1}`;
            const targetNodeId = targetSpec && typeof targetSpec === 'object' && !Array.isArray(targetSpec)
                ? targetSpec.targetNodeId
                : undefined;
            const targetNodeType = targetSpec && typeof targetSpec === 'object' && !Array.isArray(targetSpec)
            ...
            if (!targetNode && targetNodeId && targetNodeType) {
                ...
                if (candidateByPosition &&
                    ...
                    console.log(`[UnifiedGraphOrchestrator] wireSwitchCaseEdges: case "${caseValue}" — stale ID "${targetNodeId}", resolved via index+type to ${this.getNodeType(candidateByPosition)}(${candidateByPosition.id.substring(0, 8)})`);
                }
            }
            // (2) Type-based lookup — find the first unassigned node whose type matches.
            if (!targetNode && targetNodeType) {
                targetNode = downstreamNodes.find((n) => !assignedNodeIds.has(n.id) && this.getNodeType(n) === targetNodeType);
            }
            ...
            if (!targetNode && targetNodeType) {
                console.warn(`[UnifiedGraphOrchestrator] wireSwitchCaseEdges: case "${caseValue}" has no compatible target for type "${targetNodeType}" on switch ${switchNodeId}`);
                return;
            }
            if (!targetNode) {
                console.warn(`[UnifiedGraphOrchestrator] wireSwitchCaseEdges: no downstream node for case "${caseValue}" (type: ${targetNodeType || 'unknown'}, id: ${targetNodeId || 'none'})`);
                return;
            }
            ...
            const targetNodeDef = unified_node_registry_1.unifiedNodeRegistry.get(resolvedTargetNodeType);
            ...
            if (isTerminalNode && resolvedTargetNodeType === 'log_output') {
                ...
                if (existingEdgesToThisLogOutput.length > 0) {
                    console.warn(`[UnifiedGraphOrchestrator] wireSwitchCaseEdges: terminal node "${targetNode.id}" (${resolvedTargetNodeType}) ` +
            ...
            console.log(`[UnifiedGraphOrchestrator] wireSwitchCaseEdges: case "${caseValue}" → ${(targetNodeType || this.getNodeType(targetNode))}(${targetNode.id.substring(0, 8)}) via port ${portLabel}`);
        });
        return {
            ...workflow,
     ...
     */
    async injectNode(workflow, node, context) {
        const result = node_injection_coordinator_1.nodeInjectionCoordinator.injectNode(workflow, node, context);
     ...
     */
    removeEdges(workflow, shouldRemove) {
        const errors = [];
        const warnings = [];
        ...
        const executionOrder = execution_order_manager_1.executionOrderManager.initialize(updatedWorkflow);
    ...
    removeNode(workflow, nodeId) {
        ...
        executionOrder = execution_order_manager_1.executionOrderManager.removeNode(executionOrder, nodeId);
        // Reconcile edges
        const reconciliationResult = edge_reconciliation_engine_1.edgeReconciliationEngine.reconcileEdges(updatedWorkflow, executionOrder);
        if (reconciliationResult.errors.length > 0) {
            errors.push(...reconciliationResult.errors);
     ...
     */
    reconcileWorkflow(workflow, tagsFromVariation) {
        // Get current execution order
        const executionOrder = execution_order_manager_1.executionOrderManager.initialize(workflow);
        // Reconcile edges
        // ✅ PHASE 4: Pass tagsFromVariation to preserve nodes in tags
        const reconciliationResult = edge_reconciliation_engine_1.edgeReconciliationEngine.reconcileEdges(workflow, executionOrder, tagsFromVariation);
        // Structural validation must use the same execution order as reconciliation (avoid stale order + duplicate reconcile).
        const postReconcileValidation = this.validateWorkflow(reconciliationResult.workflow, executionOrder);
        return {
            workflow: reconciliationResult.workflow,
            executionOrder,
     ...
     */
    validateWorkflow(workflow, existingExecutionOrder) {
        const errors = [];
        const warnings = [];
        ...
        if (existingExecutionOrder) {
            ...
            if (reconciliation.errors.length > 0) {
                warnings.push(`Reconciliation reported ${reconciliation.errors.length} issue(s): ${reconciliation.errors.join(', ')}`);
            }
        ...
        const nodesNotInOrder = currentWorkflow.nodes.filter(n => !orderedNodeIds.includes(n.id));
        if (nodesNotInOrder.length > 0) {
            warnings.push(`${nodesNotInOrder.length} node(s) not in execution order: ${nodesNotInOrder.map(n => n.id).join(', ')}`);
        }
        // Validate edges against execution order
        const edgeValidation = edge_reconciliation_engine_1.edgeReconciliationEngine.validateEdges(currentWorkflow, executionOrder);
        ...
        if (orphanedNodes.length > 0) {
            ...
            const requiredOrphanedNodes = [];
            const nonRequiredOrphanedNodes = [];
            ...
            if (requiredOrphanedNodes.length > 0) {
                ...
                errors.push(`Found ${requiredOrphanedNodes.length} required orphaned node(s) (edge creation may have failed): ${orphanDiagnostics.join(', ')}`);
            }
            // Non-required orphaned nodes = warning (should be auto-removed by reconciliation)
            if (nonRequiredOrphanedNodes.length > 0) {
                warnings.push(`Found ${nonRequiredOrphanedNodes.length} non-required orphaned node(s) (should be auto-removed): ${nonRequiredOrphanedNodes.map(n => `${n.type || 'unknown'} (${n.id})`).join(', ')}`);
            }
        }
        ...
        const ancestorsOf = (nodeId) => {
            ...
            while (stack.length) {
                ...
                if (seen.has(id))
                    continue;
                seen.add(id);
                for (const p of reverseAdj.get(id) || [])
                    stack.push(p);
            }
            return seen;
        };
        for (const n of currentWorkflow.nodes) {
            ...
            if (op !== 'send')
                continue;
            if (cfg.recipientSource !== 'extract_from_sheet')
                continue;
            ...
            if (!hasSheetsUpstream && !inlineId) {
                warnings.push(`Gmail node "${n.id}": recipientSource is extract_from_sheet but there is no upstream Google Sheets node and no inline Spreadsheet ID. Add a Sheets node before Gmail or set optional Spreadsheet ID + sheet on Gmail for API fallback.`);
            }
        }
        ...
        for (const n of currentWorkflow.nodes) {
            ...
            const cfg = n.data?.config ?? {};
            const effectivePorts = unified_node_registry_1.unifiedNodeRegistry.getOutgoingPortsForWorkflowNode(n);
            const isCaseDriven = effectivePorts.length > 0 && effectivePorts.every(p => p !== 'output' && p !== 'true' && p !== 'false');
            if (!isCaseDriven)
                continue;
            const caseCount = Array.isArray(cfg.cases) ? cfg.cases.length : 0;
            if (caseCount === 0)
                continue; // No cases configured yet — skip (not a structural error)
            ...
            if (outDegree !== caseCount) {
                ...
                warnings.push(`Switch node "${n.id}": out-degree ${outDegree} does not match cases.length ${caseCount} — some branches may be unwired`);
            }
        }
        const ifElseFormBinding = (0, form_ifelse_binding_1.validateIfElseConditionsAgainstUpstreamForm)(currentWorkflow);
        ...
        const terminalPolicy = (0, terminal_mode_policy_1.evaluateTerminalMode)(currentWorkflow);
     ...
     */
    ensureTerminalNodes(workflow) {
        const errors = [];
        const warnings = [];
        // ✅ UNIVERSAL: Query registry for always-terminal nodes
        const alwaysTerminalNodes = unified_node_registry_1.unifiedNodeRegistry.getAlwaysTerminalNodes();
        ...
        for (const node of terminalNodesInWorkflow) {
            ...
            if (outgoingEdges.length > 0) {
                ...
                warnings.push(`Node ${node.id} (${this.getNodeType(node)}) should be terminal but has ${outgoingEdges.length} outgoing edge(s)`);
...
exports.unifiedGraphOrchestrator = new UnifiedGraphOrchestratorImpl();
