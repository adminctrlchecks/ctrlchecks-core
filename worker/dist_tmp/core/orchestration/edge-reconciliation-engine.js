...
exports.edgeReconciliationEngine = void 0;
...
const error_prevention_1 = require("../error-prevention");
const universal_edge_creation_service_1 = require("../../services/edges/universal-edge-creation-service");
const node_capability_registry_dsl_1 = require("../../services/ai/node-capability-registry-dsl");
const universal_node_type_checker_1 = require("../utils/universal-node-type-checker");
const crypto_1 = require("crypto");
class EdgeReconciliationEngineImpl {
    getNodeType(node) {
     ...
     */
    reconcileEdges(workflow, executionOrder, tagsFromVariation) {
        const errors = [];
        const warnings = [];
        ...
        const edgesToKeep = workflow.edges.filter(edge => !validation.edgesToRemove.some(e => e.id === edge.id));
        // Step 3: Create missing edges based on execution order
        const edgesToAdd = [];
        const workingEdges = [...edgesToKeep];
        const edgeIdsToDrop = new Set();
        const orderedNodeIds = execution_order_manager_1.executionOrderManager.getOrderedNodeIds(executionOrder);
        ...
        for (let i = 0; i < orderedNodeIds.length - 1; i++) {
            ...
            if (sourceNodeForLinear) {
                const linearSourceType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(sourceNodeForLinear.type || sourceNodeForLinear.data?.type || '');
                ...
                if (unified_node_registry_1.unifiedNodeRegistry.get(linearSourceType)?.isBranching === true) {
                    continue;
                }
            }
            ...
            if (sourceNodeForLinear && targetNodeForLinear) {
                ...
                const targetLinearType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(targetNodeForLinear.type || targetNodeForLinear.data?.type || '');
                const sourceLinearType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(sourceNodeForLinear.type || sourceNodeForLinear.data?.type || '');
                const targetLinearDef = unified_node_registry_1.unifiedNodeRegistry.get(targetLinearType);
                const sourceLinearDef = unified_node_registry_1.unifiedNodeRegistry.get(sourceLinearType);
                // Terminal nodes: inbound wiring is Step 7 (branch-aware); never linear backbone in/out of terminals
                if (targetLinearDef?.isTerminal === true || sourceLinearDef?.isTerminal === true) {
                    continue;
                }
                if (targetLinearDef?.workflowBehavior?.alwaysTerminal === true ||
            ...
            if (sourceNodeForLinear && targetNodeForLinear) {
                if (this.areExclusiveForkDescendantsInDifferentRegions(workflow.nodes, edgesToKeep, sourceId, targetId) ||
                    this.areConsecutivePairExclusiveBranchHeadsByOrder(workflow.nodes, orderedNodeIds, sourceId, targetId, workingEdges)) {
                    continue;
                }
            }
            // Check if edge already exists
            const edgeExists = workingEdges.some(e => e.source === sourceId && e.target === targetId);
            if (!edgeExists) {
                // Create edge using registry-driven logic
                const edge = this.createEdgeFromOrder(workflow, sourceId, targetId, executionOrder, undefined, workingEdges);
                if (edge) {
                    edgesToAdd.push(edge);
                    workingEdges.push(edge);
                }
                else {
                    warnings.push(`Could not create edge from ${sourceId} to ${targetId} - handle resolution failed`);
                }
            }
        }
        ...
        for (const branchingNode of branchingNodes) {
            ...
            const outgoingPorts = unified_node_registry_1.unifiedNodeRegistry.getOutgoingPortsForWorkflowNode(branchingNode);
            ...
            if (outgoingPorts.length > 1 && potentialTargets.length > 0) {
                const usedTargets = new Set(workingEdges
                    .filter(e => e.source === branchingNode.id)
                    .map(e => e.target));
                const branchPorts = outgoingPorts.filter(p => p !== 'output');
                const explicitBranchEdges = workingEdges.filter((e) => e.source === branchingNode.id &&
                ...
                branchPorts.forEach((portName, index) => {
                    const edgeForPortExists = workingEdges.some(e => e.source === branchingNode.id &&
                        (e.type === portName || e.sourceHandle === portName));
                    if (edgeForPortExists)
                        return;
                    ...
                    if (hasExplicitBranchMapping) {
                        errors.push(`Branch node ${branchingNode.id}: missing explicit mapping for port ${portName}; auto-wiring is disabled in deterministic branch mode`);
                        return;
                    }
                    ...
                    if (!targetCandidate) {
                        errors.push(`Switch/If node ${branchingNode.id}: no distinct target available for branch port ${portName} — the planner may have emitted too few nodes for this branch`);
                        return;
                    }
                    const newEdge = this.createEdgeFromOrder(workflow, branchingNode.id, targetCandidate, executionOrder, portName, workingEdges);
                    if (newEdge) {
                        ...
                        edgesToAdd.push({
                            id: `edge_${(0, crypto_1.randomUUID)()}`,
                ...
                if (uniqueBranchTargets.size < branchTargets.length) {
                    ...
                    for (const target of branchTargets) {
                        if (seen.has(target)) {
                            errors.push(`Switch/If node ${branchingNode.id}: branch ports share the same target node ${target} — each branch must have a distinct target node`);
                            break;
                        }
                        seen.add(target);
                    }
        ...
        for (const mergeNode of mergeNodes) {
            ...
            for (const sourceId of potentialSources) {
                ...
                if (this.shouldHaveEdge(sourceNode, mergeNode)) {
                    const edgeExists = workingEdges.some(e => e.source === sourceId && e.target === mergeNode.id);
                    if (!edgeExists) {
                        const edge = this.createEdgeFromOrder(workflow, sourceId, mergeNode.id, executionOrder, undefined, workingEdges);
                        if (edge) {
                            edgesToAdd.push(edge);
                            workingEdges.push(edge);
                        }
        ...
        const outputNodes = workflow.nodes.filter(n => {
            return (0, universal_node_type_checker_1.isOutputNode)(n);
        });
        for (const outputNode of outputNodes) {
            const outputNodeType = this.getNodeType(outputNode);
            const outputNodeDef = unified_node_registry_1.unifiedNodeRegistry.get(outputNodeType);
            // ✅ Terminal nodes are handled exclusively by Step 7 (branch-aware predecessor logic).
            // Step 6 must NOT wire terminal nodes — it has no branch awareness and would connect
            // both branch terminals to the same predecessor (the last output node in execution order).
            if (outputNodeDef?.isTerminal === true)
                continue;
            ...
            if (hasIncoming)
                continue; // Already connected (includes branch edges from if_else / switch)
            // Find the last transformation node before this output node
            let lastTransformationNodeId = null;
            for (let i = outputIndex - 1; i >= 0; i--) {
                ...
                const candidateDef = unified_node_registry_1.unifiedNodeRegistry.get(candidateType);
                // Skip trigger nodes
                if (candidateDef?.category === 'trigger')
                    continue;
                // Skip pure output nodes, but NOT primary data nodes (e.g. google_sheets has output capability)
                if ((0, universal_node_type_checker_1.isOutputNode)(candidateNode) && !(0, universal_node_type_checker_1.hasPrimaryDataRole)(candidateNode))
                    continue;
            ...
            if (lastTransformationNodeId) {
                const edgeExists = workingEdges.some(e => e.source === lastTransformationNodeId && e.target === outputNode.id);
                if (!edgeExists) {
                    const edge = this.createEdgeFromOrder(workflow, lastTransformationNodeId, outputNode.id, executionOrder, undefined, workingEdges);
                    if (edge) {
                        edgesToAdd.push(edge);
                        workingEdges.push(edge);
                        console.log(`[EdgeReconciliationEngine] ✅ Connected output node: ${this.getNodeType(outputNode)} (${outputNode.id.substring(0, 8)}) from last transformation node`);
                    }
                }
            }
            ...
            if (!lastTransformationNodeId) {
                const triggerNodeId = orderedNodeIds.find((id) => {
                    ...
                    return unified_node_registry_1.unifiedNodeRegistry.get(nt)?.category === 'trigger';
                });
                const targetHasIncoming = workingEdges.some((e) => e.target === outputNode.id);
                if (triggerNodeId && !targetHasIncoming) {
                    const edge = this.createEdgeFromOrder(workflow, triggerNodeId, outputNode.id, executionOrder, undefined, workingEdges);
                    if (edge) {
                        edgesToAdd.push(edge);
                        workingEdges.push(edge);
                        console.log(`[EdgeReconciliationEngine] ✅ Linear fallback connected trigger -> output: ${triggerNodeId.substring(0, 8)} -> ${outputNode.id.substring(0, 8)}`);
                    }
                }
            }
        }
        // ✅ STEP 7: Ensure ALL output nodes (including terminals) are properly connected
        // ✅ UNIVERSAL FIX: Check for ALL output nodes using isOutputNode(), not just specific types
        // This ensures any output node (CRM, email, terminal, etc.) is connected properly
        const allOutputNodes = workflow.nodes.filter(n => {
            return (0, universal_node_type_checker_1.isOutputNode)(n);
        });
        const terminalNodes = allOutputNodes.filter(n => {
            const nodeType = this.getNodeType(n);
            const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
            return nodeDef?.isTerminal === true;
        });
        console.log(`[EdgeReconciliationEngine] 🔍 DEBUG STEP 7: Found ${allOutputNodes.length} output node(s) (${terminalNodes.length} terminal), ` +
        ...
        // Process terminal nodes in execution order (stable wiring for multi-branch terminals)
        const terminalNodesSorted = [...terminalNodes].sort((a, b) => orderedNodeIds.indexOf(a.id) - orderedNodeIds.indexOf(b.id));
        const usedTerminalPredecessors = new Set();
        for (const terminalNode of terminalNodesSorted) {
            const terminalIndex = orderedNodeIds.indexOf(terminalNode.id);
            const terminalType = this.getNodeType(terminalNode);
            console.log(`[EdgeReconciliationEngine] 🔍 DEBUG terminal(${terminalNode.id.substring(0, 8)}): ` +
                `type=${terminalType}, index=${terminalIndex}, ` +
                `totalNodes=${orderedNodeIds.length}`);
            if (terminalIndex < 0) {
                console.warn(`[EdgeReconciliationEngine] ⚠️  Terminal node ${terminalNode.id} not found in execution order`);
                continue;
            }
            const structuralSnapshot = [...workingEdges];
            // Branch-aware predecessor: avoids wiring the wrong branch's communication node into a sibling terminal
            let lastNonTerminalNodeId = this.pickBranchAwarePredecessorForLogOutput(workflow, orderedNodeIds, structuralSnapshot, terminalNode.id, terminalIndex, {
                usedPredecessorIds: usedTerminalPredecessors,
                disableLegacyFallback: true,
            });
            if (lastNonTerminalNodeId) {
                console.log(`[EdgeReconciliationEngine] ✅ Branch-aware predecessor for terminal(${terminalNode.id.substring(0, 8)}): ` +
            ...
            if (lastNonTerminalNodeId) {
                ...
                // Check if edge already exists
                const edgeExists = workingEdges.some(e => e.source === lastNonTerminalNodeId && e.target === terminalNode.id);
                console.log(`[EdgeReconciliationEngine] 🔍 DEBUG Edge check: ` +
                    `${sourceType}(${lastNonTerminalNodeId.substring(0, 8)}) → terminal(${terminalNode.id.substring(0, 8)}): ` +
                    `exists=${edgeExists}`);
                const structuralEdgesSnapshot = [...workingEdges];
                const existingIncoming = structuralEdgesSnapshot.filter(e => e.target === terminalNode.id);
                const hasLineageValidIncoming = existingIncoming.some((inc) => {
                    if (inc.source === lastNonTerminalNodeId)
                        return true;
                    ...
                    const incomingSourceDef = unified_node_registry_1.unifiedNodeRegistry.get(incomingSourceType);
                    ...
                    if ((incomingSourceDef?.tags || []).includes('merge') || incomingSourceType === 'merge') {
                        return true;
                    }
                    // ✅ PRIMARY FIX: Case edges from branching nodes are legitimate predecessors
                    // When a terminal is a direct case target of a branching node (switch/if_else),
                    // the case edge (true/false/case_*) is a legitimate connection that should be preserved.
                    // This prevents Step 7 from incorrectly removing case edges as "stale" edges.
                    if (incomingSourceDef?.isBranching === true) {
                        const edgeType = String(inc.type || inc.sourceHandle || '').toLowerCase();
                        const isCaseEdge = edgeType === 'true' || edgeType === 'false' || edgeType.startsWith('case_');
                        if (isCaseEdge) {
                            return true;
                        }
                    }
                ...
                if (existingIncoming.length > 0 && hasLineageValidIncoming) {
                    ...
                    console.log(`[EdgeReconciliationEngine] 🔍 Skip log_output wiring: lineage-valid incoming already exists ` +
                        `(${lastNonTerminalNodeId.substring(0, 8)} -> ${terminalNode.id.substring(0, 8)})`);
                }
                else if (!edgeExists) {
                    if (existingIncoming.length > 0 && !hasLineageValidIncoming) {
                        ...
                        console.warn(`[EdgeReconciliationEngine] ⚠️  Rewiring stale incoming edges for log_output(${terminalNode.id.substring(0, 8)}): ` +
                            `${existingIncoming.map((e) => e.source.substring(0, 8)).join(', ')}`);
                    }
                    const wouldCrossForkFanIn = existingIncoming.some(inc => this.areExclusiveForkDescendantsInDifferentRegions(workflow.nodes, structuralEdgesSnapshot, lastNonTerminalNodeId, inc.source));
                    const crossExclusiveFork = this.areExclusiveForkDescendantsInDifferentRegions(workflow.nodes, structuralEdgesSnapshot, lastNonTerminalNodeId, terminalNode.id) ||
                        this.areConsecutivePairExclusiveBranchHeadsByOrder(workflow.nodes, orderedNodeIds, lastNonTerminalNodeId, terminalNode.id, structuralEdgesSnapshot);
                    if (wouldCrossForkFanIn) {
                        console.log(`[EdgeReconciliationEngine] 🔍 Skip log_output wiring: cross-fork fan-in avoided ` +
                            `(would add ${lastNonTerminalNodeId.substring(0, 8)} but log already has edge(s) from ` +
                            `${existingIncoming.map(e => e.source.substring(0, 8)).join(', ')})`);
                    }
                    else if (crossExclusiveFork) {
                        console.log(`[EdgeReconciliationEngine] 🔍 Skip log_output wiring: exclusive fork regions ` +
                            `(${lastNonTerminalNodeId.substring(0, 8)} → ${terminalNode.id.substring(0, 8)})`);
                    }
                    else {
                        // Create edge from last non-terminal node to log_output
                        console.log(`[EdgeReconciliationEngine] 🔍 DEBUG Attempting to create edge: ` +
                            `${sourceType}(${lastNonTerminalNodeId.substring(0, 8)}) → log_output(${terminalNode.id.substring(0, 8)})`);
                        const edge = this.createEdgeFromOrder(workflow, lastNonTerminalNodeId, terminalNode.id, executionOrder, undefined, workingEdges);
                        if (edge) {
                            edgesToAdd.push(edge);
                            workingEdges.push(edge);
                            usedTerminalPredecessors.add(lastNonTerminalNodeId);
                            console.log(`[EdgeReconciliationEngine] ✅ Connected last non-terminal node (${lastNonTerminalNodeId}) → log_output (${terminalNode.id})`);
                        }
                        else {
                            ...
                            const staleOutgoingToOtherLog = workingEdges.find((e) => {
                                if (e.source !== lastNonTerminalNodeId || e.target === terminalNode.id)
                                    return false;
                            ...
                            if (staleOutgoingToOtherLog) {
                                ...
                                if (staleIdx >= 0)
                                    workingEdges.splice(staleIdx, 1);
                                console.warn(`[EdgeReconciliationEngine] ⚠️  Rewiring stale outgoing log edge from ${lastNonTerminalNodeId.substring(0, 8)}: ` +
                                    `${staleOutgoingToOtherLog.target.substring(0, 8)} → ${terminalNode.id.substring(0, 8)}`);
                                const retriedEdge = this.createEdgeFromOrder(workflow, lastNonTerminalNodeId, terminalNode.id, executionOrder, undefined, workingEdges);
                                if (retriedEdge) {
                                    edgesToAdd.push(retriedEdge);
                                    workingEdges.push(retriedEdge);
                                    usedTerminalPredecessors.add(lastNonTerminalNodeId);
                                    console.log(`[EdgeReconciliationEngine] ✅ Rewired terminal edge: ${lastNonTerminalNodeId.substring(0, 8)} → ${terminalNode.id.substring(0, 8)}`);
                                    continue;
                                }
                            }
                            ...
                            const staleCrossBranchSpine = workingEdges.find((e) => {
                                ...
                                if (!tNode)
                                    return false;
                                if (this.getNodeType(tNode) === 'log_output')
                                    return false;
                                return this.areConsecutivePairExclusiveBranchHeadsByOrder(workflow.nodes, orderedNodeIds, lastNonTerminalNodeId, e.target, structuralEdgesSnapshot);
                            });
                            if (staleCrossBranchSpine) {
                                ...
                                if (spineIdx >= 0)
                                    workingEdges.splice(spineIdx, 1);
                                console.warn(`[EdgeReconciliationEngine] ⚠️  Removed erroneous cross-branch spine ${lastNonTerminalNodeId.substring(0, 8)} → ` +
                                    `${staleCrossBranchSpine.target.substring(0, 8)} (sibling branch heads cannot linearly chain)`);
                                const retriedSpine = this.createEdgeFromOrder(workflow, lastNonTerminalNodeId, terminalNode.id, executionOrder, undefined, workingEdges);
                                if (retriedSpine) {
                                    edgesToAdd.push(retriedSpine);
                                    workingEdges.push(retriedSpine);
                                    usedTerminalPredecessors.add(lastNonTerminalNodeId);
                                    console.log(`[EdgeReconciliationEngine] ✅ Rewired after spine removal: ${lastNonTerminalNodeId.substring(0, 8)} → ` +
                                        `${terminalNode.id.substring(0, 8)}`);
                                    continue;
                                }
                            }
                            const sourceNodeDef = unified_node_registry_1.unifiedNodeRegistry.get(sourceType);
                            const terminalNodeDef = unified_node_registry_1.unifiedNodeRegistry.get(terminalType);
                            console.error(`[EdgeReconciliationEngine] ❌ FAILED to create edge: ` +
                                `${sourceType}(${lastNonTerminalNodeId.substring(0, 8)}) → ${terminalType}(${terminalNode.id.substring(0, 8)})\n` +
                                `  Source node: type=${sourceType}, outgoingPorts=[${(sourceNodeDef?.outgoingPorts || []).join(', ')}], ` +
                                `incomingPorts=[${(sourceNodeDef?.incomingPorts || []).join(', ')}]\n` +
                                `  Target node: type=${terminalType}, outgoingPorts=[${(terminalNodeDef?.outgoingPorts || []).join(', ')}], ` +
                                `incomingPorts=[${(terminalNodeDef?.incomingPorts || []).join(', ')}]`);
                            warnings.push(`Could not create edge from ${lastNonTerminalNodeId} to ${terminalType} (${terminalNode.id}) - handle resolution failed`);
                        }
                    }
                }
                else {
                ...
                console.warn(`[EdgeReconciliationEngine] ⚠️  No node found to connect to terminal(${terminalNode.id.substring(0, 8)}) - ` +
                    `terminalIndex=${terminalIndex}, orderedNodeIds.length=${orderedNodeIds.length}, lineageAware=true`);
            }
        }
        // Combine all edges
        ...
        if (splitResult.splitCount > 0) {
            warnings.push(`Split ${splitResult.splitCount} multi-input log_output node(s) into separate terminals (one edge each). Planner/generator should emit one log_output per branch path to avoid fallback splitting.`);
        }
        ...
        const hasBranchingNode = workingNodes.some((n) => {
            const def = unified_node_registry_1.unifiedNodeRegistry.get(this.getNodeType(n));
        ...
        if (hasBranchingNode) {
            ...
            const logNodesSorted = workingNodes
                ...
                .sort((a, b) => orderedNodeIdsForRepair.indexOf(a.id) - orderedNodeIdsForRepair.indexOf(b.id));
            for (const logNode of logNodesSorted) {
                ...
                if (logIdx < 0)
                    continue;
                const predecessorId = this.pickBranchAwarePredecessorForLogOutput({ ...workflow, nodes: workingNodes, edges: finalEdges }, orderedNodeIdsForRepair, finalEdges, logNode.id, logIdx);
                if (!predecessorId)
                    continue;
                ...
                const predecessorDef = unified_node_registry_1.unifiedNodeRegistry.get(predecessorType);
                if (predecessorDef?.isBranching !== true) {
                    const alreadyTargetsAnotherLog = finalEdges.some((e) => {
                        if (e.source !== predecessorId)
                            return false;
                        const t = nodeById.get(e.target);
                        return !!t && this.getNodeType(t) === 'log_output';
                    });
                    if (alreadyTargetsAnotherLog)
                        continue;
                }
                const repairedEdge = this.createEdgeFromOrder({ ...workflow, nodes: workingNodes, edges: finalEdges }, predecessorId, logNode.id, executionOrder, undefined, finalEdges);
                if (repairedEdge) {
                    finalEdges.push(repairedEdge);
                    warnings.push(`Repaired orphaned branch terminal log_output ${logNode.id} by reconnecting from ${predecessorId}.`);
                    console.log(`[EdgeReconciliationEngine] ✅ Repaired orphaned log_output ${logNode.id.substring(0, 8)} from ${predecessorId.substring(0, 8)}`);
                }
            }
        }
        ...
        const { nodes: finalNodes, nodesRemoved, removedNodeTypes } = this.removeUnrequiredOrphanedNodes({ ...workflow, nodes: workingNodes }, finalEdges, orderedNodeIds, tagsFromVariation);
        // Remove edges connected to removed nodes
        const nodeIdsSet = new Set(finalNodes.map(n => n.id));
        const finalEdgesFiltered = finalEdges.filter((e) => nodeIdsSet.has(e.source) && nodeIdsSet.has(e.target));
        if (nodesRemoved > 0) {
            warnings.push(`Auto-removed ${nodesRemoved} orphaned node(s) that were not required (not connected to workflow): ${removedNodeTypes.join(', ')}`);
        }
        return {
            workflow: {
                ...workflow,
    ...
    cloneLogOutputForSplit(sourceLog, splitIndex) {
        ...
        const baseLabel = (sourceLog.data && sourceLog.data.label) ||
            sourceLog.label ||
            'Log';
     ...
     */
    splitMultiInputLogOutputs(nodes, edges) {
        const outNodes = [...nodes];
        let outEdges = [...edges];
        let splitCount = 0;
        // Find log_output nodes that need splitting when they have multiple inputs
        const logNodes = outNodes.filter(n => {
            const t = this.getNodeType(n);
            // Always split log_output nodes when they have multiple inputs
            return t === 'log_output';
        });
        for (const log of logNodes) {
            ...
            if (incoming.length <= 1)
                continue;
            splitCount++;
            const sorted = [...incoming].sort((a, b) => this.branchEdgeIncomingPriority(a) - this.branchEdgeIncomingPriority(b));
            for (let i = 1; i < sorted.length; i++) {
                const edgeToRewire = sorted[i];
                const newLog = this.cloneLogOutputForSplit(log, i);
                outNodes.push(newLog);
                const idx = outEdges.findIndex(e => edgeToRewire.id ? e.id === edgeToRewire.id
     ...
     */
    removeUnrequiredOrphanedNodes(workflow, edges, orderedNodeIds, tagsFromVariation) {
        // Find trigger nodes
        ...
        const nodesToKeep = [];
        const nodesToRemove = [];
        const removedNodeTypes = []; // ✅ Track removed node types
        for (const node of workflow.nodes) {
            ...
            if (this.shouldPreserveNode(node, tagsFromVariation)) {
                nodesToKeep.push(node);
                console.log(`[EdgeReconciliationEngine] ✅ Preserving orphaned node from tags: ${nodeType} (${node.id})`);
                continue;
            }
            // ✅ TASK 17.1: Remove alwaysRequired and exemptFromRemoval flag checks
            // Use intent-driven preservation instead
            // Only preserve trigger nodes (always required)
            ...
            const isSolePredecessorOfRequiredTerminal = edges.some((e) => {
                ...
                // ✅ TASK 17.1: Only check if target is a trigger (always required)
                const targetRequired = targetDef?.category === 'trigger';
                if (!targetRequired)
                    return false;
            ...
            if (isRequired || isSolePredecessorOfRequiredTerminal) {
                ...
                nodesToKeep.push(node);
                console.log(`[EdgeReconciliationEngine] ⚠️  Keeping required orphaned node: ${nodeType} (${node.id}) - edge creation may have failed`);
            }
            else {
                // Not required and orphaned = unnecessary = remove
                nodesToRemove.push(node);
                removedNodeTypes.push(nodeType); // ✅ Track removed node type
                console.log(`[EdgeReconciliationEngine] 🗑️  Auto-removing unnecessary orphaned node: ${nodeType} (${node.id})`);
            }
        }
        return {
            nodes: nodesToKeep,
     ...
     */
    shouldPreserveNode(node, tagsFromVariation) {
     ...
     */
    reachableNodeIdsFrom(edges, startId) {
     ...
     */
    pickBranchAwarePredecessorForLogOutput(workflow, orderedNodeIds, structuralEdges, logOutputNodeId, logOutputIndex, options) {
        const nodeById = new Map(workflow.nodes.map(n => [n.id, n]));
        const isLogType = (id) => (nodeById.get(id) ? this.getNodeType(nodeById.get(id)) : '') ===
            'log_output';
        ...
        for (let i = logOutputIndex - 1; i >= 0; i--) {
            ...
            const def = unified_node_registry_1.unifiedNodeRegistry.get(nt);
            if (!def)
                continue;
        ...
        const usedPredecessorIds = options?.usedPredecessorIds || new Set();
        const isEligibleCandidate = (id) => {
            const n = nodeById.get(id);
            if (!n)
                return false;
            const nt = this.getNodeType(n);
        ...
        const isEligibleNonForkCandidate = (id) => {
            const n = nodeById.get(id);
            if (!n)
                return false;
            const nt = this.getNodeType(n);
            const def = unified_node_registry_1.unifiedNodeRegistry.get(nt);
            if (!def || def.isBranching === true)
                return false;
            return isEligibleCandidate(id);
        };
        const isPreferredTerminalPredecessor = (id) => {
            const n = nodeById.get(id);
            if (!n)
                return false;
            const nt = this.getNodeType(n);
            if (nt === 'merge')
                return true;
            return node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nt) || (0, universal_node_type_checker_1.isOutputNode)(nt);
        };
        const pickCandidateInRange = (minIdxInclusive, maxIdxExclusive) => {
        ...
        for (let k = 0; k < candidatesBefore.length; k++) {
            ...
            if (isEligibleNonForkCandidate(id) &&
                !this.areExclusiveForkDescendantsInDifferentRegions(workflow.nodes, structuralEdges, id, logOutputNodeId) &&
        ...
        for (const brNode of workflow.nodes) {
            ...
            if (!unified_node_registry_1.unifiedNodeRegistry.get(bt)?.isBranching)
                continue;
            const forkId = brNode.id;
            const outs = structuralEdges.filter(e => this.getExclusiveBranchPortFromEdge(e, forkId) !== null);
            if (outs.length < 2)
                continue;
            const byPort = (p) => outs.find(e => String(e.type || e.sourceHandle || '').toLowerCase() === p);
            ...
            if (trueE && falseE) {
                ...
                for (let k = 0; k < candidatesBefore.length; k++) {
                    ...
                    if (ti < fi) {
                        if (logIdx >= fi) {
                            if (idx >= fi && idx < logIdx && isEligibleCandidate(id))
                                return id;
                        }
                        else if (idx >= ti && idx < fi) {
                            if (isEligibleCandidate(id))
                                return id;
                        }
                    }
                    else {
                        if (logIdx >= ti) {
                            if (idx >= ti && idx < logIdx && isEligibleCandidate(id))
                                return id;
                        }
            ...
            if (logOrdinal >= 0 && logOrdinal < caseHeads.length) {
                ...
                if (inCaseExclusiveRegion)
                    return inCaseExclusiveRegion;
                ...
                if (inCaseRegionToLog)
                    return inCaseRegionToLog;
            }
            ...
            for (let k = 0; k < candidatesBefore.length; k++) {
                const { id, idx } = candidatesBefore[k];
                if (idx >= regionStart && idx < logIdx && isEligibleCandidate(id))
                    return id;
            }
        }
        ...
        for (let i = logOutputIndex - 1; i >= 0; i--) {
            ...
            const candidateDef = unified_node_registry_1.unifiedNodeRegistry.get(ct);
            if (candidateDef?.category === 'trigger')
                continue;
            if (candidateDef?.isBranching === true)
                continue;
            if (candidateDef?.workflowBehavior?.alwaysTerminal === true)
                continue;
            if (isEligibleCandidate(id))
                return id;
        }
        if (logOutputIndex > 0) {
            ...
            if (isEligibleCandidate(fallbackId))
                return fallbackId;
        }
        return null;
    }
     ...
     */
    getExclusiveBranchPortFromEdge(edge, forkId) {
        if (edge.source !== forkId)
            return null;
        const t = String(edge.type || edge.sourceHandle || '').toLowerCase();
        if (!t || t === 'main' || t === 'default' || t === 'output')
            return null;
        return t;
    }
    /**
     ...
     */
    areExclusiveForkDescendantsInDifferentRegions(nodes, edges, u, v) {
        if (u === v)
            return false;
        for (const node of nodes) {
            ...
            for (let i = 0; i < outs.length; i++) {
                for (let j = i + 1; j < outs.length; j++) {
                    ...
                    if (uInR1Only && vInR2Only)
                        return true;
                    if (uInR2Only && vInR1Only)
                        return true;
                }
            }
        }
     ...
     */
    getHypotheticalBranchPortTargetsForFork(forkNode, orderedNodeIds) {
        const forkIndex = orderedNodeIds.indexOf(forkNode.id);
        if (forkIndex < 0)
            return null;
        const potentialTargets = orderedNodeIds.slice(forkIndex + 1);
        const outgoingPorts = unified_node_registry_1.unifiedNodeRegistry.getOutgoingPortsForWorkflowNode(forkNode);
        const branchPorts = outgoingPorts.filter(p => p !== 'output');
        if (branchPorts.length < 2)
            return null;
        const map = new Map();
     ...
     */
    getBranchHeadTargetsFromForkEdges(forkNode, edges) {
        const forkType = this.getNodeType(forkNode);
        if (!unified_node_registry_1.unifiedNodeRegistry.get(forkType)?.isBranching)
            return null;
        const outs = edges.filter(e => {
            if (e.source !== forkNode.id)
                return false;
     ...
     */
    areConsecutivePairExclusiveBranchHeadsByOrder(workflowNodes, orderedNodeIds, sourceId, targetId, structuralEdges = []) {
        ...
        for (const node of workflowNodes) {
            const nodeType = this.getNodeType(node);
            if (!unified_node_registry_1.unifiedNodeRegistry.get(nodeType)?.isBranching)
                continue;
            let targets = null;
     ...
     */
    shouldKeepEdgeDespiteNonMonotonicOrder(workflow, edge) {
        ...
        if (unified_node_registry_1.unifiedNodeRegistry.get(sourceType)?.isBranching === true && this.edgeUsesBranchPort(edge)) {
            return true;
        }
        const targetType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(targetNode.type || targetNode.data?.type || '');
        if (targetType === 'log_output' && (0, universal_node_type_checker_1.isOutputNode)(sourceNode)) {
            return true;
        }
        return false;
    }
     ...
     */
    validateEdges(workflow, executionOrder) {
        const orderedNodeIds = execution_order_manager_1.executionOrderManager.getOrderedNodeIds(executionOrder);
        ...
        workflow.edges.forEach(edge => {
            ...
            if (sourceNode) {
                ...
                if (sourceDef?.workflowBehavior?.alwaysTerminal === true) {
                    ...
                    violations.push(`Edge ${edge.source} → ${edge.target} is invalid: source node ${sourceType} is terminal`);
                    return;
                }
                if ((sourceDef?.tags || []).includes('terminal')) {
                    edgesToRemove.push(edge);
                    violations.push(`Edge ${edge.source} → ${edge.target} is invalid: source node ${sourceType} is tagged terminal`);
                    return;
                }
                if ((sourceDef?.outgoingPorts || []).length === 0 && sourceDef?.isBranching !== true) {
                    edgesToRemove.push(edge);
                    violations.push(`Edge ${edge.source} → ${edge.target} is invalid: source node ${sourceType} has no outgoing ports`);
                    return;
                }
            }
            ...
            if (sourceIdx >= targetIdx) {
                ...
                const isBranchPortEdge = branchEdgeLabel === 'true' ||
                ...
                if (isBranchPortEdge) {
                    ...
                    const sourceIsBranching = sourceNodeForBranch
                        ? unified_node_registry_1.unifiedNodeRegistry.get(this.getNodeType(sourceNodeForBranch))?.isBranching === true
                        : false;
                ...
                edgesToRemove.push(edge);
                violations.push(`Edge ${edge.source} → ${edge.target} violates execution order ` +
                    `(source at index ${sourceIdx}, target at index ${targetIdx})`);
                return;
            }
            // ✅ REGISTRY-DRIVEN: Skip edge validation for terminal nodes
            // Only validate skip edges for terminal nodes that DON'T allow multiple inputs.
            // Nodes with allowsMultipleInputs: true (e.g., log_output with merge capability)
            // can receive edges from multiple sources, so skip edge validation doesn't apply.
            const targetNode = workflow.nodes.find(n => n.id === edge.target);
            if (targetNode) {
                const targetType = this.getNodeType(targetNode);
                const targetDef = unified_node_registry_1.unifiedNodeRegistry.get(targetType);
                // Always enforce single-input constraint for log_output nodes
                if (targetType === 'log_output') {
                    // Legacy single-input log_output behavior: only allow direct predecessor
                    ...
                    if (directPredecessorId && edge.source !== directPredecessorId) {
                        ...
                        const sourceIsOutput = sourceNodeForLog ? (0, universal_node_type_checker_1.isOutputNode)(sourceNodeForLog) : false;
                        const branchAwarePredecessor = this.pickBranchAwarePredecessorForLogOutput(workflow, orderedNodeIds, workflow.edges, edge.target, targetIdx);
                        const isBranchAwareOutputToLog = sourceIsOutput &&
                            typeof branchAwarePredecessor === 'string' &&
                            branchAwarePredecessor === edge.source;
                        if (!isBranchEdge && !isBranchAwareOutputToLog) {
                            edgesToRemove.push(edge);
                            violations.push(`Edge ${edge.source} → ${edge.target} is a skip edge to log_output ` +
        ...
        const nonBranchingSourceToLogs = new Map();
        for (const edge of workflow.edges) {
            const targetNode = workflow.nodes.find(n => n.id === edge.target);
            if (!targetNode || this.getNodeType(targetNode) !== 'log_output')
                continue;
            const sourceNode = workflow.nodes.find(n => n.id === edge.source);
            if (!sourceNode)
                continue;
            const sourceType = this.getNodeType(sourceNode);
            const sourceDef = unified_node_registry_1.unifiedNodeRegistry.get(sourceType);
            if (!sourceDef || sourceDef.isBranching === true)
                continue;
            if (!nonBranchingSourceToLogs.has(edge.source)) {
                nonBranchingSourceToLogs.set(edge.source, new Set());
            }
            nonBranchingSourceToLogs.get(edge.source).add(edge.target);
        }
        for (const [sourceId, targets] of nonBranchingSourceToLogs.entries()) {
            if (targets.size > 1) {
                violations.push(`Terminal lineage violation: non-branching node ${sourceId} connects to multiple log_output terminals (${Array.from(targets).join(', ')})`);
            }
        }
        ...
        if (hasBranchingNode) {
            // Check terminal nodes - log_output nodes must always have exactly one incoming edge
            const terminalNodes = workflow.nodes.filter((n) => {
                const nodeType = this.getNodeType(n);
                const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
                // Always enforce single-input constraint for log_output nodes
                return def?.isTerminal === true || nodeType === 'log_output';
            });
            for (const terminalNode of terminalNodes) {
                const incomingCount = workflow.edges.filter((e) => e.target === terminalNode.id).length;
                if (incomingCount !== 1) {
                    violations.push(`Terminal lineage violation: branching workflow requires exactly one incoming edge for terminal node ${terminalNode.id} (found ${incomingCount})`);
                }
            }
        }
        const effectiveEdges = workflow.edges.filter(e => !edgesToRemove.some(er => er.id === e.id));
        ...
        for (let i = 0; i < orderedNodeIds.length - 1; i++) {
            ...
            if (!sourceNode || !targetNode)
                continue;
            if ((0, universal_node_type_checker_1.isOutputNode)(targetNode)) {
                const sourceType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(sourceNode.type || sourceNode.data?.type || '');
                const sourceDef = unified_node_registry_1.unifiedNodeRegistry.get(sourceType);
                ...
                if (!allowTriggerToOutput)
                    continue;
            }
            const validateSourceType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(sourceNode.type || sourceNode.data?.type || '');
            // Same rule as reconcileEdges Step 3: branching sources are wired via branch fan-out, not linear i→i+1.
            if (unified_node_registry_1.unifiedNodeRegistry.get(validateSourceType)?.isBranching === true) {
                continue;
            }
            const validateTargetType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(targetNode.type || targetNode.data?.type || '');
            const validateTargetDef = unified_node_registry_1.unifiedNodeRegistry.get(validateTargetType);
            if (validateTargetDef?.workflowBehavior?.alwaysTerminal === true ||
                (validateTargetDef?.tags || []).includes('terminal')) {
                continue;
            }
            if (this.areExclusiveForkDescendantsInDifferentRegions(workflow.nodes, effectiveEdges, sourceId, targetId) ||
                this.areConsecutivePairExclusiveBranchHeadsByOrder(workflow.nodes, orderedNodeIds, sourceId, targetId, effectiveEdges)) {
                continue;
            }
            // Check if edge should exist (registry-based)
            if (this.shouldHaveEdge(sourceNode, targetNode)) {
                const edgeExists = effectiveEdges.some(e => e.source === sourceId && e.target === targetId);
                if (!edgeExists) {
     ...
     */
    createEdgeFromOrder(workflow, sourceId, targetId, executionOrder, edgeType, existingEdges) {
        const sourceNode = workflow.nodes.find(n => n.id === sourceId);
        const targetNode = workflow.nodes.find(n => n.id === targetId);
        if (!sourceNode || !targetNode) {
            console.error(`[EdgeReconciliationEngine] 🔍 DEBUG createEdgeFromOrder: Node not found - ` +
        ...
        if (isLogOutputConnection) {
            console.log(`[EdgeReconciliationEngine] 🔍 DEBUG createEdgeFromOrder: ` +
                `${sourceType}(${sourceId.substring(0, 8)}) → log_output(${targetId.substring(0, 8)}), ` +
                `edgeType=${edgeType || 'default'}`);
        }
        // Use universal handle resolver (registry-driven)
        const sourceHandleResult = error_prevention_1.universalHandleResolver.resolveSourceHandle(sourceType, undefined, // No explicit handle
        edgeType // Connection type ('true', 'false', etc.)
        );
        const targetHandleResult = error_prevention_1.universalHandleResolver.resolveTargetHandle(targetType);
        if (isLogOutputConnection) {
            console.log(`[EdgeReconciliationEngine] 🔍 DEBUG Handle resolution: ` +
        ...
        if (!sourceHandleResult.valid || !targetHandleResult.valid) {
            if (isLogOutputConnection) {
                console.error(`[EdgeReconciliationEngine] ❌ Handle resolution failed: ` +
        ...
        if (isLogOutputConnection) {
            console.log(`[EdgeReconciliationEngine] 🔍 DEBUG Calling universalEdgeCreationService.createEdge: ` +
                `sourceHandle=${sourceHandleResult.handle}, targetHandle=${targetHandleResult.handle}`);
        }
        const edgeResult = universal_edge_creation_service_1.universalEdgeCreationService.createEdge({
            sourceNode,
            targetNode,
        ...
        if (isLogOutputConnection) {
            console.log(`[EdgeReconciliationEngine] 🔍 DEBUG Edge creation result: ` +
        ...
        if (edgeResult.success && edgeResult.edge) {
            if (isLogOutputConnection) {
                console.log(`[EdgeReconciliationEngine] ✅ Successfully created edge: ` +
                    `${sourceType}(${sourceId.substring(0, 8)}) → log_output(${targetId.substring(0, 8)})`);
            }
            return edgeResult.edge;
        }
        if (isLogOutputConnection) {
            console.error(`[EdgeReconciliationEngine] ❌ Edge creation failed: ` +
                `success=${edgeResult.success}, ` +
    ...
    shouldHaveEdge(source, target) {
        ...
        const sourceIsDataSource = sourceIntendedCapability === 'data_source' ||
            (sourceIntendedCapability === undefined && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(sourceType));
        const sourceIsTransformation = sourceIntendedCapability === 'transformation' ||
            (sourceIntendedCapability === undefined && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(sourceType));
        const sourceIsOutput = sourceIntendedCapability === 'output' ||
            (sourceIntendedCapability === undefined && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(sourceType));
        const targetIsDataSource = targetIntendedCapability === 'data_source' ||
            (targetIntendedCapability === undefined && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(targetType));
        const targetIsTransformation = targetIntendedCapability === 'transformation' ||
            (targetIntendedCapability === undefined && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(targetType));
        const targetIsOutput = targetIntendedCapability === 'output' ||
            (targetIntendedCapability === undefined && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(targetType));
        ...
        if (sourceIsTrigger && targetDef.category !== 'trigger')
            return true;
        ...
        if (sourceIsTransformation) {
            if (targetIsTransformation || targetIsOutput)
                return true;
            if (targetType === 'log_output')
                return true;
        }
        ...
        if (sourceDef.category === 'logic')
            return true;
        // ✅ RULE 7: Outputs can connect to log_output (universal terminal)
        if (sourceIsOutput && targetType === 'log_output')
            return true;
        ...
        if ((sourceCategory === 'transformation' || sourceCategory === 'ai') &&
            (targetCategory === 'transformation' || targetCategory === 'ai' || targetCategory === 'communication'))
            return true;
        if (sourceCategory === 'utility' && (targetCategory === 'transformation' || targetCategory === 'ai' || targetCategory === 'communication'))
            return true;
        return false;
    }
}
// Export singleton instance
exports.edgeReconciliationEngine = new EdgeReconciliationEngineImpl();
