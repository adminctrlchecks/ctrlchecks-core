...
exports.WorkflowAnalyzer = void 0;
/**
...
class WorkflowAnalyzer {
     ...
     */
    calculateComplexity(workflowDefinition) {
        ...
        const branchingFactor = nodeOutgoingEdges.size > 0
            ? Array.from(nodeOutgoingEdges.values()).reduce((a, b) => a + b, 0) /
                nodeOutgoingEdges.size
            : 0;
        // Complexity score (weighted combination)
        const score = nodeCount * 0.3 + depth * 0.3 + branchingFactor * 10 + edgeCount * 0.2;
        return {
     ...
     */
    extractDependencies(workflowDefinition) {
        const nodes = workflowDefinition.nodes || [];
     ...
     */
    estimateResources(workflowDefinition) {
        const nodes = workflowDefinition.nodes || [];
        const nodeCount = nodes.length;
        // Base estimates
     ...
     */
    identifyIssues(workflowDefinition) {
        const issues = [];
        const nodes = workflowDefinition.nodes || [];
        const edges = workflowDefinition.edges || [];
        ...
        const hasErrorHandler = nodes.some((node) => (node.type || node.data?.type) === 'error_handler' ||
     ...
     */
    suggestOptimizations(workflowDefinition) {
        const suggestions = [];
        const nodes = workflowDefinition.nodes || [];
        ...
        const hasLoop = nodes.some((node) => (node.type || node.data?.type) === 'loop');
        const hasHttp = nodes.some((node) => (node.type || node.data?.type)?.includes('http') ||
            (node.type || node.data?.type) === 'http_request');
        if (hasLoop && hasHttp) {
            suggestions.push({
                type: 'optimization',
                message: 'HTTP requests in loops can be slow. Consider using batch operations or parallel execution.',
                priority: 'high',
            });
        }
        return suggestions;
