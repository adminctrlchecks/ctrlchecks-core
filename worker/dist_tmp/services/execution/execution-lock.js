 ...
 */
async function isExecutionStale(db, executionId) {
    try {
 ...
 */
async function releaseExecutionLock(db, workflowId, executionId) {
    try {
 ...
 */
async function cleanupStaleLock(db, workflowId, executionId) {
    try {
        ...
        // Log cleanup event
        try {
            const { logExecutionEvent } = await Promise.resolve().then(() => __importStar(require('./execution-event-logger')));
 ...
 */
async function acquireExecutionLock(db, workflowId, executionId) {
    try {
 ...
 */
async function getActiveExecution(db, workflowId) {
    try {
        const { data, error } = await db
            .from('workflows')
            .select('active_execution_id')
