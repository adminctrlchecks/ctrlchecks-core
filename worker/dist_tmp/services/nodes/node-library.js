...
exports.CANONICAL_NODE_TYPES = exports.nodeLibrary = exports.NodeLibrary = void 0;
exports.isValidCanonicalNodeType = isValidCanonicalNodeType;
...
class NodeLibrary {
     ...
     */
    validateAllNodesHaveContext() {
        const nodesMissingContext = [];
        ...
        if (nodesMissingContext.length > 0) {
            const error = new Error(`[NodeLibrary] ❌ ROOT-LEVEL ERROR: ${nodesMissingContext.length} node(s) missing required context:\n` +
     ...
     */
    extractBaseNodeNameFromCompound(compoundName) {
        ...
        try {
            ...
            for (const nodeType of allNodeTypes) {
                ...
                if (schema) {
                    ...
                    const patterns = (schema.commonPatterns || []).map((p) => p.name.toLowerCase());
     ...
     */
    validateInputs(nodeType, config) {
        const schema = this.getSchema(nodeType);
        if (!schema) {
            return {
                valid: false,
     ...
     * @returns Validation result
     */
    validateCredentials(nodeType, config) {
        const schema = this.getSchema(nodeType);
        if (!schema) {
            return {
                valid: false,
     ...
     */
    findNodesByKeywords(keywords) {
        const lowerKeywords = keywords.map(k => k.toLowerCase());
        return Array.from(this.schemas.values()).filter(schema => {
            return lowerKeywords.some(keyword => schema.aiSelectionCriteria.keywords.some(k => k.toLowerCase().includes(keyword)) ||
    ...
    addSchema(schema) {
        ...
        if (!schema.outputType) {
            schema.outputType = (0, node_output_types_1.getNodeOutputType)(schema.type);
        }
        if (!schema.outputSchema) {
            schema.outputSchema = (0, node_output_types_1.getNodeOutputSchema)(schema.type);
        }
                        ...
                        contextHints: [
                            {
                                ...
                                message: 'Enter recipients in Recipient emails (comma-separated). Recipient source is only the mode selector—not where you type email addresses.',
                            },
                            {
                                whenValue: 'extract_from_sheet',
                                message: 'Precedence: (1) emails from upstream nodes (prefer a Google Sheets node with spreadsheet + range there). (2) If none found, optional inline Spreadsheet ID + sheet/range below fetches rows via Sheets API. (3) Connect Google with Gmail + Sheets scopes.',
                            },
                        ],
                    },
                    recipientEmails: {
    ...
    createTelegramSchema() {
        return {
            ...
            validationRules: [
                ...
                {
                    field: 'messageType',
                    validator: (value) => ['text', 'photo', 'video', 'document', 'audio', 'animation', 'location', 'poll'].includes(value),
                    errorMessage: 'Telegram messageType must be one of: text, photo, video, document, audio, animation, location, poll',
                },
            ],
        ...
        return {
            ...
            configSchema: {
                ...
                optional: {
                    ...
                    operation: {
                        type: 'string',
                        description: 'Salesforce operation: query (SOQL), search (SOSL), get, create, update, delete, upsert, bulk*',
                        examples: [
                            'query',
                            'search',
                            'get',
            ...
            validationRules: [
                {
                    ...
                    errorMessage: 'Salesforce operation must be one of: query, search, get, create, update, delete, upsert, bulkCreate, bulkUpdate, bulkDelete, bulkUpsert',
                },
                {
                    field: 'resource',
                    validator: (value) => typeof value === 'string' && value.length > 0,
    ...
    createMicrosoftDynamicsSchema() {
        return {
            ...
            validationRules: [
                {
                    field: 'operation',
                    validator: (value) => ['getRecords', 'getRecord', 'createRecord', 'updateRecord', 'deleteRecord', 'fetchXml', 'associate', 'disassociate'].includes(value),
            ...
            validationRules: [
                {
                    ...
                    errorMessage: 'Operation must be one of: create_task, get_tasks_list, get_tasks_space.',
                },
            ],
        };
    }
    createChatTriggerSchema() {
        return {
            type: 'chat_trigger',
            label: 'Chat Trigger',
            category: 'triggers',
    ...
    createTallySchema() {
        return {
            ...
            validationRules: [
                {
                    field: 'operation',
                    validator: (v) => ['get_ledger', 'get_voucher', 'create_voucher', 'get_stock_items', 'get_company_info'].includes(v),
    ...
    createFirebaseSchema() {
        return {
            ...
            validationRules: [
                ...
                {
                    field: 'operation',
                    validator: (value) => ['get', 'add', 'update', 'delete', 'query', 'realtime_get', 'realtime_set'].includes(value),
    ...
    createGoogleCloudStorageSchema() {
        return {
            ...
            validationRules: [
                ...
                {
                    field: 'operation',
                    validator: (value) => ['upload', 'download', 'delete', 'list'].includes(value),
    ...
    createOdooSchema() {
        return {
            ...
            aiSelectionCriteria: {
                ...
                intentDescription: 'Odoo ERP integration node that interacts with Odoo models via JSON-RPC. Supports reading, creating, updating, and deleting records as well as calling custom model methods. Used for Odoo ERP automation, CRM management, and business process integration.',
            ...
            validationRules: [
                {
                    field: 'operation',
                    validator: (value) => ['getRecords', 'createRecord', 'updateRecord', 'deleteRecord', 'executeMethod'].includes(value),
            ...
            validationRules: [
                {
                    ...
                    errorMessage: 'Operation is required. Choose one of: getSchedules, createAppointment, updateAppointment, deleteAppointment',
                },
            ],
            outputType: 'object',
            outputSchema: {
    ...
    createCalendlySchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['get_events', 'get_event_types', 'get_scheduled_events', 'get_user'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createChargebeeSchema() {
        return {
            type: 'chargebee',
            label: 'Chargebee',
            category: 'payment',
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['create_customer', 'create_subscription', 'get_customer', 'cancel_subscription'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createTypeformSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['get_responses', 'create_form', 'get_form'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createXeroSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['get_many', 'get_by_id', 'create', 'update'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createOracleDatabaseSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['select', 'insert', 'update', 'insert_or_update', 'delete', 'execute_sql'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createSqlServerSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['executeQuery', 'insert', 'update', 'delete', 'storedProcedure'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createTimescaleDBSchema() {
        return {
            type: 'timescaledb',
            label: 'TimescaleDB',
            category: 'database',
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['executeQuery', 'insert', 'update', 'delete', 'timeBucket', 'first', 'last'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createContentfulSchema() {
        return {
            type: 'contentful',
            label: 'Contentful',
            category: 'cms',
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['get_entries', 'get_entry', 'create_entry', 'update_entry', 'delete_entry'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createWordPressSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['create_post', 'get_posts', 'update_post', 'delete_post'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createZendeskSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['get_tickets', 'get_ticket', 'create_ticket', 'update_ticket', 'delete_ticket', 'get_users'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createNetlifySchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['list_sites', 'get_site', 'create_deploy', 'list_deploys', 'get_deploy'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createWorkdaySchema() {
    ...
    createPineconeSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['upsert', 'query', 'delete'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createQdrantSchema() {
    ...
    createCohereSchema() {
        return {
            ...
            validationRules: [
                { field: 'model', validator: (v) => ['command', 'command-light', 'command-r', 'command-r-plus'].includes(v), errorMessage: 'Invalid Cohere model' },
            ],
        };
    }
    createLangchainSchema() {
        return {
            ...
            validationRules: [
                { field: 'operation', validator: (v) => ['run_chain', 'run_agent'].includes(v), errorMessage: 'Invalid operation' },
            ],
        };
    }
    createLightricksSchema() {
        return {
            type: 'lightricks',
            ...
            validationRules: [
                { field: 'prompt', validator: (v) => typeof v === 'string' && v.trim().length > 0, errorMessage: 'Prompt is required' },
            ],
        };
    }
    createHuggingFaceSchema() {
        return {
            type: 'huggingface',
            label: 'Hugging Face',
            category: 'ai',
...
exports.nodeLibrary = new NodeLibrary();
/**
 ...
 */
exports.CANONICAL_NODE_TYPES = exports.nodeLibrary.getAllCanonicalTypes();
/**
 * Type guard for canonical node types
 */
function isValidCanonicalNodeType(nodeType) {
    return exports.CANONICAL_NODE_TYPES.includes(nodeType);
}
