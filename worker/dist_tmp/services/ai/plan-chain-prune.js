...
exports.stripPlanTokenToType = stripPlanTokenToType;
exports.explicitPlanIdSuffix = explicitPlanIdSuffix;
exports.formatPlanChainToken = formatPlanChainToken;
exports.formatPlanChainTokenWithBranchTag = formatPlanChainTokenWithBranchTag;
exports.extractBranchTag = extractBranchTag;
exports.pruneProposedPlanChain = pruneProposedPlanChain;
 ...
 */
function stripPlanTokenToType(raw) {
    ...
    return (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(head) || head;
}
/** Suffix starting at `#` or `@` when present (plan explicit node id). */
function explicitPlanIdSuffix(raw) {
    const s = String(raw || '').trim();
...
function formatPlanChainToken(raw, canonicalType) {
 ...
 */
function formatPlanChainTokenWithBranchTag(nodeType, branchTag) {
    if (branchTag)
        return `${nodeType}[${branchTag}]`;
    return nodeType;
}
/**
 ...
 */
function extractBranchTag(token) {
    const match = String(token || '').match(/\[(.+?)\]$/);
    return match ? match[1] : undefined;
}
function pruneProposedPlanChain(chain) {
    ...
    chain.forEach((rawItem, idx) => {
        ...
        if (!nt || !node_library_1.nodeLibrary.isNodeTypeRegistered(nt))
            return;
        const pastFork = firstFork >= 0 && idx > firstFork && nt !== 'log_output';
        ...
        if (!pastFork && prev === nt && nt !== 'log_output')
            return;
