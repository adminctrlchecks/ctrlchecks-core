...
exports.getDiscriminantFieldForUpstreamType = getDiscriminantFieldForUpstreamType;
exports.planSwitchCasesFromPrompt = planSwitchCasesFromPrompt;
 ...
 */
function getDiscriminantFieldForUpstreamType(upstreamNodeType, upstreamConfig) {
    const t = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(upstreamNodeType || '');
    ...
    if (t === 'form' && upstreamConfig) {
        const fields = Array.isArray(upstreamConfig.fields) ? upstreamConfig.fields : [];
 ...
 */
function getKnownNodeTypeStrings() {
...
function isValidConditionToken(token, knownNodeTypes) {
    ...
    if (tokenParts.length >= 3 && actionVerbs.includes(tokenParts[0]))
        return false;
    return true;
}
/**
...
function extractEnumeratedCasesFromPrompt(prompt) {
    ...
    const routingVerbPattern = /\b(?:route|classify|bucket|label)\b[^.]*?\b(?:as|into|by|to)\b\s*(?:\w+\s*[:\-]\s*)?([^.]+)/gi;
    // Pattern 2: "by <field>: X, Y, Z" or "based on <field>: X, Y, Z" or "depending on <field>: X, Y, Z"
    const byFieldPattern = /\b(?:by|based\s+on|depending\s+on)\s+\w+\s*[:\-]\s*([^.]+)/gi;
    // Pattern 3: "status/type/category: X, Y, Z"
    const fieldColonPattern = /\b(?:status|type|category)\s*[:\-]\s*([^.]+)/gi;
    ...
    const whenIfPattern = /\b(?:when|if)\s+\w+\s+(?:is|equals?)\s+([a-z0-9_]+)/gi;
    ...
    const casesListPattern = /\b(?:with\s+)?cases?\s+([a-z0-9][^.]+)/gi;
    const allPatterns = [
        routingVerbPattern,
        byFieldPattern,
        fieldColonPattern,
 ...
 */
function planSwitchCasesFromPrompt(originalPrompt, upstreamNodeType, intent, upstreamConfig) {
    ...
    const cases = [];
    const caseToLabel = (v) => v.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
