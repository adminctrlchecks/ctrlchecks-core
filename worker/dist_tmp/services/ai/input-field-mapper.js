...
exports.inputFieldMapper = exports.InputFieldMapper = void 0;
...
class InputFieldMapper {
     ...
     */
    mapInputField(fieldName, fieldType, targetNode, previousNode, allNodes, nodeIndex) {
        ...
        const match = this.findMatchingField(fieldName, fieldType, previousOutput, targetNodeType);
        if (match) {
            return {
                field: fieldName,
        ...
        for (let i = upstreamNodes.length - 1; i >= 0; i--) {
            ...
            const upstreamMatch = this.findMatchingField(fieldName, fieldType, upstreamOutput, targetNodeType);
            if (upstreamMatch) {
                return {
                    field: fieldName,
    ...
    inferFieldType(fieldName, nodeType) {
        ...
        if (fieldLower.includes('url') || fieldLower.includes('link'))
            return 'string';
        if (fieldLower.includes('timestamp') || fieldLower.includes('date') || fieldLower.includes('time'))
            return 'string';
        if (fieldLower.includes('count') || fieldLower.includes('number') || fieldLower.includes('amount'))
            return 'number';
        if (fieldLower.includes('is') || fieldLower.includes('has') || fieldLower === 'active')
            return 'boolean';
        if (fieldLower.includes('items') || fieldLower.includes('rows') || fieldLower.includes('list'))
            return 'array';
        if (fieldLower.includes('data') || fieldLower.includes('body') || fieldLower.includes('content'))
            return 'object';
        // Default to string
        return 'string';
    }
     ...
     */
    generateSampleValue(fieldName, fieldType, nodeType) {
        const fieldLower = fieldName.toLowerCase();
        switch (fieldType) {
            case 'string':
                if (fieldLower.includes('email'))
                ...
                if (fieldLower.includes('title'))
                    return 'Sample Title';
                if (fieldLower.includes('body') || fieldLower.includes('content'))
     ...
     */
    findMatchingField(fieldName, fieldType, previousOutput, targetNodeType) {
        ...
        for (const [key, synonyms] of Object.entries(semanticMatches)) {
            if (fieldNameLower.includes(key)) {
                for (const synonym of synonyms) {
                    const match = previousOutput.outputFields.find(f => f.toLowerCase() === synonym.toLowerCase() ||
    ...
    getFieldType(fieldName, output) {
        ...
        if (fieldLower.includes('count') || fieldLower.includes('number'))
            return 'number';
        if (fieldLower.includes('is') || fieldLower.includes('has'))
            return 'boolean';
     ...
     */
    formatTemplateExpression(fieldName, sourceOutput) {
     ...
     */
    mapToTriggerInput(fieldName, fieldType, targetNodeType) {
        return {
            field: fieldName,
    ...
    validateTypeCompatibility(sourceType, targetType) {
        ...
        if (targetType === 'object' && sourceType === 'array')
            return true;
        // Array can accept single objects (wrapped)
        if (targetType === 'array' && sourceType === 'object')
            return true;
        // Email is a string subtype
        if (targetType === 'email' && sourceType === 'string')
            return true;
     ...
     */
    validateNodeInputs(node, previousNode, allNodes, nodeIndex) {
        const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(node);
        const nodeSchema = node_library_1.nodeLibrary.getSchema(nodeType);
        ...
        for (const fieldName of allFields) {
            ...
            // Map the field
            const mapping = this.mapInputField(fieldName, fieldType, node, previousNode, allNodes, nodeIndex);
            mappings.push(mapping);
            // Check if required field is valid
            if (requiredFields.includes(fieldName) && !mapping.valid) {
                errors.push(`Required field ${fieldName} has invalid mapping: ${mapping.validationErrors?.join(', ')}`);
            }
        }
        return {
...
exports.InputFieldMapper = InputFieldMapper;
// Export singleton instance
exports.inputFieldMapper = new InputFieldMapper();
