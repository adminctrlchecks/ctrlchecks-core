...
exports.workflowValidator = exports.WorkflowValidator = void 0;
const workflow_graph_normalizer_1 = require("../../core/utils/workflow-graph-normalizer");
...
class WorkflowValidator {
     ...
     */
    async validateAndFix(workflow, depth = 0, originalPrompt, userPrompt, graphNormalizeOptions) {
        ...
        if (!workflowValidation.valid) {
            return {
                valid: false,
                errors: workflowValidation.errors.map((err) => ({
                    type: 'invalid_configuration',
                    severity: 'critical',
                    message: err,
                    fixable: false,
                })),
                warnings: workflowValidation.warnings.map((warn) => ({
        ...
        const normalized = (0, workflow_graph_normalizer_1.normalizeWorkflowGraph)({
     ...
     */
    validateStructure(workflow, result) {
     ...
     */
    validateNodeProperties(node, workflow, index) {
        const issues = [];
     ...
     */
    fixTransformationNodes(workflow, issues) {
        const fixed = JSON.parse(JSON.stringify(workflow));
        const transformationIssues = issues.filter(issue => issue.nodeId && this.isTransformationNode(fixed.nodes.find((n) => n.id === issue.nodeId)?.type || ''));
     ...
     */
    fixTransformationNode(node, allNodes, index) {
        ...
        if (!config.outputFields || (Array.isArray(config.outputFields) && config.outputFields.length === 0)) {
            if (config.inputFields && Array.isArray(config.inputFields)) {
                config.outputFields = config.inputFields.map((field) => `transformed_${field}`);
            }
            else {
        ...
        if (!config.mappingRules || (Array.isArray(config.mappingRules) && config.mappingRules.length === 0)) {
            ...
            config.mappingRules = inputFields.map((inputField, i) => ({
     ...
     */
    validateBusinessLogic(workflow, result) {
        // Check for error handling on external API calls
        const apiNodes = workflow.nodes.filter(n => n.type === 'http_request' || n.type === 'http_post');
        if (apiNodes.length > 0) {
            const hasErrorHandling = workflow.nodes.some(n => n.type === 'error_handler' || n.type === 'error_trigger');
            if (!hasErrorHandling) {
        ...
        const hasUserInput = workflow.nodes.some(n => n.type === 'form' || n.type === 'webhook' || n.type === 'manual_trigger');
        if (hasUserInput) {
            const hasValidation = workflow.nodes.some(n => n.type === 'if_else' || n.type === 'javascript' || n.type === 'filter');
            if (!hasValidation) {
                result.warnings.push({
        ...
        // Check for logging
        const hasLogging = workflow.nodes.some(n => n.type === 'log_output' || n.type === 'database_write');
     ...
     * @deprecated Use proactive-error-prevention.ts instead
     */
    async attemptAutoFix(workflow, result) {
        ...
        switch (error.type) {
            case 'missing_trigger':
                return {
                    ...
                    changes: {
                        node: {
                            id: `trigger_${(0, crypto_1.randomUUID)()}`,
                            type: 'custom',
                            data: {
        ...
        switch (fix.type) {
            ...
            case 'fix_configuration':
                if (fix.nodeId && fix.changes.field) {
                    const node = fixed.nodes.find((n) => n.id === fix.nodeId);
                    if (node) {
                        if (!node.data.config) {
                            node.data.config = {};
                        }
            ...
            case 'remove_duplicate':
                if (fix.changes.nodesToRemove) {
                    fixed.nodes = fixed.nodes.filter((n) => !fix.changes.nodesToRemove.includes(n.id));
                    fixed.edges = fixed.edges.filter((e) => !fix.changes.nodesToRemove.includes(e.source) &&
    ...
    traverseConnections(nodeId, edges, visited) {
        edges
     ...
     */
    isNodeReachableViaDSLOrder(node, workflow, connectedNodeIds) {
        const { NodeMetadataHelper } = require('../../core/types/node-metadata');
        const metadata = NodeMetadataHelper.getMetadata(node);
        if (!metadata?.dsl?.dslId) {
     ...
     */
    validateTypeCompatibility(workflow, result) {
    ...
    hasField(node, field) {
        ...
        const configs = [
            node.data?.config || {},
            node.config || {}, // Some nodes might have config at root level
            node.data || {},
            node
        ];
     ...
     */
    hasDefaultValue(nodeType, field) {
        try {
            const def = node_definition_1.nodeDefinitionRegistry.get(nodeType);
            if (!def)
                return false;
    ...
    getDefaultValueForField(nodeType, field) {
        const defaults = {
            http_request: {
                method: 'GET',
                timeout: 10000,
     ...
     * semantics, not here.
     */
    validateExecutionOrder(_workflow, _result) {
     ...
     */
    validateDataFlow(workflow, result) {
     ...
     * Merged from comprehensive-workflow-validator
     */
    validateTypeCompatibilityEnhanced(workflow, result) {
    ...
    getOutputFieldType(nodeType, fieldName) {
        ...
        if (fieldName.includes('json') || fieldName.includes('data') || fieldName.includes('body'))
            return 'object';
        if (fieldName.includes('text') || fieldName.includes('message'))
            return 'string';
        if (fieldName.includes('status') || fieldName.includes('count'))
            return 'number';
     ...
     * Merged from comprehensive-workflow-validator
     */
    getInputFieldType(nodeType, fieldName) {
        // Use connection validator's input schema if available
        try {
            const { connectionValidator } = require('./connection-validator');
            const inputSchema = connectionValidator.getNodeInputSchema?.(nodeType);
        ...
        if (fieldName.includes('json') || fieldName.includes('data') || fieldName.includes('body'))
            return 'object';
        if (fieldName.includes('text') || fieldName.includes('message') || fieldName.includes('input'))
            return 'string';
     ...
     * Merged from comprehensive-workflow-validator
     */
    canReach(sourceId, targetId, edges, nodes) {
     ...
     */
    validateTransformations(workflow, originalPrompt, result) {
        try {
     ...
     * @param result - Validation result to populate
     */
    validateAIUsage(workflow, userPrompt, result) {
        ...
        const isChatbotWorkflow = promptLower.includes('chatbot') ||
        ...
        const needsAI = isChatbotWorkflow ||
     ...
     * @param result - Validation result to populate
     */
    validateRequiredServices(workflow, userPrompt, result) {
        const promptLower = userPrompt.toLowerCase();
     ...
     * @param result - Validation result to populate
     */
    async validateAIIntentMatching(workflow, userPrompt, result) {
        try {
            // Dynamically import to avoid circular dependencies
            const { aiWorkflowValidator } = await Promise.resolve().then(() => __importStar(require('./ai-workflow-validator')));
            ...
            const aiValidation = await aiWorkflowValidator.validateWorkflowStructure(userPrompt, structure, workflow.nodes, workflow.edges);
...
exports.workflowValidator = new WorkflowValidator();
