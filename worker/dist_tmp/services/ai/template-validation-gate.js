 ...
 */
function validateMapping(mapping, upstreamSchema, targetNodeType) {
    const reasons = [];
    ...
    const typeCompatibility = checkTypeCompatibility(mapping.sourceField, mapping.targetField, upstreamSchema, targetNodeType);
 ...
 */
function validateMappings(mappings, upstreamSchema, targetNodeType) {
    ...
    for (const mapping of mappings) {
        ...
        if (validation.ok) {
            ...
            if (validation.score >= CONFIDENCE_THRESHOLDS.LOW && validation.score < CONFIDENCE_THRESHOLDS.MEDIUM) {
                warnings.push(`Mapping "${mapping.targetField}" → "${mapping.sourceField}" has medium confidence ` +
 ...
 */
function checkTypeCompatibility(sourceField, targetField, upstreamSchema, targetNodeType) {
    const reasons = [];
    ...
    if (sourceType && targetType) {
        ...
        if (!compatible) {
            reasons.push(`Type mismatch: source field "${sourceField}" is ${sourceType}, ` +
