...
exports.aiWorkflowValidator = exports.AIWorkflowValidator = void 0;
...
class AIWorkflowValidator {
     ...
     */
    async validateWorkflowStructure(userPrompt, structure, nodes, edges) {
        try {
            ...
            const aiResponse = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-generation', {
     ...
     */
    prepareWorkflowSummary(structure, nodes, edges) {
        // ✅ Build structured graph JSON
        const graphData = {
            trigger: structure.trigger || null,
            triggerNodeId: nodes?.find(n => {
                const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(n) || n.type || '';
                return nodeType.includes('trigger') || nodeType === structure.trigger;
            })?.id || null,
            executionOrder: this.buildExecutionOrder(nodes || [], edges || []),
            nodes: (nodes || []).map(node => ({
                id: node.id,
                type: (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(node) || node.type || 'unknown',
                label: node.data?.label || node.id,
                isTrigger: this.isTriggerNode(node),
                operation: node.data?.config?.operation || node.data?.operation || null,
            })),
...
- Trigger Node ID: ${graphData.triggerNodeId || 'NOT FOUND'}`;
     ...
     */
    checkAllNodesReachable(nodes, edges) {
        const triggerNode = nodes.find(n => this.isTriggerNode(n));
        if (!triggerNode)
            return false;
        // Build adjacency list
     ...
     * Create AI validation prompt
     */
    createValidationPrompt(userPrompt, workflowSummary) {
...
Return the JSON now:`;
     ...
     */
    async validateNodeOrder(userPrompt, nodes) {
        const nodeOrder = nodes.map(n => {
            const type = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(n) || n.type || 'unknown';
            const operation = n.data?.config?.operation || n.data?.operation || '';
            return { type, operation, label: n.data?.label || n.id };
        });
...
Return JSON:
{
  ...
  "issues": ["issue1", "issue2"]
        ...
        let response = '';
        try {
            response = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-generation', {
                prompt,
                temperature: 0.1,
            });
            // Extract JSON from response (handle markdown code blocks and text responses)
...
exports.AIWorkflowValidator = AIWorkflowValidator;
exports.aiWorkflowValidator = new AIWorkflowValidator();
