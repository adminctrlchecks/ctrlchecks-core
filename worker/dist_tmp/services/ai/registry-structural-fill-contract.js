...
exports.buildRegistryStructuralFillContractSection = buildRegistryStructuralFillContractSection;
...
function effectiveFillDefault(field) {
 ...
 */
function buildRegistryStructuralFillContractSection(nodeTypes, options) {
    ...
    lines.push('- **manual_static**: user, vault/credential attach, or fixed config — not inferred by build-time AI unless user unlocks.');
    lines.push('- **buildtime_ai_once**: AI may generate a static value once during workflow build/configure; user can edit before run.');
    lines.push('- **runtime_ai**: executor resolves from upstream JSON + workflow intent when the node runs; may stay empty in saved graph until first execution.');
    lines.push('- **ownership=structural** (when shown): shape-defining (form `fields`, if_else `conditions`, switch `cases`, etc.) — required for structural validity; honor the field default fill mode.');
    lines.push('- **ownership=credential**: secrets / webhook URLs / OAuth — attach-credentials or vault; not AI-filled by default.');
    lines.push('');
    const seenType = new Set();
    for (const raw of nodeTypes) {
        const nt = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(String(raw || '').trim()) || String(raw || '').trim();
        if (!nt || seenType.has(nt))
            continue;
        seenType.add(nt);
        ...
        const parts = [];
        ['buildtime_ai_once', 'runtime_ai', 'manual_static'].forEach((mode) => {
