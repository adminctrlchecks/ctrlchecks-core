...
exports.credentialDiscoveryPhase = exports.CredentialDiscoveryPhase = void 0;
exports.mapDiscoveryResultToCredentialStatusResolution = mapDiscoveryResultToCredentialStatusResolution;
exports.mapDiscoveryMissingToWizardDiscoveredCredentials = mapDiscoveryMissingToWizardDiscoveredCredentials;
...
const credential_config_satisfaction_1 = require("./credential-config-satisfaction");
 ...
 */
class CredentialDiscoveryPhase {
    constructor() {
        this.credentialResolver = new credential_resolver_1.CredentialResolver(node_library_1.nodeLibrary);
    }
    /**
     ...
     */
    async discoverCredentials(workflowOrId, userId, supabaseClient) {
    ...
    async enrichWithAIGuidance(credentials) {
        ...
        const credList = credentials
            ...
            .join('\n');
        const systemPrompt = 'You are a workflow automation credential expert. ' +
        ...
        const raw = await gemini_orchestrator_1.geminiOrchestrator.processRequest('credential-guidance', { system: systemPrompt, message: userMessage }, { model: 'gemini-3.5-flash', temperature: 0.2, cache: false });
     ...
     * ✅ CRITICAL: Checks vault during discovery to mark satisfied credentials
     */
    async discoverNodeCredentials(nodeType, nodeId, node, userId) {
        const credentials = [];
        const registryDescriptor = unified_node_registry_1.unifiedNodeRegistry.getCredentialPreflightDescriptor(nodeType);
        ...
        if (contracts && contracts.length > 0) {
            for (const contract of contracts) {
                ...
                let satisfied = userId ? await this.credentialResolver.checkVaultForCredential(contract.vaultKey, contract.type, userId) : false;
                ...
                if (!satisfied && contract.type === 'oauth' && contract.provider === 'google') {
                    ...
                    satisfied = userId ? await this.credentialResolver.checkVaultForCredential('google', // Use 'google' as vaultKey for Google OAuth
                    'oauth', userId) : false;
                }
                ...
                if (!satisfied) {
                    const schema = node_library_1.nodeLibrary.getSchema(nodeType);
                    satisfied = (0, credential_config_satisfaction_1.isCredentialSatisfiedByNodeConfig)(node, {
                    ...
                    if (satisfied) {
                        ...
                        if (userId) {
                            ...
                            if (credentialId) {
                                const rows = await (0, db_pool_1.queryAsService)(`SELECT 1 FROM connections WHERE user_id = $1 AND id = $2 LIMIT 1`, [userId, credentialId]);
                                if (!rows.length) {
                                    satisfied = false;
     ...
     */
    async validateCredentialsAvailable(workflow, availableCredentials) {
...
exports.CredentialDiscoveryPhase = CredentialDiscoveryPhase;
/** Aligns discovery output with `buildCredentialStatuses` / wizard filtering (uses vaultKey as credentialId). */
function mapDiscoveryResultToCredentialStatusResolution(discovery) {
    const toRow = (c) => ({
...
function mapDiscoveryMissingToWizardDiscoveredCredentials(discovery) {
    return (discovery.missingCredentials || [])
        .filter((c) => {
        const isGoogleOAuth = (String(c.provider || '').toLowerCase() === 'google' && c.type === 'oauth') ||
        ...
        vaultKey: c.vaultKey,
        nodeIds: c.nodeIds || [],
        primaryFieldName: c.type === 'webhook' && String(c.provider || '').toLowerCase() === 'slack' ? 'webhookUrl' : undefined,
    }));
}
// Export singleton instance
exports.credentialDiscoveryPhase = new CredentialDiscoveryPhase();
