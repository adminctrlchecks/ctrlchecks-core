...
exports.intentConstraintEngine = exports.IntentConstraintEngine = void 0;
exports.getRequiredNodes = getRequiredNodes;
...
class IntentConstraintEngine {
     ...
     */
    getRequiredNodes(intent, originalPrompt, explicitNodeTypes) {
        ...
        if (intent.trigger) {
            ...
            if (triggerNode) {
                ...
                const schema = node_library_1.nodeLibrary.getSchema(triggerNode);
                if (schema) {
        ...
        if (intent.transformations && intent.transformations.length > 0) {
            for (const tf of intent.transformations) {
                ...
                if (tfTypeLower === 'loop') {
                    const promptRequestsLoop = originalPromptLower.includes('for each') ||
        ...
        if (originalPrompt) {
            const transformationDetection = (0, transformation_detector_1.detectTransformations)(originalPrompt);
            if (transformationDetection.detected) {
                ...
                for (const nodeType of transformationDetection.requiredNodeTypes) {
                    ...
                    if (!schema) {
                        ...
                        const normalizedSchema = node_library_1.nodeLibrary.getSchema(normalized);
                        if (normalizedSchema) {
        ...
        if (requiredNodes.has('ai_agent')) {
            ...
            const isSimpleOperation = promptLower.includes('summarize') ||
            ...
            const needsToolsOrMemory = promptLower.includes('tool') ||
        ...
        if (requiredNodes.has('ai_agent') && requiredNodes.has('ai_chat_model')) {
            ...
            const bothAreAIOperations = (aiAgentConstraint?.reason?.toLowerCase().includes('summar') ||
    ...
    mapActionToNodeTypes(action) {
        ...
        const emailAliasResolved = unified_node_registry_1.unifiedNodeRegistry.resolveAlias(actionType);
        ...
        if (actionType === 'salesforce_crm' || actionType === 'salesforce crm' || actionType.includes('salesforce_crm')) {
            ...
            const salesforceSchema = node_library_1.nodeLibrary.getSchema('salesforce');
            if (salesforceSchema) {
        ...
        if (capability_resolver_1.capabilityResolver.isCapability(actionType)) {
            ...
            const resolution = capability_resolver_1.capabilityResolver.resolveCapability(actionType);
            if (resolution) {
        ...
        const normalizedType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(actionType);
        ...
        const directSchema = node_library_1.nodeLibrary.getSchema(actionType);
        if (directSchema) {
     ...
     */
    mapActionPatternToNodeTypes(actionType, operation) {
        ...
        const keywordMatches = node_library_1.nodeLibrary.findNodesByKeywords([actionType]);
     ...
     */
    mapConditionToNodeType(conditionType, originalPrompt) {
     ...
     * - No repair nodes unless failure handling required
     */
    filterUnnecessaryNodes(nodeTypes, intent, originalPromptLower) {
        const filtered = [];
     ...
     */
    normalizeSemanticEquivalences(nodeTypes, intent, explicitNodeTypes) {
        ...
        for (const nodeType of nodeTypes) {
            ...
            if (explicitNodeTypes && explicitNodeTypes.has(nodeType)) {
                ...
                if (seenCanonicals.has(canonical.toLowerCase()) && canonical !== nodeType) {
                    console.log(`[IntentConstraintEngine] ⚠️  Skipping explicit node ${nodeType} ` +
                ...
                console.log(`[IntentConstraintEngine] ✅ Preserved explicit node: ${nodeType} ` +
            ...
            if (seenCanonicals.has(canonical.toLowerCase())) {
                console.log(`[IntentConstraintEngine] ⚠️  Skipping semantic duplicate: ${nodeType} ` +
            ...
            if (canonical !== nodeType) {
                console.log(`[IntentConstraintEngine] ✅ Normalized ${nodeType} → ${canonical} ` +
                    `(semantic equivalence)`);
            }
        }
        const result = Array.from(normalized);
        if (result.length < nodeTypes.length) {
            console.log(`[IntentConstraintEngine] ✅ Removed ${nodeTypes.length - result.length} semantic duplicate(s), ` +
    ...
    validateNodeTypes(nodeTypes) {
        ...
        for (const nodeType of nodeTypes) {
            ...
            const schema = node_library_1.nodeLibrary.getSchema(normalized);
            if (schema) {
                // ✅ ROOT-LEVEL FIX: Use schema.type (canonical name) instead of normalized
     ...
     */
    understandTransformationIntent(transformation, originalPrompt) {
        ...
        if ((tfType.includes('aggregate') || tfType.includes('group') || tfType.includes('sum')) &&
            !prompt.includes('ai') &&
            !prompt.includes('summarize') &&
     ...
     */
    selectNodesByIntent(semanticIntent, transformation, originalPrompt) {
     ...
     */
    findNodesByIntentCategories(intentCategories) {
        ...
        for (const schema of allSchemas) {
            ...
            const hasMatchingCategory = intentCategories.some(cat => nodeCategories.some(nodeCat => nodeCat.toLowerCase().includes(cat.toLowerCase()) ||
                cat.toLowerCase().includes(nodeCat.toLowerCase())));
            // Also check intent description for keywords
            const hasMatchingDescription = intentCategories.some(cat => nodeDescription.includes(cat.toLowerCase()));
     ...
     */
    validateNodeMatchesIntent(nodeType, semanticIntent, originalPrompt) {
     ...
     * @returns Array of required node types (trigger + data sources + transformations + outputs)
     */
    static getRequiredNodes(intent, originalPrompt, explicitNodeTypes) {
        const engine = new IntentConstraintEngine();
        return engine.getRequiredNodes(intent, originalPrompt, explicitNodeTypes);
    }
}
exports.IntentConstraintEngine = IntentConstraintEngine;
// Export singleton instance
exports.intentConstraintEngine = new IntentConstraintEngine();
// Export convenience function
function getRequiredNodes(intent, originalPrompt) {
