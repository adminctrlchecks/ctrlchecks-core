...
exports.WorkflowConstructionLogic = exports.HARD_FAIL_CONDITIONS = exports.NODE_SELECTION_HEURISTICS = exports.TRIGGER_SELECTION_RULES = exports.AI_USAGE_RULES = exports.NodeSelectionPriority = exports.WorkflowConstructionPhase = void 0;
// Workflow Construction Phases (MANDATORY)
var WorkflowConstructionPhase;
(function (WorkflowConstructionPhase) {
    WorkflowConstructionPhase["PHASE_1_TRIGGER_SELECTION"] = "PHASE_1_TRIGGER_SELECTION";
    WorkflowConstructionPhase["PHASE_2_ACTION_NODE_SELECTION"] = "PHASE_2_ACTION_NODE_SELECTION";
    WorkflowConstructionPhase["PHASE_3_DATA_MAPPING"] = "PHASE_3_DATA_MAPPING";
    WorkflowConstructionPhase["PHASE_4_CONDITIONS_AND_LOGIC"] = "PHASE_4_CONDITIONS_AND_LOGIC";
    WorkflowConstructionPhase["PHASE_5_AI_USAGE"] = "PHASE_5_AI_USAGE";
    WorkflowConstructionPhase["PHASE_6_ERROR_HANDLING"] = "PHASE_6_ERROR_HANDLING";
    WorkflowConstructionPhase["PHASE_7_WORKFLOW_FINALIZATION"] = "PHASE_7_WORKFLOW_FINALIZATION";
})(WorkflowConstructionPhase || (exports.WorkflowConstructionPhase = WorkflowConstructionPhase = {}));
// Node Selection Priority (STRICT ORDER)
var NodeSelectionPriority;
(function (NodeSelectionPriority) {
    NodeSelectionPriority[NodeSelectionPriority["NATIVE_INTEGRATION"] = 1] = "NATIVE_INTEGRATION";
    NodeSelectionPriority[NodeSelectionPriority["AI_AGENT"] = 2] = "AI_AGENT";
    NodeSelectionPriority[NodeSelectionPriority["HTTP_REQUEST"] = 3] = "HTTP_REQUEST";
})(NodeSelectionPriority || (exports.NodeSelectionPriority = NodeSelectionPriority = {}));
exports.AI_USAGE_RULES = {
    allowed: [
        'text generation',
        'summarization',
        'classification',
...
exports.TRIGGER_SELECTION_RULES = [
    {
...
exports.NODE_SELECTION_HEURISTICS = [
    {
 ...
 */
class WorkflowConstructionLogic {
    constructor() {
        this.currentPhase = null;
        this.phaseHistory = [];
        this.constructionErrors = [];
    }
    /**
     * PHASE 1: Trigger Selection
     */
    selectTrigger(userIntent, answers) {
        ...
        if (answers) {
            ...
            const explicitTrigger = Object.values(answers).find(answer => typeof answer === 'string' &&
            ...
            if (explicitTrigger) {
                const triggerMatch = exports.TRIGGER_SELECTION_RULES.find(rule => rule.triggerType === explicitTrigger.toLowerCase().replace(/\s+/g, '_'));
                if (triggerMatch) {
        ...
        const matches = exports.TRIGGER_SELECTION_RULES
            .map(rule => ({
            rule,
     ...
     */
    selectActionNodes(requirements, triggerType, availableNodes) {
     ...
     */
    validateDataMapping(nodes, edges) {
     ...
     */
    validateConditionsAndLogic(nodes) {
    ...
    validateAIUsage(nodes) {
        ...
        nodes.forEach(node => {
            ...
            if (nodeType === 'ai_agent' || nodeType.includes('ai') || nodeType.includes('gpt') || nodeType.includes('claude') || nodeType.includes('gemini')) {
                ...
                const isAllowed = exports.AI_USAGE_RULES.allowed.some(allowed => purpose.toLowerCase().includes(allowed.toLowerCase()));
                const isForbidden = exports.AI_USAGE_RULES.forbidden.some(forbidden => purpose.toLowerCase().includes(forbidden.toLowerCase()));
                if (isForbidden) {
                    errors.push({
                        nodeId: node.id,
     ...
     */
    validateErrorHandling(nodes, edges) {
        ...
        externalNodes.forEach(node => {
            ...
            const hasErrorPath = edges.some(edge => edge.source === node.id &&
     ...
     */
    finalizeWorkflow(nodes, edges, credentials) {
        ...
        const aiNodes = nodes.filter(n => (n.data?.type || n.type)?.includes('ai') ||
    ...
    findNativeNodes(requirements, availableNodes) {
        const nativeNodes = [];
    ...
    shouldUseAI(requirements) {
        const prompt = (requirements.userPrompt || '').toLowerCase();
        const needsAI = exports.AI_USAGE_RULES.allowed.some(allowed => prompt.includes(allowed.toLowerCase()));
        const forbiddenAI = exports.AI_USAGE_RULES.forbidden.some(forbidden => prompt.includes(forbidden.toLowerCase()));
        if (forbiddenAI) {
    ...
    needsHTTPRequest(requirements, selectedNodes) {
    ...
    detectInfiniteLoops(nodes, edges) {
    ...
    mapServiceToNodeType(serviceName) {
        const serviceMap = {
            'slack': 'slack_message',
            'email': 'email',
            'gmail': 'gmail',
    ...
    getConstructionErrors() {
        return [...this.constructionErrors];
    }
}
exports.WorkflowConstructionLogic = WorkflowConstructionLogic;
