...
exports.getCredentialContractsForNode = getCredentialContractsForNode;
 ...
 */
function getCredentialContractsForNode(nodeType) {
...
class CredentialResolver {
    ...
    async resolve(workflow, userId) {
        ...
        const credentialMap = new Map(); // Deduplicate by credentialId
        // STEP 1: Iterate all final nodes (canonical type + connector registry aligned with CredentialDiscoveryPhase)
        for (const node of workflow.nodes || []) {
            const rawType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(node);
            ...
            for (const contract of contracts) {
                ...
                if (!vaultResolved) {
                    configResolved = (0, credential_config_satisfaction_1.isCredentialSatisfiedByNodeConfig)(node, contract);
                ...
                if (!existing) {
                    ...
                    const anyVault = vaultResolved ||
                        (existing.resolved && existing.source === 'vault');
                    const mergedSource = mergedResolved
                        ? anyVault
                            ? 'vault'
                            : 'user_input'
                        : undefined;
                    ...
                    console.log(`[CredentialResolution] ${credentialId}: Merged nodes — resolved=${mergedResolved} (${contract.provider})`);
                }
            }
        }
     ...
     */
    async checkVaultForCredential(vaultKey, credentialType, userId, scopes) {
        return this.checkVault(vaultKey, credentialType, userId, scopes);
    }
    /**
     * Check if credential exists in vault (private implementation)
     */
    async checkVault(vaultKey, credentialType, userId, scopes) {
        try {
            ...
            if (credentialType === 'api_key' || credentialType === 'webhook' || credentialType === 'token' || credentialType === 'basic_auth') {
                try {
                    ...
                    if (!connError && Array.isArray(connRows) && connRows.length > 0) {
                        const match = connRows.some((row) => typeof row.credential_type_id === 'string' &&
            ...
            try {
                ...
                const exists = await vault.exists({ userId: effectiveUserId }, vaultKey);
                if (exists) {
     ...
     */
    getCredentialContracts(nodeType) {
     ...
     */
    assertGmailIntegrity(workflow, originalPrompt) {
        ...
        if (!hasGmailNode) {
            ...
            if (hasEmailNode) {
                throw new Error(`🚨 CRITICAL: Prompt mentions Gmail but workflow contains generic email node (SMTP). ` +
                    `Gmail must use google_gmail node with OAuth, not SMTP. ` +
                    `This indicates a node selection or repair error.`);
            }
            else {
                throw new Error(`🚨 CRITICAL: Prompt mentions Gmail but no Gmail node exists in workflow. ` +
     ...
     */
    validateGoogleIntegrations(workflow, allRequirements) {
        ...
        const credentialIntegrations = new Set();
     ...
     */
    assertGmailCredentials(resolution, workflow) {
        const hasGmailNode = workflow.nodes.some(node => {
            const nodeType = this.normalizeNodeType(node);
        ...
        if (!hasGoogleCredential) {
            throw new Error(`🚨 CRITICAL: Workflow contains google_gmail node but no Google OAuth credential detected. ` +
        ...
        if (googleCredential && googleCredential.type !== 'oauth') {
            throw new Error(`🚨 CRITICAL: Gmail node requires OAuth credential, but detected type: ${googleCredential.type}. ` +
        ...
        if (hasSmtpCredential) {
            throw new Error(`🚨 CRITICAL: Workflow contains Gmail node but SMTP credentials are also requested. ` +
