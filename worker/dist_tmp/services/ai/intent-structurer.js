...
class IntentStructurer {
     ...
     */
    normalizeEmailDestinations(intent, userPrompt) {
        ...
        (intent.actions || []).forEach(normalizeAction);
        (intent.dataSources || []).forEach(normalizeAction);
        (intent.transformations || []).forEach(normalizeAction);
    }
    /**
     * Extract structured intent using AI
     */
    async extractStructuredIntent(userPrompt) {
...
Return the JSON now:`;
        ...
        // Parse JSON response
        try {
            const responseText = typeof aiRaw === 'string'
                ? aiRaw
                : aiRaw?.response ?? String(aiRaw);
            const jsonMatch = responseText.match(/\{[\s\S]*\}/);
            if (!jsonMatch) {
                throw new Error('No JSON found in response');
            }
            const structuredIntent = JSON.parse(jsonMatch[0]);
            ...
            structuredIntent.requires_credentials = this.extractCredentials(structuredIntent.actions, structuredIntent.dataSources, structuredIntent.transformations);
            return structuredIntent;
        }
        catch (error) {
     ...
     */
    inferBestDefaultTrigger(userPrompt) {
        const p = (userPrompt || '').toLowerCase();
        if (p.includes('webhook') || p.includes('api call') || p.includes('http request'))
            return 'webhook';
        if (p.includes('form') || p.includes('submitted') || p.includes('submission'))
            return 'form';
        if (p.includes('chat') || p.includes('bot'))
            return 'chat_trigger';
        if (p.includes('manual') || p.includes('on demand') || p.includes('run now'))
            return 'manual_trigger';
     ...
     */
    inferDefaultTriggerConfig(trigger, userPrompt) {
        if (trigger !== 'schedule')
            return undefined;
        const p = (userPrompt || '').toLowerCase();
        const interval = p.includes('hourly') ? 'hourly' :
            p.includes('weekly') ? 'weekly' :
                p.includes('monthly') ? 'monthly' :
                    'daily';
        // Keep cron simple and stable; user can override in UI
        const cron = interval === 'hourly' ? '0 * * * *' :
     ...
     */
    applyDefaultTriggerPolicy(intent, userPrompt) {
        const p = (userPrompt || '').toLowerCase();
     ...
     */
    extractCredentials(actions, dataSources, transformations) {
     ...
     * Validate structured intent
     */
    validateStructuredIntent(intent) {
        const errors = [];
        // Validate trigger
...
exports.IntentStructurer = IntentStructurer;
exports.intentStructurer = new IntentStructurer();
