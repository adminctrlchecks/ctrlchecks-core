...
class NodeResolver {
    constructor(nodeLibrary) {
        this.capabilityIndex = new Map(); // capability -> node types
        this.providerIndex = new Map(); // provider -> node types
        this.keywordIndex = new Map(); // keyword -> node types
     ...
     */
    extractIntents(prompt, contextPrompt) {
        const intents = [];
        ...
        for (const [keyword, nodeTypes] of this.keywordIndex) {
            ...
            if (importantKeywordWords.length === 0) {
                ...
                const anyWordMatches = keywordWords.some(kw => promptWords.some(pw => pw === kw || pw.includes(kw) || kw.includes(pw)));
                if (!anyWordMatches)
                    continue;
            }
            else {
            ...
            const position = promptWords.findIndex(pw => pw === firstKeywordWord || pw.includes(firstKeywordWord) || firstKeywordWord.includes(pw)) * 10; // Multiply by 10 for approximate character position
        ...
        if (contextPrompt && contextLower.length > 0 && contextLower !== promptLower) {
            ...
            for (const [keyword, nodeTypes] of this.keywordIndex) {
                ...
                if (!allWordsFound)
                    continue;
                const firstKeywordWord = keywordWords[0];
                const position = contextWords.findIndex(pw => pw === firstKeywordWord || pw.includes(firstKeywordWord) || firstKeywordWord.includes(pw)) * 10 + contextPositionOffset;
                for (const nodeType of nodeTypes) {
                    if (this.nodeLibrary.isNodeTypeRegistered(nodeType)) {
                        const existing = detectedNodes.get(nodeType);
                        if (!existing) {
        ...
        const nodeTypeToSpecificWord = { google_gmail: 'gmail', slack_message: 'slack', instagram: 'instagram', outlook: 'outlook', telegram: 'telegram', linkedin: 'linkedin', discord: 'discord', twilio: 'twilio' };
        for (const [nodeType, detection] of [...detectedNodes.entries()]) {
            ...
            if (!isGenericMatch || !communicationNodeTypes.has(nodeType))
                continue;
     ...
     */
    detectOperationFromPrompt(prompt, keyword, keywordPosition, schema) {
        const promptLower = prompt.toLowerCase();
     ...
     */
    getOperationsFromSchema(schema) {
        try {
     ...
     * ✅ UNIVERSAL: Infer resource from node type and schema
     */
    inferResourceFromNodeType(nodeType, schema) {
     ...
     */
    inferProviderFromNodeType(nodeType, schema, promptLower, contextLower) {
        const nodeTypeLower = nodeType.toLowerCase();
        const label = (schema.label || '').toLowerCase();
        // Extract provider from node type (e.g., "google_gmail" → "google")
        if (nodeTypeLower.includes('google'))
            return 'google';
        if (nodeTypeLower.includes('microsoft') || nodeTypeLower.includes('ms_'))
            return 'microsoft';
        if (nodeTypeLower.includes('salesforce') || nodeTypeLower.includes('sf_'))
            return 'salesforce';
        if (nodeTypeLower.includes('slack'))
            return 'slack';
        if (nodeTypeLower.includes('hubspot'))
            return 'hubspot';
        if (nodeTypeLower.includes('zoho'))
            return 'zoho';
        if (nodeTypeLower.includes('airtable'))
            return 'airtable';
     ...
     */
    extractKeywordsForNode(promptLower, matchedKeyword, nodeType, schema) {
    ...
    resolveIntent(intent) {
        ...
        try {
            ...
            for (const example of modernExamples) {
                ...
                const registeredSelectedNodes = selectedNodes.filter((nodeType) => this.nodeLibrary.isNodeTypeRegistered(nodeType));
     ...
     */
    nodeMatchesIntent(nodeType, intent, schema) {
        ...
        return intentKeywords.some(keyword => capabilityKeywords.some((capKeyword) => capKeyword.toLowerCase().includes(keyword.toLowerCase())));
    }
    /**
     * Match intent by capability
     */
    matchByCapability(intent) {
    ...
    matchByProvider(intent) {
        ...
        const matchingNodes = Array.from(nodes).filter(nodeType => {
            ...
            const capabilities = schema.capabilities || [];
            const resourceMatch = capabilities.some((cap) => cap.includes(intent.resource) || cap.includes(intent.action));
        ...
        for (const keyword of intent.keywords) {
            ...
            if (nodes && nodes.size > 0) {
                for (const nodeType of nodes) {
                    ...
                    const capabilities = schema.capabilities || [];
                    if (capabilities.some((cap) => cap.includes(intent.resource) || cap.includes(intent.action))) {
                        score += 0.2;
                    }
    ...
    getSuggestions(intent) {
        ...
        for (const schema of schemas) {
            const capabilities = schema.capabilities || [];
            if (capabilities.some((cap) => cap.includes(intent.resource))) {
     ...
     */
    assertGmailIntegrity(prompt, resolvedNodes) {
        const promptLower = prompt.toLowerCase();
        ...
        if (!hasGmailNode) {
            ...
            if (hasEmailNode) {
                throw new Error(`🚨 CRITICAL: Prompt mentions Gmail but node resolver selected generic email node (SMTP). ` +
                    `Gmail must use google_gmail node with OAuth, not SMTP. ` +
                    `This indicates a node resolution failure.`);
            }
            else {
                throw new Error(`🚨 CRITICAL: Prompt mentions Gmail but node resolver did not select google_gmail node. ` +
     ...
     * @param contextPrompt - Optional context prompt (e.g., original prompt) for disambiguation
     */
    resolvePrompt(prompt, contextPrompt, options) {
        const promptLower = prompt.toLowerCase();
    ...
    isGmailMentionedOnly(prompt) {
        ...
        const windowMatch = gmailSheetsPattern1.exec(prompt) || gmailSheetsPattern2.exec(prompt);
        if (!windowMatch) {
            return false;
        }
    ...
    extractGmailKeywords(prompt) {
        const keywords = [];
        if (prompt.includes('gmail'))
            keywords.push('gmail');
        if (prompt.includes('google mail'))
            keywords.push('google mail');
        if (prompt.includes('google email'))
            keywords.push('google email');
        return keywords;
    }
    extractSlackKeywords(prompt) {
        const keywords = [];
        if (prompt.includes('slack'))
            keywords.push('slack');
        return keywords;
    }
    extractEmailKeywords(prompt) {
        const keywords = [];
        if (prompt.includes('email'))
            keywords.push('email');
        if (prompt.includes('mail'))
            keywords.push('mail');
        return keywords;
    }
    extractGoogleSheetsKeywords(prompt) {
        const keywords = [];
        if (prompt.includes('google sheet'))
            keywords.push('google sheet');
        if (prompt.includes('google spreadsheet'))
            keywords.push('google spreadsheet');
        if (prompt.includes('sheets'))
            keywords.push('sheets');
        return keywords;
    }
}
