...
exports.buildNodeDescriptionBlocks = buildNodeDescriptionBlocks;
...
function getCategory(nodeType) {
    return unified_node_registry_1.unifiedNodeRegistry.get(nodeType)?.category;
}
/**
 * Check if a node has a specific tag — registry-driven.
 */
function hasTag(nodeType, tag) {
    return unified_node_registry_1.unifiedNodeRegistry.get(nodeType)?.tags?.includes(tag) ?? false;
}
/**
 ...
 */
function isFormLike(nodeType) {
    const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
    if (!def)
        return false;
    if (def.tags?.includes('form'))
        return true;
    return def.inputSchema?.fields !== undefined;
}
/**
 * Detect if_else: isBranching AND outgoingPorts contains 'true' and 'false'.
 * Registry-driven — no hardcoded type name.
 */
function isIfElseNode(nodeType) {
    const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
    if (!def)
        return false;
 ...
 * Registry-driven — no hardcoded type name.
 */
function isSwitchNode(nodeType) {
    const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
    if (!def)
        return false;
    return (def.isBranching === true &&
 ...
 */
function isTerminalNode(nodeType) {
    return unified_node_registry_1.unifiedNodeRegistry.get(nodeType)?.workflowBehavior?.alwaysTerminal === true;
}
/**
 ...
 */
function extractFieldsByFillMode(nodeType) {
    const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
    if (!def)
    ...
    for (const [name, fieldDef] of Object.entries(inputSchema)) {
        const fd = fieldDef;
        const fillMode = fd?.fillMode?.default ?? 'manual_static';
        const ownership = fd?.ownership ?? '';
        if (ownership === 'credential')
            continue; // never show credential fields
        const entry = {
            name,
            type: fd?.type || 'string',
 ...
 */
function getSwitchDiscriminant(upstreamNodeType) {
    if (!upstreamNodeType)
        return 'value';
    const def = unified_node_registry_1.unifiedNodeRegistry.get(upstreamNodeType);
    if (!def)
        return 'value';
    ...
    const defaultPort = def.outputSchema?.default;
    if (defaultPort?.schema?.properties) {
        for (const key of preferredKeys) {
            if (key in defaultPort.schema.properties)
                return key;
        }
 ...
 */
function getUpstreamOutputKeys(upstreamNodeType) {
    if (!upstreamNodeType)
        return [];
    const def = unified_node_registry_1.unifiedNodeRegistry.get(upstreamNodeType);
    if (!def)
 ...
 */
function deriveNodeOperation(nodeType, intentOperation) {
    ...
    const opField = def.inputSchema?.operation;
    if (opField) {
        if (Array.isArray(opField.enum) && opField.enum.length > 0)
            return opField.enum[0];
 ...
 */
function buildNodeActionSentence(nodeType, operation, upstreamLabel, downstreamLabel, userPrompt, branchTag) {
    const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
    if (!def)
        return `Executes ${nodeType}`;
 ...
 */
function extractConditionFromPrompt(userPrompt, upstreamNodeType) {
    // Pattern 1: numeric comparison — "score >= 700", "age > 18", "amount <= 100"
    const numericMatch = userPrompt.match(/\b([a-zA-Z_][a-zA-Z0-9_\s]*?)\s*(>=|<=|>|<|!=|==|=)\s*([0-9]+(?:\.[0-9]+)?)\b/);
    if (numericMatch) {
    ...
    const equalityMatch = userPrompt.match(/\b(?:if|when|check(?:s)?|verify|validate)\s+([a-zA-Z_][a-zA-Z0-9_\s]*?)\s+(?:is|equals?|==)\s+([a-zA-Z0-9_"']+)/i);
    if (equalityMatch) {
    ...
    const booleanMatch = userPrompt.match(/\b(?:if|when)\s+(eligible|approved|valid|verified|active|enabled|true|false)\b/i);
    if (booleanMatch) {
        return {
 ...
 */
function buildNodeDescriptionBlocks(intent, chain, caseNodeMapping) {
    const blocks = [];
    ...
    for (let i = 0; i < chain.length; i++) {
        ...
        const nodeType = (0, plan_chain_prune_1.stripPlanTokenToType)(rawToken);
        const branchTag = (0, plan_chain_prune_1.extractBranchTag)(rawToken);
        const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
        const prevType = i > 0 ? (0, plan_chain_prune_1.stripPlanTokenToType)(chain[i - 1]) : undefined;
        const nextType = i < chain.length - 1 ? (0, plan_chain_prune_1.stripPlanTokenToType)(chain[i + 1]) : undefined;
        ...
        if (isIfElseNode(nodeType)) {
            ...
            let falseBranchTarget;
            for (let j = i + 2; j < chain.length; j++) {
                const t = (0, plan_chain_prune_1.stripPlanTokenToType)(chain[j]);
                if (t !== trueBranchTarget && !isTerminalNode(t)) {
                    falseBranchTarget = t;
                    break;
                }
                // If it's the same type as true branch but different branchTag, it's the false branch
                const bt = (0, plan_chain_prune_1.extractBranchTag)(chain[j]);
                if (t === trueBranchTarget && bt && bt !== ((0, plan_chain_prune_1.extractBranchTag)(chain[i + 1]) || 'true')) {
                    falseBranchTarget = t;
                    break;
                }
            }
        ...
        if (isSwitchNode(nodeType)) {
            ...
            if (caseNodeMapping && Object.keys(caseNodeMapping).length > 0) {
                for (const [caseValue, target] of Object.entries(caseNodeMapping)) {
                    ...
                    if (targetType)
                        switchCases.push({ value: caseValue, target: targetType });
                }
            }
            else {
        ...
        // ── Integration / action / output / utility nodes ─────────────────────────
        {
            ...
            if (upstreamKeys.length > 0 && prevType) {
                block.integrationDataSources = Object.fromEntries(upstreamKeys.map(k => [k, `from ${nodeLabel(prevType)}`]));
            }
            // Build the action sentence from registry metadata
            block.prose = buildNodeActionSentence(nodeType, operation, prevType ? nodeLabel(prevType) : undefined, nextType ? nodeLabel(nextType) : undefined, userPrompt, branchTag);
