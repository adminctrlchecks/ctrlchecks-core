...
exports.workflowTrainingService = exports.WorkflowTrainingService = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const training_monitor_1 = require("./training-monitor");
class WorkflowTrainingService {
    constructor() {
    ...
    loadModernExamples() {
        try {
            ...
            const modernDataset = JSON.parse(fileContent);
     ...
     */
    getModernExamples(limit = 5, userPrompt) {
     ...
     */
    getSimilarModernExamples(prompt, limit) {
    ...
    getAllWorkflows() {
        ...
        return allWorkflows.sort((a, b) => {
            const priorityA = a._priority || 5;
            const priorityB = b._priority || 5;
     ...
     */
    calculateTrainingWorkflowPriority(workflow) {
    ...
    getWorkflowsByPriority(priority) {
        ...
        return allWorkflows.filter(w => w._priority === priority);
    }
    /**
     ...
     */
    getSimilarWorkflows(userPrompt, limit = 3) {
     ...
     */
    getSimilarWorkflowsWithScores(userPrompt, limit = 3) {
        // Get all workflows (modern examples + training dataset)
        const allWorkflows = this.getAllWorkflows();
        if (allWorkflows.length === 0) {
            return [];
        ...
        const patternMatches = scored
            .filter(item => item.score >= PATTERN_MATCH_THRESHOLD)
            .sort((a, b) => b.score - a.score)
            .map(item => ({ ...item, matchType: 'pattern' }));
        if (patternMatches.length > 0) {
        ...
        const hybridMatches = scored
            .filter(item => item.score > 0)
            .sort((a, b) => b.score - a.score)
            .map(item => ({ ...item, matchType: 'hybrid' }));
        ...
        return allWorkflows
            ...
            .slice(0, limit)
            .map(workflow => ({ workflow, score: 0, matchType: 'fallback' }));
    }
    /**
     * Get training examples for system prompt generation
     */
    getSystemPromptExamples(limit = 5) {
        if (!this.dataset || !this.dataset.workflows) {
            return [];
        }
     ...
     */
    getRequirementsExamples(limit = 5) {
        if (!this.dataset || !this.dataset.workflows) {
            return [];
        }
        return this.dataset.workflows
     ...
     */
    getNodeSelectionExamples(limit = 5, userPrompt) {
        if (!this.dataset || !this.dataset.workflows) {
            return [];
        }
     ...
     */
    getClarificationQuestionExamples(userPrompt, limit = 3) {
        if (!this.dataset || !this.dataset.workflows) {
            return [];
        }
        ...
        return similar
            ...
            questions: (w.phase1.step2?.clarificationQuestions || []).map((q, idx) => {
     ...
     */
    buildSystemPromptFewShotPrompt(userPrompt) {
        // Check if dataset is loaded
        if (!this.isLoaded()) {
            console.warn('⚠️  Training dataset not loaded, skipping few-shot examples');
            training_monitor_1.trainingMonitor.recordUsage('systemPrompt', userPrompt, 0, false);
            return '';
        }
        try {
            ...
            training_monitor_1.trainingMonitor.recordUsage('systemPrompt', userPrompt, examples.length, true);
            return prompt;
        }
        catch (error) {
     ...
     */
    buildRequirementsFewShotPrompt(userPrompt, systemPrompt) {
        // Check if dataset is loaded
        if (!this.isLoaded()) {
            console.warn('⚠️  Training dataset not loaded, skipping few-shot examples');
            training_monitor_1.trainingMonitor.recordUsage('requirements', userPrompt, 0, false);
            return '';
        }
        try {
            ...
            training_monitor_1.trainingMonitor.recordUsage('requirements', userPrompt, examples.length, true);
            return prompt;
        }
        catch (error) {
     ...
     */
    buildNodeSelectionFewShotPrompt(requirements, availableNodes) {
     ...
     */
    buildExecutionReasoningFewShotPrompt(goal, currentState, availableActions) {
        // Check if dataset is loaded
        if (!this.isLoaded()) {
            console.warn('⚠️  Training dataset not loaded, skipping few-shot examples');
            training_monitor_1.trainingMonitor.recordUsage('execution', goal, 0, false);
            return '';
        }
        try {
            ...
            prompt += `Reasoning:`;
            training_monitor_1.trainingMonitor.recordUsage('execution', goal, examples.length, true);
            return prompt;
        }
        catch (error) {
    ...
    getWorkflowsByCategory(category) {
        ...
        return this.dataset.workflows.filter(w => w.category.toLowerCase() === category.toLowerCase());
    }
    /**
     * Get all categories
     */
     ...
     */
    adaptPattern(referenceWorkflow, newRequirements) {
        const modifications = [];
        const errors = [];
    ...
    validateCoreStarterSet() {
        ...
        const missing = [];
        const workflowTexts = this.dataset.workflows.map(w => `${w.goal} ${w.phase1.step1.userPrompt} ${w.category}`.toLowerCase());
        coreWorkflows.forEach(core => {
            const hasMatch = workflowTexts.some(text => core.keywords.some(keyword => text.includes(keyword.toLowerCase())));
            if (!hasMatch) {
    ...
    isCandidateReference(workflow) {
        ...
        const hasTrigger = workflow.nodes.some((n) => (n.data?.type || n.type)?.includes('trigger'));
        if (!hasTrigger) {
    ...
    validateMandatoryCategories() {
        ...
        const present = new Set(this.dataset.workflows
            .map(w => w.category)
            .filter(Boolean)
            .map(c => c.toLowerCase()));
        const missing = mandatory.filter(cat => !Array.from(present).some(p => p.includes(cat.toLowerCase()) || cat.toLowerCase().includes(p)));
        return {
            valid: missing.length === 0,
            missing,
...
exports.WorkflowTrainingService = WorkflowTrainingService;
// Export singleton instance
exports.workflowTrainingService = new WorkflowTrainingService();
