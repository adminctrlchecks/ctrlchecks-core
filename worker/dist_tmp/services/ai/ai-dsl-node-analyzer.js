...
exports.aiDSLNodeAnalyzer = exports.AIDSLNodeAnalyzer = void 0;
...
const node_replacement_tracker_1 = require("./node-replacement-tracker");
class AIDSLNodeAnalyzer {
    constructor() {
     ...
     */
    analyzeDSLNodes(dataSources, transformations, outputs, intent, originalPrompt, confidenceScore) {
        ...
        const ruleBasedResult = this.ruleBasedAnalysis(dataSources, transformations, outputs, intent, originalPrompt, confidenceScore);
     ...
     */
    ruleBasedAnalysis(dataSources, transformations, outputs, intent, originalPrompt, confidenceScore) {
        ...
        const { filtered: filteredDataSources, removed: dsRemovals } = this.removeDuplicateNodeTypes(dataSources, 'dataSource', confidenceScore);
        removals.push(...dsRemovals);
        const { filtered: filteredTransformations, removed: tfRemovals } = this.removeDuplicateNodeTypes(transformations, 'transformation', confidenceScore);
        removals.push(...tfRemovals);
        const { filtered: filteredOutputs, removed: outRemovals } = this.removeDuplicateNodeTypes(outputs, 'output', confidenceScore);
        removals.push(...outRemovals);
        // 2. Remove redundant HTTP requests
        const { filtered: optimizedDataSources, removed: httpRemovals } = this.removeRedundantHttpRequests(filteredDataSources, confidenceScore);
        removals.push(...httpRemovals);
        // 3. Remove category duplicates (if category already covered)
        const { filtered: optimizedTransformations, removed: categoryRemovals } = this.removeCategoryDuplicates(filteredTransformations, confidenceScore);
        ...
        const { filtered: finalTransformations, removed: aiRemovals } = this.removeUnnecessaryAINodes(optimizedTransformations, intent, originalPrompt, confidenceScore);
     ...
     */
    removeDuplicateNodeTypes(nodes, category, confidenceScore) {
        const seen = new Map();
        const removals = [];
        const filtered = [];
        for (const node of nodes) {
            const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(node.type);
            const operation = node.operation || '';
            const key = `${nodeType}:${operation}`;
            // ✅ CRITICAL: Never remove protected nodes (user-explicit nodes)
            const isProtected = node.protected === true || node.origin?.source === 'user';
            if (isProtected) {
            ...
            if (seen.has(key)) {
                ...
                if (existingIsProtected) {
                    ...
                    // ✅ TRACK REPLACEMENT
                    node_replacement_tracker_1.nodeReplacementTracker.trackReplacement({
                        nodeType,
                        operation,
                        category,
                        reason,
                        ...
                        metadata: {
                            existingNodeType: (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(existing.type),
                        },
                    });
     ...
     * NEVER removes protected nodes (user-explicit nodes)
     */
    removeRedundantHttpRequests(dataSources, confidenceScore) {
        const httpRequests = dataSources.filter(ds => {
            const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(ds.type);
        ...
        for (const ds of dataSources) {
            ...
            if (isHttpRequest) {
                // ✅ CRITICAL: Never remove protected nodes (user-explicit nodes)
                const isProtected = ds.protected === true || ds.origin?.source === 'user';
                if (isProtected) {
     ...
     * NEVER removes protected nodes (user-explicit nodes)
     */
    removeCategoryDuplicates(transformations, confidenceScore) {
        const removals = [];
        const filtered = [];
        const categoryCoverage = new Map(); // category -> nodeType
        for (const tf of transformations) {
            const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(tf.type);
            ...
            if (category && categoryCoverage.has(category)) {
                ...
                const existingNode = filtered.find(t => (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(t.type) === existingNodeType);
                ...
                if (shouldKeepThis) {
                    ...
                    const existingIndex = filtered.findIndex(t => (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(t.type) === existingNodeType);
                    if (existingIndex >= 0) {
                        removals.push({
     ...
     */
    isPreferredNode(nodeType1, nodeType2) {
        // Prefer ai_chat_model over ai_agent (simpler)
        if (nodeType1 === 'ai_chat_model' && nodeType2 === 'ai_agent')
            return true;
        if (nodeType1 === 'ai_agent' && nodeType2 === 'ai_chat_model')
            return false;
        // Prefer google_gmail over email (more specific)
        if (nodeType1 === 'google_gmail' && nodeType2 === 'email')
            return true;
        if (nodeType1 === 'email' && nodeType2 === 'google_gmail')
            return false;
        // Prefer shorter names (simpler)
        if (nodeType1.length < nodeType2.length)
            return true;
        if (nodeType1.length > nodeType2.length)
            return false;
        // Default: keep first
     ...
     */
    removeUnnecessaryAINodes(transformations, intent, originalPrompt, confidenceScore) {
        ...
        const removals = [];
        const filtered = [];
        const aiOperations = new Map(); // operation -> node
        for (const tf of transformations) {
            const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(tf.type);
            ...
            if (isAINode && operation) {
                if (aiOperations.has(operation)) {
                    ...
                    const existingType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(existing.type);
                    const existingIsProtected = existing.protected === true || existing.origin?.source === 'user';
     ...
     */
    async aiBasedAnalysis(dataSources, transformations, outputs, intent, originalPrompt) {
        const removals = [];
        try {
            // Build node summary for AI
            const allNodes = [
                ...dataSources.map(ds => ({ type: (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(ds.type), category: 'dataSource', operation: ds.operation })),
                ...transformations.map(tf => ({ type: (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(tf.type), category: 'transformation', operation: tf.operation })),
                ...outputs.map(out => ({ type: (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(out.type), category: 'output', operation: out.operation })),
            ];
...
Return empty array [] if all nodes are necessary.`;
            ...
            const { filteredDataSources, filteredTransformations, filteredOutputs, validatedRemovals } = this.applyAIRemovals(dataSources, transformations, outputs, aiRemovals);
    ...
    parseAIResponse(response) {
        try {
            ...
            return parsed.map((item) => ({
                nodeType: (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(item.nodeType || item.type || ''),
                reason: item.reason || 'AI determined node is unnecessary',
                confidence: typeof item.confidence === 'number' ? item.confidence : 0.7,
            })).filter((item) => item.nodeType); // Filter out invalid entries
        }
        catch (error) {
     ...
     */
    applyAIRemovals(dataSources, transformations, outputs, aiRemovals) {
        const validatedRemovals = [];
        // Filter data sources
        const filteredDataSources = dataSources.filter(ds => {
            const nodeType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(ds.type);
            const removal = aiRemovals.find(r => (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(r.nodeType) === nodeType);
...
exports.AIDSLNodeAnalyzer = AIDSLNodeAnalyzer;
// Export singleton instance
exports.aiDSLNodeAnalyzer = new AIDSLNodeAnalyzer();
