...
exports.workflowAnalyzer = exports.WorkflowAnalyzer = void 0;
...
class WorkflowAnalyzer {
     ...
     */
    fastAnalyzePrompt(userPrompt, context) {
     ...
     */
    async analyzePrompt(userPrompt, context) {
...
Return questions in JSON format as specified in the analysis prompt below.`;
     ...
     */
    filterGoogleOAuthQuestions(questions) {
     ...
     */
    filterChatbotQuestions(questions) {
     ...
     */
    buildAnalysisPrompt(userPrompt, context) {
        // Get training examples for clarification questions
        let fewShotExamples = '';
        try {
            const examples = workflow_training_service_1.workflowTrainingService.getClarificationQuestionExamples(userPrompt, 3);
...
Return ONLY valid JSON in this exact format (no markdown, no code blocks):
{
  ...
  "implicitRequirements": ["requirement1", "requirement2"]
     ...
     */
    parseAnalysisResponse(response, userPrompt) {
        try {
            ...
            if (result.questions.length < 3) {
                ...
                for (const fallbackQ of fallback.questions) {
                    ...
                    const isDuplicate = Array.from(existingTexts).some(existing => this.calculateTextSimilarity(fallbackText, existing) > 0.85);
                    if (!isDuplicate) {
     ...
     */
    detectAIFunctionalityNeeded(userPrompt, analysis) {
        const lowerPrompt = userPrompt.toLowerCase();
        ...
        const hasAIQuestion = analysis.questions.some(q => q.text.toLowerCase().includes('ai') ||
            q.text.toLowerCase().includes('model') ||
            q.text.toLowerCase().includes('provider') ||
            q.category === 'node_selection' && (q.options.some(opt => opt.toLowerCase().includes('ai') ||
                opt.toLowerCase().includes('gpt') ||
                opt.toLowerCase().includes('claude') ||
                opt.toLowerCase().includes('gemini'))));
        // Check implicit requirements
        const hasAIRequirement = analysis.implicitRequirements?.some(req => req.toLowerCase().includes('ai') ||
     ...
     */
    generateFallbackQuestions(userPrompt) {
        const lowerPrompt = userPrompt.toLowerCase();
     ...
     */
    generateFallbackSummary(userPrompt) {
        const words = userPrompt.split(/\s+/).slice(0, 25);
        return words.join(' ') + ' workflow automation with error handling and validation.';
    }
}
exports.WorkflowAnalyzer = WorkflowAnalyzer;
// Export singleton instance
exports.workflowAnalyzer = new WorkflowAnalyzer();
