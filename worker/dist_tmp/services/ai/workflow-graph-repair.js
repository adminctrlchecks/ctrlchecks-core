...
exports.workflowGraphRepair = exports.WorkflowGraphRepair = void 0;
const unified_node_type_normalizer_1 = require("../../core/utils/unified-node-type-normalizer");
class WorkflowGraphRepair {
    /**
     * Check graph integrity and detect missing structural nodes
     */
    checkGraphIntegrity(structure, nodes, requirements, userPrompt) {
        const missingNodes = [];
        ...
        const stepTypes = structure.steps.map(s => {
            const stepAny = s;
            return (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(stepAny.type || stepAny.nodeType || '') || stepAny.type || '';
        });
        const nodeTypes = nodes.map(n => (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(n) || n.type || '');
        ...
        const readOps = stepTypes.filter((t, idx) => {
            const step = structure.steps[idx];
            const operation = step?.operation || nodes.find(n => {
                const nType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(n) || n.type || '';
                return nType === t;
     ...
     */
    repairWorkflowGraph(structure, nodes, requirements, userPrompt) {
        const issues = [];
        const injectedNodes = [];
        // Check integrity
        ...
        const rebuiltConnections = this.rebuildConnections(structure.trigger || 'manual_trigger', modifiedSteps, structure.connections || []);
        ...
        // Add metadata
        modifiedStructure._repaired = true;
        modifiedStructure._injectedNodes = injectedNodes;
        return {
     ...
     */
    getNodeDescription(nodeType, userPrompt) {
        const promptLower = userPrompt.toLowerCase();
        if (nodeType === 'if_else') {
     ...
     * Rebuild connections after node injection
     */
    rebuildConnections(trigger, steps, originalConnections) {
        const connections = [];
...
exports.WorkflowGraphRepair = WorkflowGraphRepair;
exports.workflowGraphRepair = new WorkflowGraphRepair();
