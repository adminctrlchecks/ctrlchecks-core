...
exports.summarizeLayerService = exports.SummarizeLayerService = exports.AIIntentClarifier = exports.AliasKeywordCollector = void 0;
exports.isStructuralLlmSinglePlanEnabled = isStructuralLlmSinglePlanEnabled;
const node_library_1 = require("../nodes/node-library");
const node_type_pattern_registry_1 = require("../../core/registry/node-type-pattern-registry");
...
const universal_variation_node_categorizer_1 = require("../../core/utils/universal-variation-node-categorizer");
const gemini_node_selector_1 = require("./gemini-node-selector");
...
const node_description_builder_1 = require("./node-description-builder");
...
const summary_v2_validator_1 = require("../../core/validation/summary-v2-validator");
const pipeline_reasoning_coordinator_1 = require("./pipeline-reasoning-coordinator");
/** Structural single-plan LLM defaults on outside production; set ENABLE_STRUCTURAL_LLM_SINGLE_PLAN=false to disable. */
function isStructuralLlmSinglePlanEnabled() {
...
class AliasKeywordCollector {
    ...
    getAllAliasKeywords() {
        ...
        const allPatterns = (0, node_type_pattern_registry_1.getAllNodePatterns)();
        ...
        for (const schema of allSchemas) {
            ...
            for (const operation of operations) {
                ...
                const equivalents = semantic_node_equivalence_registry_1.semanticNodeEquivalenceRegistry.getEquivalents(nodeType, operation, category);
                ...
                const canonical = semantic_node_equivalence_registry_1.semanticNodeEquivalenceRegistry.getCanonicalType(nodeType, operation, category);
...
class AIIntentClarifier {
     ...
     */
    enrichNodeMentionsWithOperations(nodeMentions) {
        ...
        return nodeMentions.map(mention => {
            ...
            if (mention.operations && Array.isArray(mention.operations)) {
                return {
                    nodeType: mention.nodeType,
                    operations: mention.operations,
                    defaultOperation: mention.defaultOperation || '',
                };
            }
     ...
     */
    async clarifyIntentAndGenerateVariations(userPrompt) {
        ...
        let allExtractedNodeTypes;
        if (config_1.config.useGeminiFirstNodeSelection) {
            ...
            const pipelineContext = {
                ...
                structuredIntent: { trigger: 'manual_trigger', actions: [], requires_credentials: [] },
                registryKnowledgeSummary: pipeline_reasoning_coordinator_1.PipelineReasoningCoordinator.buildRegistryKnowledgeSummary(),
                priorStageOutputs: {},
            };
            const coordinator = new pipeline_reasoning_coordinator_1.PipelineReasoningCoordinator('gemini-3.1-pro-preview', 'gemini-3.5-flash', pipelineContext);
            let geminiNodeTypes = [];
            try {
                geminiNodeTypes = await coordinator.executeStage('node-selection', async () => {
                    const geminiResult = await (0, gemini_node_selector_1.selectNodesFromIntent)(userPrompt);
                    return {
                    ...
                    const allValid = proposal.proposal.every((t) => unified_node_registry_1.unifiedNodeRegistry.get(t) != null);
                    const tooMany = proposal.proposal.length > 8;
                    if (allValid && !tooMany) {
                        return { approved: true };
                    }
                    // Filter to valid registry types only
                    const filtered = proposal.proposal.filter((t) => unified_node_registry_1.unifiedNodeRegistry.get(t) != null).slice(0, 8);
                    return {
                        approved: false,
            ...
            else if (config_1.config.useGeminiFirstExclusively) {
        ...
        if (allExtractedNodeTypes.length > 0) {
            ...
            if (nodesWithoutOps.length > 0) {
                ...
                nodesWithoutOps.forEach(node => {
                    const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(node.nodeType);
        ...
        const logicNodeTypes = unified_node_registry_1.unifiedNodeRegistry.getAllTypes().filter(type => {
        ...
        if (hasLogicIntent && !hasAnyLogicNode) {
            const toAdd = logicNodeTypes.filter(t => node_library_1.nodeLibrary.isNodeTypeRegistered(t) && !extractedNodeTypes.includes(t));
            if (toAdd.length > 0) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const aiResponse = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-analysis', // Use this type to allow higher temperature
                {
                ...
                try {
                    ...
                    if (attempt === 1 && isJsonOrTruncation) {
                        ...
                        const retryResponse = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-analysis', { system: this.getSystemPrompt(extractedNodeTypes), message: aiPrompt }, { temperature: 0.7, max_tokens: 8192, cache: false });
                ...
                if (this.isValidResult(result)) {
                    ...
                    if (variationCount < 1 || variationCount > 4) {
                        console.warn(`[AIIntentClarifier] ⚠️ Invalid variation count ${variationCount} (expected between 1 and 4), retrying...`);
                        throw new Error(`Invalid variation count: got ${variationCount}, need between 1 and 4 valid variations`);
                    }
        ...
        if (allExtractedNodeTypes.length === 0) {
            ...
            const diagnosticError = new Error(`Failed to extract any nodes from user prompt: "${userPrompt}". ` +
        ...
        return this.createFallbackResultWithExtractedNodes(userPrompt, allKeywords, allExtractedNodeTypes, allKeywordData, lastError);
    }
    /**
     * ✅ PRODUCTION: Check if error is retryable
     */
     ...
     */
    validateVariationsIncludeNodes(result, requiredNodeTypes, nodeMentions, nodeMentionsWithOperations) {
        ...
        for (const variation of result.promptVariations) {
            ...
            const missingNodes = [];
            const missingOperations = []; // ✅ OPERATIONS-FIRST: Track missing operations
            for (const nodeType of requiredNodeTypes) {
                ...
                if (allNodesForMatching.length > 0) {
                    const matchResult = unified_node_type_matcher_1.unifiedNodeTypeMatcher.isRequirementSatisfied(nodeType, // required node
            ...
            for (const nodeType of requiredNodeTypes) {
                ...
                if (!found) {
                    const allNodesForMatching = v.allExtractedNodes && v.allExtractedNodes.length > 0
                        ...
                        : v.matchedKeywords;
                    if (allNodesForMatching.length > 0) {
                        const matchResult = unified_node_type_matcher_1.unifiedNodeTypeMatcher.isRequirementSatisfied(nodeType, allNodesForMatching, { strict: false });
                        if (matchResult.matches) {
                            found = true;
     ...
     */
    enhanceVariationsIfNeeded(result, userPrompt) {
        ...
        if (uniqueOutputs.size < 2 && uniqueTriggers.size < 2) {
            ...
            const enhancedVariations = result.promptVariations.map((v, idx) => {
                ...
                const manualTrigger = triggerNodes.find(nt => nt.includes('manual') || (unified_node_registry_1.unifiedNodeRegistry.get(nt)?.tags || []).includes('manual')) || triggerNodes[0];
                const webhookTrigger = triggerNodes.find(nt => nt.includes('webhook') || (unified_node_registry_1.unifiedNodeRegistry.get(nt)?.tags || []).includes('webhook')) || triggerNodes[1] || triggerNodes[0];
                if (idx < 2 && manualTrigger && !prompt.includes(manualTrigger.toLowerCase()) && !prompt.includes((unified_node_registry_1.unifiedNodeRegistry.get(manualTrigger)?.label || '').toLowerCase())) {
                    const triggerLabel = unified_node_registry_1.unifiedNodeRegistry.get(manualTrigger)?.label || manualTrigger;
                    enhancedPrompt = `Create a workflow with ${triggerLabel}. ${enhancedPrompt}`;
                }
                else if (idx >= 2 && webhookTrigger && !prompt.includes(webhookTrigger.toLowerCase()) && !prompt.includes((unified_node_registry_1.unifiedNodeRegistry.get(webhookTrigger)?.label || '').toLowerCase())) {
                    const triggerLabel = unified_node_registry_1.unifiedNodeRegistry.get(webhookTrigger)?.label || webhookTrigger;
                ...
                const outputNodeTypes = allNodeTypes.filter(nt => {
                    return node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nt) && !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nt);
                });
                if (outputNodeTypes.length >= 2) {
                    const output1 = outputNodeTypes[0];
                    const output2 = outputNodeTypes[1];
                    const output1Label = (unified_node_registry_1.unifiedNodeRegistry.get(output1)?.label || output1).toLowerCase();
                    const output2Label = (unified_node_registry_1.unifiedNodeRegistry.get(output2)?.label || output2).toLowerCase();
        ...
        const variationNodes = variations.map(v => {
            ...
            for (const nodeType of allNodeTypes) {
                ...
                if (promptLower.includes(nodeTypeLower) ||
                    nodeTypeWords.some((word) => word.length > 3 && promptLower.includes(word))) {
                    nodes.add(nodeType);
                }
            }
            return nodes;
        ...
        for (let i = 0; i < variationNodes.length; i++) {
            for (let j = i + 1; j < variationNodes.length; j++) {
                ...
                if (nodeSimilarity > 0.8 && union.size > 3) {
                    nodeDiversityIssues.push(`Variations ${i + 1} and ${j + 1} share ${intersection.size}/${union.size} nodes (${(nodeSimilarity * 100).toFixed(1)}% overlap) - too similar`);
                }
                // Check node count progression (Variation 1 should have fewer nodes than Variation 3)
                if (i === 0 && j === 2 && nodes1.size >= nodes2.size) {
                    nodeDiversityIssues.push(`Variation 1 (${nodes1.size} nodes) should have FEWER nodes than Variation 3 (${nodes2.size} nodes)`);
                }
            }
        }
     ...
     */
    createFallbackResultWithExtractedNodes(userPrompt, allKeywords, extractedNodeTypes, allKeywordData, error) {
        ...
        for (const nodeType of augmentedNodeTypes) {
            ...
            if (nodeDef?.inputSchema) {
                ...
                if (opField && (opField.enum || opField.oneOf)) {
                    const ops = opField.enum || (opField.oneOf?.map((o) => o.const).filter(Boolean) || []);
        ...
        return {
            ...
            registryTags: (0, gemini_node_selector_1.buildTagsFromRegistry)(augmentedNodeTypes),
        };
    }
    /**
     ...
     */
    categorizeExtractedNodes(extractedNodeTypes) {
        const categories = {
            dataSources: [],
            transformations: [],
            outputs: [],
        ...
        for (const nodeType of extractedNodeTypes) {
            ...
            if ((nodeCategory === 'communication' || nodeTags.includes('email') || nodeTags.includes('mail')) && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType)) {
            ...
            // Sheet/data nodes can be data sources (read) or outputs (write) - check context
                ...
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nodeType)) {
            ...
            else if ((nodeCategory === 'data' || nodeTags.includes('database') || nodeTags.includes('db')) && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nodeType)) {
                categories.dataSources.push(nodeType);
            }
     ...
     */
    identifyRequiredNodesFromIntent(userPrompt, categorizedNodes, allExtractedNodes) {
        const promptLower = userPrompt.toLowerCase();
        const required = {
            requiredDataSources: [],
        ...
        for (const nodeType of allExtractedNodes) {
            ...
            if (isMentioned) {
                ...
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType) &&
                    !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nodeType) &&
        ...
        const implicitMappings = [
            // Cloud storage - find storage nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && ((nodeDef.category === 'data' || (nodeDef.tags || []).includes('storage')) &&
            ...
            // Payment processing - find payment nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && ((nodeDef.tags || []).includes('payment') ||
            ...
            // Memory/remembering - find memory nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && (nt === 'memory' || (nodeDef.tags || []).includes('memory') || nt.includes('memory'));
                    });
                },
                category: 'transformation'
            },
            // Error handling - find error handling nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && ((nodeDef.tags || []).includes('error') ||
            ...
            // Database write operations - find database write nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && ((nodeDef.category === 'data' || (nodeDef.tags || []).includes('database')) &&
            ...
            // Calendar - find calendar nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && ((nodeDef.tags || []).includes('calendar') ||
            ...
            // Message/Notification - find message nodes from registry
            {
                ...
                findNodes: (registry) => {
                    return registry.getAllTypes().filter((nt) => {
                        const nodeDef = registry.get(nt);
                        return nodeDef && ((nodeDef.category === 'communication' || (nodeDef.tags || []).includes('message')) &&
        ...
        if (hasTransformationIntent) {
            ...
            const aiNodesInTransformations = categorizedNodes.transformations.filter(nt => {
                ...
                const isOutput = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nt) &&
                    !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nt);
                if (isOutput) {
                    return false;
                }
                return isAINode;
            ...
            if (aiNodesInTransformations.length > 0) {
                ...
                if (geminiIndex >= 0)
                    aiNodesInTransformations.splice(geminiIndex, 1);
                aiNodesInTransformations.unshift(preferredAi);
                ...
                const allAiNodes = allExtractedNodes.filter(nt => {
                    ...
                    return node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(nt) ||
                        (nodeDef?.category === 'ai');
                });
                if (allAiNodes.length > 0) {
                    ...
                    if (geminiNodes.length > 0) {
                        ...
                        const summarizerNode = allAiNodes.find(nt => nt.toLowerCase().includes('summarizer'));
                        if (summarizerNode) {
        ...
        if (hasOutputIntent) {
            ...
            for (const nodeType of allExtractedNodes) {
                ...
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType) &&
                    ...
                    if (promptLower.includes('all social') || promptLower.includes('all platforms')) {
                        ...
                        if (isSocialNode && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType)) {
            ...
            if (required.requiredOutputs.length === 0) {
                const realOutputs = categorizedNodes.outputs.filter(nt => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nt) &&
                    !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nt));
     ...
     */
    getNodeCapabilityCategory(nodeType) {
     ...
     */
    extractExplicitNodeTypesFromVariation(variationText, allKeywordData) {
     ...
     */
    buildWorkflowChain(requiredNodes, categorizedNodes, triggerType, userPrompt, allExtractedNodes, variationIndex = 0, // ✅ PHASE 1: 0 = Variation 1, 3 = Variation 4
    explicitNodeTypes // ✅ NEW: Nodes explicitly mentioned in selected variation
    ) {
        const chain = [triggerType];
        const usedNodes = new Set([triggerType]);
        const usedCapabilities = new Set(); // ✅ WORLD-CLASS: Track used capabilities
        const promptLower = userPrompt.toLowerCase();
        ...
        const isDuplicate = (nodeType) => {
            for (const existingNode of chain) {
                if (semantic_node_equivalence_registry_1.semanticNodeEquivalenceRegistry.areEquivalent(existingNode, nodeType)) {
        ...
        const getCanonical = (nodeType) => {
            return semantic_node_equivalence_registry_1.semanticNodeEquivalenceRegistry.getCanonicalType(nodeType) || nodeType;
        };
        ...
        const getAlternativeForVariation = (nodeType, variationIdx, explicitNodeTypes // ✅ NEW: Explicit nodes from selected variation
        ) => {
            ...
            const equivalents = semantic_node_equivalence_registry_1.semanticNodeEquivalenceRegistry.getEquivalents(nodeType);
        ...
        const addNodeToChain = (nodeType) => {
            ...
            const dedupeKey = (0, node_capability_dedupe_1.getNodeCapabilityDedupeKey)(nodeType);
            ...
            console.log(`[AIIntentClarifier] ✅ Added node: ${alternativeNode} (dedupe: ${dedupeKey ?? 'branching'})`);
        ...
        const categorizer = universal_variation_node_categorizer_1.UniversalVariationNodeCategorizer.getInstance();
     ...
     */
    buildVariationPrompt(chain, nodeLabels, nodeOperations, variationIndex) {
        if (chain.length < 2) {
        ...
        for (let i = 1; i < chain.length; i++) {
            ...
            const isDataSource = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nodeType) &&
                !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType);
            const isTransformation = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(nodeType) &&
                !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType) &&
                !isConditional;
            const isOutput = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType) &&
                !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(nodeType);
            let description = '';
            if (isConditional) {
     ...
     */
    cleanRepetitiveNodeText(prompt) {
     ...
     */
    analyzePromptStructure(userPrompt) {
        const promptLower = userPrompt.toLowerCase();
        const mentionedNodeTypes = [];
        const crmNodes = [];
        ...
        if (isConditional) {
            ...
            const conditionalNodeTypes = allNodeTypes.filter(nt => {
                const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(nt);
                return nodeDef && (nt === 'if_else' || nt === 'switch' ||
        ...
        for (const schema of allSchemas) {
            ...
            const isCRMNode = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'crm') ||
                node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'write_crm');
            const isNotificationNode = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'communication') ||
                node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'send_email') ||
                node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'send_message') ||
                node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'notification');
            const isTransformationNode = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'transformation') ||
                node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'ai_processing') ||
                node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'llm');
        ...
        if (mentionsCRM && crmNodes.length === 0) {
            ...
            for (const schema of allSchemas) {
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'crm') ||
                    node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'write_crm')) {
                    crmNodes.push(schema.type);
                    if (crmNodes.length >= 4)
                        break; // Get top 4 for variations
                }
            }
        }
        if (mentionsNotify && notificationNodes.length === 0) {
            ...
            for (const schema of allSchemas) {
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'communication') ||
                    node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'send_email') ||
                    node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'send_message') ||
                    node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.hasCapability(schema.type, 'notification')) {
                    notificationNodes.push(schema.type);
                    if (notificationNodes.length >= 4)
                        break; // Get top 4 for variations
                }
            }
        }
        ...
        for (const triggerNode of allTriggerNodes) {
            const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(triggerNode);
        ...
        if (triggerTypes.length === 0 && allTriggerNodes.length > 0) {
            ...
            const manualTrigger = allTriggerNodes.find(nt => nt.includes('manual') || (unified_node_registry_1.unifiedNodeRegistry.get(nt)?.tags || []).includes('manual')) || allTriggerNodes[0];
     ...
     */
    buildClarificationPrompt(userPrompt, allKeywords, extractedNodeTypes = [], nodeMentionsWithOperations) {
        ...
        const requiredNodeKeywords = new Set();
        for (const nodeType of extractedNodeTypes) {
...
- Each variation must describe a complete workflow that uses ALL ${extractedNodeTypes.length} REQUIRED NODES: ${extractedNodeTypes.join(', ')}`;
     ...
     */
    filterRelevantKeywordsWithSemanticExpansion(allKeywords, userPromptLower) {
     ...
     */
    getSystemPrompt(extractedNodeTypes = []) {
...
The "variations" array MUST contain exactly 4 items, each with a detailed prompt that:
  ...
  - Uses at most one primary data source and one primary output per variation.`;
     ...
     */
    deduplicateVariationTextByCapability(result) {
        ...
        const deduplicatedVariations = result.promptVariations.map((variation, idx) => {
            ...
            for (const nodeType of mentionedNodes) {
                const capability = (0, node_capability_dedupe_1.getNodeCapabilityDedupeKey)(nodeType) ?? `branching:${nodeType}`;
     ...
     */
    normalizeVariantsByStyle(result) {
        if (!result.promptVariations || result.promptVariations.length === 0) {
            return result;
        }
        const styledVariations = result.promptVariations.map((variation, index) => {
            ...
            console.log(`[AIIntentClarifier] ✅ Variant normalization: style=${style}, strategy=${strategy}, ` +
    ...
    parseAIResponse(aiResponse, originalPrompt, allKeywordData, extractedNodeTypes = []) {
        ...
        try {
            ...
            if (parsed.variations && Array.isArray(parsed.variations)) {
                for (let i = 0; i < parsed.variations.length; i++) {
                    ...
                    const strategy = rawStrategy === 'registry_minimal' ||
            ...
            const fromLlm = Array.isArray(parsed.matchedKeywords) && parsed.matchedKeywords.length > 0
                ? parsed.matchedKeywords.map((s) => String(s).trim()).filter(Boolean)
                : [];
            ...
            if (mandatoryNodesWithOperations.length > 0) {
                console.log(`[AIIntentClarifier] ✅ Extracted operation hints for ${mandatoryNodesWithOperations.length} node(s):`, mandatoryNodesWithOperations.map(n => `${n.nodeType}(${n.operationHint || 'none'})`).join(', '));
            }
            ...
            const pureVariations = variations.map(variation => {
                ...
                const matchedNodeTypes = variationNodeTypes.filter(nodeType => mandatoryNodeTypes.includes(nodeType));
                ...
                const chainWithCapabilities = this.buildChainWithCapabilities(variationNodeTypes, cleanedPrompt, mandatoryNodeTypes);
                return {
                    ...variation,
                    prompt: cleanedPrompt,
                    // ✅ PHASE 6: Tags include capability: ["webhook", "google_sheets:data_source", "google_gemini:transformation", "gmail:output"]
                    keywords: chainWithCapabilities.map((node) => node.capability ? `${node.type}:${node.capability}` : node.type),
                    matchedKeywords: chainWithCapabilities
                        .filter((node) => mandatoryNodeTypes.includes(node.type))
                        .map((node) => node.capability ? `${node.type}:${node.capability}` : node.type),
            ...
            return {
                ...
                registryTags: (0, gemini_node_selector_1.buildTagsFromRegistry)(mandatoryNodeTypes),
            };
        }
        catch (error) {
            ...
            return {
                ...
                registryTags: (0, gemini_node_selector_1.buildTagsFromRegistry)(fallbackNodeTypes),
            };
        }
    }
    /**
     ...
     */
    salvageMatchedKeywordsFromRawResponse(raw) {
        ...
        try {
            ...
            if (!Array.isArray(arr))
                return undefined;
            const strings = arr.filter((x) => typeof x === 'string').map(s => String(s).trim()).filter(Boolean);
     ...
     */
    extractKeywordsFromPrompt(userPrompt, allKeywordData) {
     ...
     */
    detectByNodeTypeName(nodeType, promptWords) {
        ...
        try {
            ...
            if (nodeTypeWords.every(word => promptWords.some(pw => pw === word || pw.includes(word) || word.includes(pw)))) {
                return { confidence: 0.95, method: 'type_name_all_words', match: nodeType };
            }
            // Partial match: "sheets" → "google_sheets"
            const overlap = nodeTypeWords.filter(word => promptWords.some(pw => pw.includes(word) || word.includes(pw))).length;
            if (overlap > 0) {
     ...
     */
    detectByRegistryLabel(nodeDef, promptLower) {
        ...
        try {
            ...
            const labelWords = label.split(/[\s_-]+/).filter((w) => w.length > 0);
            if (labelWords.length === 0)
                return null;
            const matchedWords = labelWords.filter((word) => word.length > 3 && promptLower.includes(word));
     ...
     */
    detectByRegistryTags(nodeDef, promptLower) {
        if (!nodeDef || !promptLower || typeof promptLower !== 'string') {
            return null;
        }
        try {
            const tags = Array.isArray(nodeDef.tags)
                ? nodeDef.tags.map((t) => t && typeof t === 'string' ? t.toLowerCase() : '').filter((t) => t.length > 0)
                : [];
            const aliases = Array.isArray(nodeDef.aliases)
                ? nodeDef.aliases.map((a) => a && typeof a === 'string' ? a.toLowerCase() : '').filter((a) => a.length > 0)
                : [];
            // Check tags
     ...
     */
    generateAdjacentWordPairs(promptWords) {
     ...
     */
    matchKeywordByWords(keyword, promptWords) {
        ...
        for (const keywordWord of keywordWords) {
            ...
            const found = promptWords.some(promptWord => promptWord === keywordWord || // Exact match
     ...
     */
    detectByKeywords(nodeType, allKeywords, promptLower) {
        ...
        try {
            ...
            for (const keywordData of nodeKeywords) {
                ...
                if (keywordLower.length < 3)
                    continue;
                if (overlyGenericKeywords.has(keywordLower))
                    continue;
     ...
     */
    detectBySemanticWords(nodeType, nodeDef, promptWords) {
        ...
        try {
            ...
            if (typeof nodeType === 'string') {
                nodeType.toLowerCase().split(/[_\s-]+/).filter((w) => w.length > 0).forEach((w) => semanticWords.add(w));
            }
            // From label: "Google Sheets" → ["google", "sheets"]
            if (nodeDef.label && typeof nodeDef.label === 'string') {
                nodeDef.label.toLowerCase().split(/[\s_-]+/).filter((w) => w.length > 0).forEach((w) => semanticWords.add(w));
            }
            ...
            if (nodeDef.description && typeof nodeDef.description === 'string') {
                ...
                const descriptionWords = description.split(/\s+/).filter((w) => w.length > 4);
                descriptionWords.forEach((w) => semanticWords.add(w));
            }
            if (semanticWords.size === 0)
                return null;
            // Count matches
            const matches = promptWords.filter(pw => Array.from(semanticWords).some(sw => pw.includes(sw) || sw.includes(pw))).length;
            if (matches > 0) {
     ...
     */
    detectByFuzzyMatching(nodeType, promptWords) {
        ...
        try {
            ...
            for (const promptWord of promptWords) {
                if (!promptWord || typeof promptWord !== 'string')
                    continue;
     ...
     */
    universalNodeDetection(userPrompt, allKeywordData) {
        ...
        try {
            ...
            let allNodeTypes = [];
            try {
                allNodeTypes = unified_node_registry_1.unifiedNodeRegistry.getAllTypes();
            }
            catch (error) {
     ...
     */
    extractNodesWithOperations(userPrompt, allKeywordData, nodeTypes) {
        const promptLower = userPrompt.toLowerCase();
        const result = [];
     ...
     */
    groupNodesBySemanticCategory(nodeTypes) {
     ...
     */
    selectOneNodePerCategoryWithIntentPreservation(groupedByCategory, extractedKeywords) {
        const selected = [];
     ...
     */
    classifyIntentType(keyword, nodeType, promptLower) {
        const keywordLower = keyword.toLowerCase();
     ...
     */
    findExactNodeTypeMatch(keyword) {
        try {
            const allNodeTypes = unified_node_registry_1.unifiedNodeRegistry.getAllTypes();
            const keywordLower = keyword.toLowerCase();
            // Check exact match
     ...
     */
    findNodeTypeViaAlias(keyword, expectedNodeType) {
        try {
            const keywordData = this.keywordCollector.getAllAliasKeywords();
            const keywordLower = keyword.toLowerCase();
            // Check if keyword maps to expected node type
            const match = keywordData.find(kd => kd.keyword.toLowerCase() === keywordLower &&
                kd.nodeType === expectedNodeType &&
                unified_node_registry_1.unifiedNodeRegistry.get(kd.nodeType) !== undefined);
     ...
     */
    isSpecificNodeName(keyword, nodeType) {
        const keywordLower = keyword.toLowerCase();
        const nodeTypeLower = nodeType.toLowerCase();
        ...
        for (const registeredNodeType of allNodeTypes) {
            const registeredNodeDef = unified_node_registry_1.unifiedNodeRegistry.get(registeredNodeType);
            if (!registeredNodeDef)
                continue;
     ...
     */
    isGeneralCategoryTerm(keyword, nodeType) {
        const keywordLower = keyword.toLowerCase();
     ...
     */
    validateKeywordContext(prompt, keyword, nodeType) {
        const promptLower = prompt.toLowerCase();
        const keywordLower = keyword.toLowerCase();
        // ✅ ROOT-LEVEL: Get node definition from registry
        const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
        if (!nodeDef)
            return true; // If node not in registry, allow (fallback)
        const nodeLabel = (nodeDef.label || nodeType).toLowerCase();
        const nodeCategory = (nodeDef.category || '').toLowerCase();
        const nodeTags = (nodeDef.tags || []).map(t => t.toLowerCase());
        ...
        if (keywordLower === 'post' || keywordLower === 'posting') {
            ...
            if (match && match[1]) {
                ...
                const nodeKeywords = this.keywordCollector.getAllAliasKeywords()
                    .filter(k => k.nodeType === nodeType)
                    .map(k => k.keyword.toLowerCase());
                const platformMatchesNode = nodeKeywords.some(k => platform.includes(k) || k.includes(platform) ||
        ...
        if (keywordLower === 'ai') {
            const isGenericAI = nodeCategory === 'ai' && (nodeType.includes('service') ||
     ...
     */
    getNodeSpecificPattern(nodeType, keyword) {
     ...
     */
    mapKeywordsToNodeTypes(keywords, originalPrompt) {
        ...
        for (const keyword of keywords) {
            ...
            if (node_library_1.nodeLibrary.isNodeTypeRegistered(keyword)) {
            ...
            if (keyword.toLowerCase() === 'email' && mentionsGmail && !mentionsSmtp) {
                if (node_library_1.nodeLibrary.isNodeTypeRegistered('google_gmail')) {
            ...
            const keywordData = allAliasKeywords.find(kd => kd.keyword.toLowerCase() === keyword.toLowerCase());
            if (keywordData) {
                ...
                if (node_library_1.nodeLibrary.isNodeTypeRegistered(mappedNodeType)) {
     ...
     */
    getOutputNodeKeywords() {
        const allKeywordData = this.keywordCollector.getAllAliasKeywords();
        const outputKeywords = new Set();
        for (const keywordData of allKeywordData) {
            // Check if node has output capability
            if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(keywordData.nodeType)) {
     ...
     */
    detectOutputNodeFromPrompt(prompt) {
        const promptLower = prompt.toLowerCase();
        const allKeywordData = this.keywordCollector.getAllAliasKeywords();
        // Get all output node types
        const allNodeTypes = node_library_1.nodeLibrary.getRegisteredNodeTypes();
        const outputNodeTypes = allNodeTypes.filter(nodeType => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType));
     ...
     */
    buildDynamicExampleJSON(extractedNodeTypes) {
        ...
        const categorizeNodes = () => {
            ...
            for (const nodeType of allNodes) {
                ...
                else if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nodeType)) {
                    categories.outputs.push(nodeType);
                }
                else {
                    categories.others.push(nodeType);
        ...
        const getNodeByCategory = (categoryNodes, fallbackIndex) => {
        ...
        const getTriggerNode = (preferWebhook = false) => {
            // First, try to find from extracted nodes
            if (preferWebhook) {
                const webhookTrigger = allNodes.find(n => {
                    const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(n);
                    return nodeDef && ((nodeDef.category === 'trigger' || (nodeDef.tags || []).includes('trigger')) &&
            ...
            if (categories.triggers.length > 0)
                return categories.triggers[0];
            // ✅ UNIVERSAL: Find any trigger from registry (NO hardcoded 'manual_trigger' or 'webhook')
            const allRegisteredTriggers = node_library_1.nodeLibrary.getRegisteredNodeTypes().filter(nodeType => {
                const nodeDef = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
                return nodeDef && (nodeDef.category === 'trigger' || (nodeDef.tags || []).includes('trigger'));
            });
            if (preferWebhook) {
                const webhookFromRegistry = allRegisteredTriggers.find(t => t.includes('webhook'));
                if (webhookFromRegistry)
                    return webhookFromRegistry;
            }
        ...
        const buildExample = (triggerType, nodeIndex) => {
  ...
  "variations": [
     ...
     */
    inferCapabilityFromVariationContext(nodeType, variationText, positionInChain, chainLength) {
        ...
        const availableCapabilities = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.getCapabilities(nodeType);
     ...
     */
    validateCapability(nodeType, inferredCapability) {
        const availableCapabilities = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.getCapabilities(nodeType);
     ...
     */
    buildChainWithCapabilities(extractedNodes, variationText, mandatoryNodes) {
        const chain = [];
        ...
        const dataSources = extractedNodes.filter(node => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(node) &&
            !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(node) &&
            !usedNodes.has(node));
        for (const ds of dataSources) {
            const inferredCapability = this.inferCapabilityFromVariationContext(ds, variationText, chain.length, extractedNodes.length);
            // Validate capability
        ...
        const transformations = extractedNodes.filter(node => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(node) &&
            !usedNodes.has(node));
        for (const tf of transformations) {
            const inferredCapability = this.inferCapabilityFromVariationContext(tf, variationText, chain.length, extractedNodes.length);
            // Validate capability
        ...
        const outputs = extractedNodes.filter(node => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(node) &&
            !node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(node) &&
            !usedNodes.has(node));
        for (const out of outputs) {
            const inferredCapability = this.inferCapabilityFromVariationContext(out, variationText, chain.length, extractedNodes.length);
            // Validate capability
        ...
        for (const mandatory of mandatoryNodes) {
            if (!usedNodes.has(mandatory)) {
                ...
                const inferredCapability = this.inferCapabilityFromVariationContext(mandatory, variationText, chain.length, extractedNodes.length);
                // Validate capability
                ...
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(mandatory)) {
                    chain.splice(1, 0, { type: mandatory, capability: capability || 'data_source' }); // After trigger
                }
                else if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(mandatory)) {
                    // ✅ PHASE 6: Find last data source index (backward search)
                    let lastDataSourceIdx = -1;
                    for (let i = chain.length - 1; i >= 0; i--) {
                        if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isDataSource(chain[i].type)) {
                            lastDataSourceIdx = i;
                            break;
                        }
                    }
                ...
                else if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(mandatory)) {
        ...
        chain.sort((a, b) => {
            ...
            if (a.capability && !b.capability)
                return 1;
            if (!a.capability && !b.capability)
                return 0; // Both triggers - preserve order
     ...
     */
    async clarifyIntentAndGenerateSinglePlan(userPrompt) {
        ...
        if (isStructuralLlmSinglePlanEnabled() && config_1.config.geminiApiKey) {
            try {
                const aiRaw = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-generation', {
                    system: this.getSinglePlanSystemPrompt(extractedNodeTypes),
                    message: this.buildSinglePlanUserMessage(userPrompt, allKeywords, extractedNodeTypes, enrichedNodeMentions),
                }, { cache: false, temperature: 0.2 });
                const rawStr = typeof aiRaw === 'string'
                    ? aiRaw
                    : aiRaw != null && typeof aiRaw === 'object'
                        ? JSON.stringify(aiRaw)
                        : String(aiRaw);
                const aiPlan = this.parseSinglePlanAIResponse(rawStr, userPrompt, allKeywordData, extractedNodeTypes, enrichedNodeMentions);
                const { canonicalizePlanChainStrict, validateCanonicalChainCompleteness, validateCanonicalChainSemantics, } = await Promise.resolve().then(() => __importStar(require('../../api/plan-chain-guards')));
                const canon = canonicalizePlanChainStrict(aiPlan.proposedNodeChain);
                if (canon.issues.length > 0) {
                    throw new Error(`LLM plan chain non-canonical: ${canon.issues.map(i => i.reason).join('; ')}`);
                }
                const guardIssues = [
                ...
                if (guardIssues.length > 0) {
                    throw new Error(`LLM plan failed structural guards: ${guardIssues.map(i => i.reason).join('; ')}`);
                }
                this.assertPlanConsistency(aiPlan, userPrompt, extractedNodeTypes);
                return this.summarizeLayerResultFromPlan(aiPlan, userPrompt, allKeywords, allKeywordData, extractedNodeTypes, enrichedNodeMentions);
            }
            catch (e) {
                console.warn('[SinglePlan] Structural LLM path failed, using deterministic chain:', e instanceof Error ? e.message : e);
            }
        }
        ...
        const branchingOverview = undefined;
        const syntheticWf = { nodes: [], edges: [] };
        const intentModelDigest = (0, workflow_intent_model_1.formatWorkflowIntentModelDigest)((0, workflow_intent_model_1.buildWorkflowIntentModel)(syntheticWf, userPrompt));
        const chainBaseTypes = proposedNodeChain.map((t) => (0, plan_chain_prune_1.stripPlanTokenToType)(t));
        const mandatoryNodeTypes = [...new Set([...extractedNodeTypes, ...chainBaseTypes])];
        const mandatoryNodesWithOperations = [];
        ...
        const finalStructuredSummary = this.buildStructuredSummaryFromChain(annotatedChain, userPrompt, branching, intentModelDigest, caseNodeMappingResult.caseNodeMapping);
        // Build per-node description blocks for UI rendering (stored on the plan).
        let nodeDescriptionBlocks;
        if (annotatedChain.length > 0) {
            ...
            nodeDescriptionBlocks = (0, node_description_builder_1.buildNodeDescriptionBlocks)(minimalIntent, annotatedChain, caseNodeMappingResult.caseNodeMapping);
        }
        const plan = {
            ...
            orderingDiagnostics: this.buildOrderingDiagnostics(annotatedChain, userPrompt, branching),
            mandatoryNodeTypes,
            mandatoryNodesWithOperations: mandatoryNodesWithOperations.length > 0 ? mandatoryNodesWithOperations : undefined,
            registryTags: (0, gemini_node_selector_1.buildTagsFromRegistry)(mandatoryNodeTypes),
            branching,
            branchingOverview,
            originalPrompt: userPrompt,
        ...
        return this.summarizeLayerResultFromPlan(plan, userPrompt, allKeywords, allKeywordData, extractedNodeTypes, enrichedNodeMentions);
    }
    getSinglePlanSystemPrompt(extractedNodeTypes) {
        const nodesHint = extractedNodeTypes.length > 0
...
`;
    ...
    buildSinglePlanUserMessage(userPrompt, allKeywords, extractedNodeTypes, enrichedNodeMentions) {
        const kwSample = allKeywords.slice(0, 200).join(', ');
        const opsBlock = enrichedNodeMentions.length > 0
            ? enrichedNodeMentions
                .map(m => `- ${m.nodeType}: operations [${m.operations.join(', ') || 'n/a'}], defaultOperation: ${m.defaultOperation || 'n/a'}`)
                .join('\n')
            : '(none)';
    ...
    parseSinglePlanAIResponse(raw, userPrompt, _allKeywordData, extractedNodeTypes, enrichedNodeMentions) {
        ...
        let structuredSummary = typeof obj.structuredSummary === 'string' ? obj.structuredSummary.trim() : '';
        let proposedNodeChain = Array.isArray(obj.proposedNodeChain)
            ? obj.proposedNodeChain.filter(x => typeof x === 'string').map(x => x.trim())
            : [];
        const branchingOverview = typeof obj.branchingOverview === 'string' ? obj.branchingOverview.trim() : undefined;
        const nodeInclusionReasons = obj.nodeInclusionReasons && typeof obj.nodeInclusionReasons === 'object'
            ? Object.entries(obj.nodeInclusionReasons).reduce((acc, [k, v]) => {
        ...
        if (!structuredSummary || structuredSummary.length < 20) {
            const syntheticWf = { nodes: [], edges: [] };
            const digest = (0, workflow_intent_model_1.formatWorkflowIntentModelDigest)((0, workflow_intent_model_1.buildWorkflowIntentModel)(syntheticWf, userPrompt));
            structuredSummary = this.buildStructuredSummaryFromChain(proposedNodeChain, userPrompt, branching, digest);
        }
        const mandatoryFromAi = Array.isArray(obj.mandatoryNodeTypes)
            ? obj.mandatoryNodeTypes.filter((x) => typeof x === 'string')
            : [];
        const chainBaseForMandatory = proposedNodeChain.map((t) => (0, plan_chain_prune_1.stripPlanTokenToType)(t));
        ...
        let nodeDescriptionBlocksAi;
        if (annotatedChainAi.length > 0) {
            const minimalIntentAi = {
                trigger: userPrompt, // pass full prompt so condition extraction works
                actions: enrichedNodeMentions.map(m => ({ type: m.nodeType, operation: m.defaultOperation || m.operations[0] || '' })),
                requires_credentials: [],
            };
            nodeDescriptionBlocksAi = (0, node_description_builder_1.buildNodeDescriptionBlocks)(minimalIntentAi, annotatedChainAi, caseNodeMappingResultAi.caseNodeMapping);
        }
        ...
        const syntheticWfAi = { nodes: [], edges: [] };
        const digestAi = (0, workflow_intent_model_1.formatWorkflowIntentModelDigest)((0, workflow_intent_model_1.buildWorkflowIntentModel)(syntheticWfAi, userPrompt));
        const finalStructuredSummaryAi = this.buildStructuredSummaryFromChain(annotatedChainAi, userPrompt, branching, digestAi, caseNodeMappingResultAi.caseNodeMapping);
        return {
            structuredSummary: finalStructuredSummaryAi,
            proposedNodeChain: annotatedChainAi,
            nodeInclusionReasons,
            rankedSelectionDiagnostics: this.rankNodesByIntent(userPrompt, mandatoryNodeTypes, nodeInclusionReasons || {}),
            ...
            branchingOverview,
            originalPrompt: userPrompt,
            intentModelDigest: (0, workflow_intent_model_1.formatWorkflowIntentModelDigest)((0, workflow_intent_model_1.buildWorkflowIntentModel)({ nodes: [], edges: [] }, userPrompt)),
     ...
     */
    _resolveCaseNodeMappingOrError(proposedNodeChain, userPrompt) {
     ...
     */
    buildCaseNodeMappingForPlan(proposedNodeChain, userPrompt) {
        return (0, switch_case_node_mapping_1.buildCaseNodeMappingFromPlanChain)(proposedNodeChain, userPrompt);
    }
    normalizeAndEnsureLogOutput(chain) {
        const out = [];
        const seen = new Set();
        ...
        for (const raw of chain) {
            ...
            if (hasBranchingNode &&
                ...
                out.push((0, plan_chain_prune_1.formatPlanChainToken)(String(raw || '').trim(), normalized));
                continue;
            }
        ...
        if (!hasTrigger && unified_node_registry_1.unifiedNodeRegistry.has('manual_trigger')) {
            out.unshift('manual_trigger');
        }
        const last = out[out.length - 1];
        if (last !== 'log_output' && unified_node_registry_1.unifiedNodeRegistry.has('log_output')) {
            out.push('log_output');
        }
        return (0, plan_chain_prune_1.pruneProposedPlanChain)(out);
    }
    buildDeterministicSinglePlanChain(userPrompt, extractedNodeTypes) {
        ...
        const branchNode = extractedNodeTypes.find((t) => t === 'if_else' || t === 'switch') ||
            (conditionalIntent && unified_node_registry_1.unifiedNodeRegistry.has('if_else') ? 'if_else' : null);
        let normalized = this.sequenceNodesByIntent(userPrompt, extractedNodeTypes);
        if (branchNode) {
            const requiredTargets = (0, branch_intent_model_1.expectedBranchTargetCount)(signals);
            let working = normalized.filter((t) => t !== 'log_output');
            const currentOutputs = working.filter((t) => !!unified_node_registry_1.unifiedNodeRegistry.get(t) && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t));
            if (currentOutputs.length < requiredTargets) {
                ...
                const normalizedMentioned = signals.mentionedOutputNodeTypes.map((t) => (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(t) || t);
                ...
                const preferredWithRepeats = preferred.length >= missingCount
                    ? preferred
                    : preferred.length > 0
                        ? [
                            ...preferred,
                ...
                const fallbackOutputTypes = shouldUseGenericFallbacks
                    ? unified_node_registry_1.unifiedNodeRegistry
                        .getAllTypes()
                        .filter((t) => t !== 'log_output' && !working.includes(t) && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t))
                    : [];
            ...
            const branchOutputs = working.filter((t) => t !== selectedTrigger && t !== branchNode && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t));
            ...
            const branchChainBody = branchOutputs.length > 0 ? branchOutputs.flatMap((o) => [o, 'log_output']) : [];
            ...
            for (const raw of rawChain) {
                ...
                if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(nt) && nt !== 'log_output') {
                    out.push(nt);
                    continue;
                }
                // Triggers: only one
            ...
            if (!out.some((t) => this.isTriggerNodeType(t)) && unified_node_registry_1.unifiedNodeRegistry.has('manual_trigger')) {
                out.unshift('manual_trigger');
            }
            if (out[out.length - 1] !== 'log_output' && unified_node_registry_1.unifiedNodeRegistry.has('log_output')) {
                out.push('log_output');
            }
            normalized = (0, plan_chain_prune_1.pruneProposedPlanChain)(out);
        }
        return normalized;
    }
    /**
     ...
     */
    annotateSameTypeBranchTokens(chain) {
        const result = [...chain];
        for (let i = 0; i < result.length; i++) {
            const nodeType = (0, plan_chain_prune_1.stripPlanTokenToType)(result[i]);
            ...
            const trueIdx = i + 1;
            const falseIdx = i + 2;
            if (trueIdx >= result.length || falseIdx >= result.length)
                continue;
            const trueType = (0, plan_chain_prune_1.stripPlanTokenToType)(result[trueIdx]);
            const falseType = (0, plan_chain_prune_1.stripPlanTokenToType)(result[falseIdx]);
            if (trueType === falseType) {
                // Same node type on both branches — annotate with branch tags
                result[trueIdx] = (0, plan_chain_prune_1.formatPlanChainTokenWithBranchTag)(trueType, 'true');
                result[falseIdx] = (0, plan_chain_prune_1.formatPlanChainTokenWithBranchTag)(falseType, 'false');
            }
        }
        return result;
    }
     ...
     */
    buildBranchMetadataForPlan(userPrompt, chain) {
        const signals = (0, branch_intent_model_1.extractBranchIntentSignals)(userPrompt);
        if (!signals.hasBranchingIntent) {
            return undefined;
        }
        // Determine branching node type present in the chain, if any.
        const branchingNodeType = chain.find((t) => this.isBranchingNodeType(t)) || (signals.branchType === 'switch' ? 'switch' : signals.branchType === 'if_else' ? 'if_else' : undefined);
        ...
        const downstreamOutputs = chain.filter((t) => t !== 'log_output' &&
            t !== branchingNodeType &&
            !this.isTriggerNodeType(t) &&
            node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t));
        ...
        const estimatedCount = Math.max(signals.estimatedBranchCount || 0, distinctDescriptors.length || 0, 2);
        const effectiveBranchKind = signals.branchType && (signals.branchType === 'switch' || signals.branchType === 'if_else')
            ? signals.branchType
            : 'if_else';
        const cases = [];
        ...
        for (let idx = 0; idx < caseCount; idx++) {
            const label = distinctDescriptors[idx] ||
                (effectiveBranchKind === 'switch' ? `case_${idx + 1}` : idx === 0 ? 'true' : 'false');
            const condition = effectiveBranchKind === 'switch'
                ? {
                    type: 'equality',
    ...
    classifyNodeRole(nodeType) {
        if (this.isTriggerNodeType(nodeType))
            return 'trigger';
        if (this.isBranchingNodeType(nodeType))
            return 'branching';
        ...
        if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(nodeType))
            return 'transformation';
        if (node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.canServeAsOutput(nodeType))
            return 'output';
        return 'other';
    }
    sequenceNodesByIntent(userPrompt, extractedNodeTypes) {
        const signals = (0, branch_intent_model_1.extractBranchIntentSignals)(userPrompt);
        const conditionalIntent = signals.hasBranchingIntent;
        const branchNode = extractedNodeTypes.find((t) => t === 'if_else' || t === 'switch') ||
            (conditionalIntent && unified_node_registry_1.unifiedNodeRegistry.has('if_else') ? 'if_else' : null);
        ...
        const others = [];
        for (const nodeType of extractedNodeTypes) {
            if (nodeType === selectedTrigger || nodeType === 'log_output')
                continue;
            if (branchNode && nodeType === branchNode)
                continue;
            const role = this.classifyNodeRole(nodeType);
            if (role === 'data_source')
                dataSources.push(nodeType);
            else if (role === 'transformation')
                transformations.push(nodeType);
            else if (role === 'output')
                outputs.push(nodeType);
            else
                others.push(nodeType);
        }
    ...
    buildOrderingDiagnostics(chain, userPrompt, branching) {
        const edges = this.buildConnectionPlanEdges(chain, userPrompt, branching);
        const hopRationales = edges.map((e) => `${e.fromType} → ${e.toType}${e.via ? ` [${e.via}]` : ''}: ${e.intent}`);
        let penalties = 0;
        for (const e of edges) {
            const fromRole = this.classifyNodeRole(e.fromType);
            const toRole = this.classifyNodeRole(e.toType);
            if (fromRole === 'transformation' && toRole === 'data_source')
                penalties += 0.2;
            if (fromRole === 'output' && toRole !== 'output' && e.toType !== 'log_output')
                penalties += 0.1;
        }
        const confidence = Math.max(0.4, Number((1 - penalties).toFixed(2)));
        return { confidence, hopRationales };
    }
    buildIntentMinimalNodeSelection(userPrompt, extractedNodeTypes) {
        const lower = userPrompt.toLowerCase();
        const signals = (0, branch_intent_model_1.extractBranchIntentSignals)(userPrompt);
        const reasons = {};
        const selected = [];
        const add = (nodeType, reason) => {
        ...
        const triggerFromPrompt = /\bform\b/.test(lower) ? 'form' :
        ...
        for (const out of signals.mentionedOutputNodeTypes) {
            if (unified_node_registry_1.unifiedNodeRegistry.get(out)) {
        ...
        if (signals.hasBranchingIntent) {
            const branchTargetsRequired = Math.max(2, (0, branch_intent_model_1.expectedBranchTargetCount)(signals));
            const selectedOutputs = selected.filter((t) => t !== 'log_output' && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t));
            if (selectedOutputs.length < branchTargetsRequired) {
                const explicitOutputs = signals.mentionedOutputNodeTypes.filter((t) => unified_node_registry_1.unifiedNodeRegistry.get(t));
                const fallbackOutputPool = explicitOutputs.length > 0
                    ? explicitOutputs
                    : extractedNodeTypes.filter((t) => t !== 'log_output' && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t));
                for (const fallback of fallbackOutputPool) {
                    if (selected.filter((t) => t !== 'log_output' && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t)).length >= branchTargetsRequired)
                        break;
    ...
    applyEmailChannelExclusivity(selected, lowerPrompt, reasons) {
    ...
    rankNodesByIntent(userPrompt, candidates, reasons) {
        const lower = userPrompt.toLowerCase();
        const scored = candidates.map((nodeType) => {
            ...
            if (role === 'trigger')
                score += 0.35;
            if (role === 'branching' && /\bif\b|\belse\b|\bcondition\b|\bcase\b/.test(lower))
                score += 0.35;
            if (role === 'data_source' && /\bfetch|get|read|from|sheet|database|db\b/.test(lower))
                score += 0.3;
            if (role === 'transformation' && /\bsummari[sz]e|analy[sz]e|classif|transform\b/.test(lower))
                score += 0.3;
            if (role === 'output' && /\bgmail|email|slack|discord|telegram|notify|send\b/.test(lower))
                score += 0.3;
            if (nodeType === 'email' && /\bgmail\b/.test(lower))
                score -= 0.25;
            if (reasons[nodeType])
                score += 0.2;
    ...
    isBranchingNodeType(nodeType) {
        const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
        if (!def)
            return false;
        return !!def.isBranching || nodeType === 'if_else' || nodeType === 'switch';
    }
    extractCanonicalNodeMentionsFromExecutionSummary(summary) {
        if (!summary)
            return [];
        ...
        for (const rawLine of executionSection.split('\n')) {
            ...
            for (const id of ids) {
                if (!unified_node_registry_1.unifiedNodeRegistry.get(id) || seen.has(id))
                    continue;
                seen.add(id);
                mentions.push(id);
            }
    ...
    assertPlanConsistency(plan, userPrompt, strictSelectedNodes) {
        ...
        if (summaryMissingFromChain.length > 0) {
            throw new Error(`Summary/chain mismatch: canonical nodes in summary but missing in chain: ${summaryMissingFromChain.join(', ')}`);
        }
        ...
        const strictMissingFromChain = strictSelectedNodes.filter((n) => {
            if (chainSet.has(n))
                return false;
            const normalized = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(n);
            return !chainSet.has(normalized);
        });
        if (strictMissingFromChain.length > 0) {
            throw new Error(`Strict-selection mismatch: selected canonical nodes missing in chain: ${strictMissingFromChain.join(', ')}`);
        }
        ...
        if (branchingNodes.length > 0) {
            const signals = (0, branch_intent_model_1.extractBranchIntentSignals)(userPrompt);
            const requiredTargets = (0, branch_intent_model_1.expectedBranchTargetCount)(signals);
            const downstreamOutputs = chain.filter((n) => n !== 'log_output' && !this.isTriggerNodeType(n) && node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(n));
            if (downstreamOutputs.length < requiredTargets) {
                throw new Error(`Branching chain incomplete: requires at least ${requiredTargets} downstream output target(s), found ${downstreamOutputs.length}`);
            }
            const branchLogTerminals = chain.filter((n) => n === 'log_output').length;
            if (branchLogTerminals < downstreamOutputs.length) {
                throw new Error(`Branching chain incomplete: requires one log_output per branch output (${downstreamOutputs.length}), found ${branchLogTerminals}`);
            }
        }
        if (branchingNodes.length > 0) {
            const selectedOutputs = strictSelectedNodes.filter((n) => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(n));
            if (selectedOutputs.length > 0) {
                ...
                if (missingSelectedOutputs.length > 0) {
                    throw new Error(`Branching intent detected but selected output targets are missing: ${missingSelectedOutputs.join(', ')}`);
                }
            }
        }
    }
    resolveRegistryNodeType(raw) {
        const head = (0, plan_chain_prune_1.stripPlanTokenToType)(String(raw || '').trim());
        if (!head)
            return null;
        try {
            return (0, node_type_resolver_util_1.resolveCanonicalNodeTypeStrict)(head);
        }
        catch {
            return null;
        }
     ...
     */
    isPlanBranchOutputHead(nodeType) {
     ...
     */
    tryParseOutputLogPairsAfterBranch(chain, branchingIdx) {
     ...
     */
    buildConnectionPlanEdges(chain, userPrompt, branching) {
        const edges = [];
        const push = (fromType, toType, intent, via) => {
        ...
        const hasBranchMetadata = !!branching && Array.isArray(branching.cases) && branching.cases.length > 0;
        if (branchingIdx >= 0 && hasBranchMetadata) {
            ...
            const outputTargets = downstream.filter((t) => node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isOutput(t));
            const uniqueLogTargets = new Set();
            ...
            for (let idx = 0; idx < caseCount; idx++) {
                ...
                if (branchType === 'if_else') {
                    ...
                    const matchValue = branchCase.condition?.matchValue !== undefined
        ...
        const pairedOutputs = branchingIdx >= 0 ? this.tryParseOutputLogPairsAfterBranch(chain, branchingIdx) : null;
        if (pairedOutputs && pairedOutputs.length >= 2) {
            ...
            pairedOutputs.forEach((outputType, idx) => {
                ...
                const branchIntent = branchType === 'if_else'
                    ? idx === 0
                ...
                if (chain.includes('log_output')) {
                    push(outputType, 'log_output', 'persist final observable output on this branch path');
                }
            });
            return edges;
        }
     ...
     */
    buildStructuredSummaryFromChain(chain, userPrompt, branching, intentModelDigest, caseNodeMapping) {
        ...
        const minimalIntent = {
            trigger: userPrompt,
            actions: chain.map(t => {
                const nodeType = (0, plan_chain_prune_1.stripPlanTokenToType)(t);
                const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
                const defaultOp = (() => {
                    try {
                        return def?.defaultConfig?.()?.operation || '';
                    }
                    catch {
                        return '';
                    }
        ...
        // Pass caseNodeMapping so switch nodes show their cases.
        const blocks = (0, node_description_builder_1.buildNodeDescriptionBlocks)(minimalIntent, chain, caseNodeMapping);
        const executionLines = [];
        const terminalLines = [];
        for (const block of blocks) {
            // Terminal nodes go to the Terminals: section
            if (unified_node_registry_1.unifiedNodeRegistry.get(block.nodeType)?.workflowBehavior?.alwaysTerminal === true) {
        ...
        const alignmentBlock = intentModelDigest?.trim()
            ...
            : '';
        const fillContract = (0, registry_structural_fill_contract_1.buildRegistryStructuralFillContractSection)(chain);
        return [
            `Goal:\n${goalDisplay}`,
     ...
     */
    describeHopIntent(fromType, toType, userPrompt) {
        const fromDef = unified_node_registry_1.unifiedNodeRegistry.get(fromType);
        const toDef = unified_node_registry_1.unifiedNodeRegistry.get(toType);
        ...
        if (fromDef?.isBranching) {
            const disc = fromDef.outputSchema
                ? Object.keys(fromDef.outputSchema.properties || {})[0] || 'value'
                : 'value';
    ...
    summarizeLayerResultFromPlan(plan, userPrompt, allKeywords, allKeywordData, extractedNodeTypes, _enrichedNodeMentions) {
        ...
        const mandatory = plan.mandatoryNodeTypes?.length
            ...
            : [
                ...new Set([
                    ...extractedNodeTypes,
                    ...plan.proposedNodeChain.map((t) => (0, plan_chain_prune_1.stripPlanTokenToType)(t)),
                ]),
            ];
        return {
            shouldShowLayer: true,
            ...
            registryTags: plan.registryTags?.length
                ? plan.registryTags
                : (0, gemini_node_selector_1.buildTagsFromRegistry)(mandatory),
        };
    }
    createSinglePlanFallbackResult(userPrompt, allKeywords, allKeywordData, extractedNodeTypes, error) {
        const fb = this.createFallbackResultWithExtractedNodes(userPrompt, allKeywords, extractedNodeTypes, allKeywordData, error);
        ...
        const fallbackSummary = this.buildStructuredSummaryFromChain(normalized, userPrompt, fallbackBranching, undefined, fallbackCaseMapping.caseNodeMapping);
        const plan = {
            ...
            orderingDiagnostics: this.buildOrderingDiagnostics(normalized, userPrompt, fallbackBranching),
    ...
    compilePlanSummaryV2(plan, userPrompt) {
        try {
            const nodes = plan.proposedNodeChain.map((rawType, index) => {
                const nodeType = unified_node_registry_1.unifiedNodeRegistry.resolveAlias((0, plan_chain_prune_1.stripPlanTokenToType)(rawType)) || (0, plan_chain_prune_1.stripPlanTokenToType)(rawType);
                const def = unified_node_registry_1.unifiedNodeRegistry.get(nodeType);
                return {
                    ...
                    data: {
                        ...
                        config: { ...(unified_node_registry_1.unifiedNodeRegistry.getDefaultConfig(nodeType) || {}) },
                    },
                };
            });
            const initialized = unified_graph_orchestrator_1.unifiedGraphOrchestrator.initializeWorkflow(nodes);
            const reconciled = unified_graph_orchestrator_1.unifiedGraphOrchestrator.reconcileWorkflow(initialized.workflow);
            const validation = unified_graph_orchestrator_1.unifiedGraphOrchestrator.validateWorkflow(reconciled.workflow, reconciled.executionOrder);
            if (!validation.valid)
                return undefined;
            const summaryV2 = (0, summary_v2_compiler_1.compileSummaryV2FromWorkflow)(reconciled.workflow, userPrompt);
            const summaryValidation = (0, summary_v2_validator_1.validateSummaryV2)(summaryV2);
...
exports.AIIntentClarifier = AIIntentClarifier;
/**
 * Summarize Layer Service (Main Entry Point)
 */
class SummarizeLayerService {
    constructor() {
    ...
    async processPromptToPlan(userPrompt) {
     ...
     */
    inferFromTextPatterns(nodeType, variationText, textLower, nodeTypeLower) {
        ...
        const verbPrepositionPattern = new RegExp(`(${DATA_SOURCE_VERBS.join('|')}|${OUTPUT_VERBS.join('|')}|${TRANSFORMATION_VERBS.join('|')})\\s+(from|to|via|using|through|with)\\s+${nodeTypeLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i');
        ...
        const nodePrepositionVerbPattern = new RegExp(`${nodeTypeLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+(to|for)\\s+(${DATA_SOURCE_VERBS.join('|')}|${OUTPUT_VERBS.join('|')}|${TRANSFORMATION_VERBS.join('|')})`, 'i');
        ...
        const verbNodePattern = new RegExp(`(${DATA_SOURCE_VERBS.join('|')}|${OUTPUT_VERBS.join('|')}|${TRANSFORMATION_VERBS.join('|')})\\s+${nodeTypeLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i');
        ...
        const nodeVerbPattern = new RegExp(`${nodeTypeLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s+(${DATA_SOURCE_VERBS.join('|')}|${OUTPUT_VERBS.join('|')}|${TRANSFORMATION_VERBS.join('|')})`, 'i');
     ...
     * @returns Inferred capability or null
     */
    inferFromPosition(nodeType, positionInChain, chainLength, availableCapabilities) {
     ...
     */
    getSingleCapability(nodeType) {
        const availableCapabilities = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.getCapabilities(nodeType);
        // Check if node can only be one of the three main capabilities
        const mainCapabilities = availableCapabilities.filter(cap => cap === 'data_source' || cap === 'transformation' || cap === 'output');
        if (mainCapabilities.length === 1) {
            return mainCapabilities[0];
        }
...
exports.SummarizeLayerService = SummarizeLayerService;
// Export singleton instance
exports.summarizeLayerService = new SummarizeLayerService();
