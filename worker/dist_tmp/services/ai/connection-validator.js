...
exports.connectionValidator = exports.ConnectionValidator = void 0;
...
class ConnectionValidator {
    ...
    areTypesCompatible(sourceType, targetType) {
        ...
        if (sourceType === 'string' && ['string', 'text', 'content'].includes(targetType))
            return true;
        if (targetType === 'string' && ['string', 'text', 'content'].includes(sourceType))
            return true;
        // Object compatibility
        if (sourceType === 'object' && ['object', 'json', 'data'].includes(targetType))
            return true;
        if (targetType === 'object' && ['object', 'json', 'data'].includes(sourceType))
            return true;
        // Array compatibility
        if (sourceType === 'array' && ['array', 'list', 'values'].includes(targetType))
            return true;
        if (targetType === 'array' && ['array', 'list', 'values'].includes(sourceType))
            return true;
        // Number compatibility
        if (sourceType === 'number' && ['number', 'numeric'].includes(targetType))
            return true;
        if (targetType === 'number' && ['number', 'numeric'].includes(sourceType))
            return true;
        return false;
    }
    /**
     ...
     */
    validateConnection(sourceNode, targetNode, edge) {
        const errors = [];
        const warnings = [];
        ...
        if (targetType === 'ai_agent') {
            ...
            if (!sourceOutputSchema.fields[outputField]) {
                ...
                const suitableFields = Object.keys(sourceOutputSchema.fields).filter(f => this.areTypesCompatible(sourceOutputSchema.fields[f], 'string'));
     ...
     */
    validateAllConnections(nodes, edges) {
        const results = [];
        const allErrors = [];
        const allWarnings = [];
...
exports.connectionValidator = new ConnectionValidator();
