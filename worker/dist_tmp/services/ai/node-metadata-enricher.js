...
exports.nodeMetadataEnricher = exports.NodeMetadataEnricher = void 0;
...
class NodeMetadataEnricher {
    ...
    enrichAllNodes() {
        ...
        const enriched = [];
        const registeredTypes = node_library_1.nodeLibrary.getRegisteredNodeTypes();
   ...
   Category: ${node.category}`;
    ...
    enrichNode(nodeType) {
        ...
        const semanticContext = this.createSemanticContext(nodeType, keywords, capabilities, description, useCases);
        return {
            type: nodeType,
            keywords,
            capabilities,
     ...
     */
    extractKeywords(schema, nodeType) {
        ...
        const capabilityKeywords = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.getCapabilities(nodeType);
     ...
     */
    extractCapabilities(schema, nodeType) {
        const capabilities = new Set();
        // Get from capability registry
        const registryCaps = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.getCapabilities(nodeType);
        ...
        if (schema.capabilities && Array.isArray(schema.capabilities)) {
            schema.capabilities.forEach((cap) => capabilities.add(cap.toLowerCase()));
        }
    ...
    extractUseCases(schema) {
        ...
        if (schema.aiSelectionCriteria?.useCases && Array.isArray(schema.aiSelectionCriteria.useCases)) {
            schema.aiSelectionCriteria.useCases.forEach((uc) => useCases.push(uc));
        }
        // From commonPatterns
        if (schema.commonPatterns && Array.isArray(schema.commonPatterns)) {
            schema.commonPatterns.forEach((pattern) => {
                if (pattern.name) {
     ...
     */
    createSemanticContext(nodeType, keywords, capabilities, description, useCases) {
        const parts = [];
     ...
     */
    formatRegistryForNodeSelection(nodes, maxChars) {
        if (!nodes || nodes.length === 0) {
            return 'No nodes available.';
        }
...
exports.NodeMetadataEnricher = NodeMetadataEnricher;
// Export singleton instance
exports.nodeMetadataEnricher = new NodeMetadataEnricher();
