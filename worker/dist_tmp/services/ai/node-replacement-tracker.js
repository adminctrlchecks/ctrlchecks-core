...
exports.nodeReplacementTracker = exports.NodeReplacementTracker = void 0;
class NodeReplacementTracker {
    constructor() {
        this.replacements = [];
    ...
    trackReplacement(replacement) {
        ...
        const confidenceInfo = replacement.confidence
            ...
            : '';
        console.log(`[NodeReplacementTracker] ${logLevel} ${protectionStatus} ` +
        ...
        if (replacement.isProtected && replacement.wasRemoved) {
            console.error(`[NodeReplacementTracker] 🚨 CRITICAL: Protected node removed! ` +
        ...
        if (replacement.confidence && replacement.confidence >= this.HIGH_CONFIDENCE_THRESHOLD) {
            console.warn(`[NodeReplacementTracker] ⚠️  High confidence node replaced! ` +
                `Confidence: ${(replacement.confidence * 100).toFixed(1)}%, ` +
    ...
    getStatistics() {
        const stats = {
            ...
            highConfidenceReplacements: this.replacements.filter(r => r.confidence && r.confidence >= this.HIGH_CONFIDENCE_THRESHOLD).length,
...
exports.NodeReplacementTracker = NodeReplacementTracker;
// Singleton instance
exports.nodeReplacementTracker = new NodeReplacementTracker();
