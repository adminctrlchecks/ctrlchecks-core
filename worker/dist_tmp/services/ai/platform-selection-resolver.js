...
exports.platformSelectionResolver = exports.PlatformSelectionResolver = exports.PLATFORM_GROUPS = void 0;
exports.PLATFORM_GROUPS = [
    {
        category: 'CRM',
 ...
 */
class PlatformSelectionResolver {
    /**
     ...
     */
    analyzePlatformSelection(userPrompt) {
        const promptLower = userPrompt.toLowerCase();
        const ambiguousCategories = [];
        const selectedPlatforms = new Map(); // category -> nodeType
        // Check each platform group
        for (const group of exports.PLATFORM_GROUPS) {
            // Check if user mentioned category (e.g., "CRM", "email")
            const mentionedCategory = group.platforms.some(platform => promptLower.includes(platform.toLowerCase())) || promptLower.includes(group.category.toLowerCase());
        ...
        if (ambiguousCategories.length > 0) {
            const questions = ambiguousCategories.map(category => {
                const group = exports.PLATFORM_GROUPS.find(g => g.category === category);
                if (!group)
                    return '';
     ...
     */
    checkDuplicatePlatforms(nodes) {
        const operationGroups = new Map(); // operation -> nodeTypes
...
exports.PlatformSelectionResolver = PlatformSelectionResolver;
// Export singleton instance
exports.platformSelectionResolver = new PlatformSelectionResolver();
