...
exports.intelligentConfigFiller = exports.IntelligentConfigFiller = void 0;
...
class IntelligentConfigFiller {
     ...
     */
    async getLLMSuggestedBindings(prompt, upstreamKeys, targetFieldNames) {
...
Return a JSON object mapping each target field to exactly one upstream key. Example: {"field_a": "upstream_key_a", "field_b": "upstream_key_b"}. Use only keys from the upstream list. Output nothing but the JSON.`;
     ...
     */
    async fillConfigurationsFromPrompt(workflow, enhancedPrompt, originalPrompt) {
        ...
        for (const nodeId of order) {
            ...
            const intelligentConfig = await this.analyzeAndFillConfig(node, nodeType, schema, enhancedPrompt, originalPrompt, workflow, upstreamSchema);
     ...
     */
    getUpstreamOutputSchema(workflow, nodeId, filledNodes) {
        const edges = (workflow.edges || []).filter((e) => (e.target ?? e.targetHandle) === nodeId && (e.type === 'main' || !e.type));
        ...
        return unified_node_registry_1.unifiedNodeRegistry.getEffectiveOutputSchema(type, config);
    }
    /**
     ...
     */
    async analyzeAndFillConfig(node, nodeType, schema, enhancedPrompt, originalPrompt, workflow, upstreamSchema) {
        const existingConfig = node.data?.config || {};
        ...
        const branchTag = node.data?.meta?.branchTag;
        ...
        const shouldFillField = (fieldName) => {
            const v = existingConfig[fieldName];
        ...
        const chooseUpstreamKey = (fieldName) => {
            ...
            if (exact)
                return { key: exact, strategy: 'exact', confidence: 0.98, candidates: [exact] };
        ...
        for (const fieldName of candidateFields) {
            ...
            if (fillMode !== 'buildtime_ai_once')
                continue;
            // ─────────────────────────────────────────────────────────────────────
            if (!shouldFillField(fieldName))
                continue;
            const pick = llmBindings?.[fieldName]
                ? { key: llmBindings[fieldName], strategy: 'keyword', confidence: 0.9, candidates: [llmBindings[fieldName]] }
        ...
        const priorFillMode = typeof existingConfig._fillMode === 'object' && existingConfig._fillMode !== null
            ? { ...existingConfig._fillMode }
            : {};
        const inFlightFillMode = typeof filled._fillMode === 'object' && filled._fillMode !== null
            ? { ...filled._fillMode }
            : {};
        // Merge: start from registry defaults, then apply in-flight stamps, then prior stamps (highest priority)
        const existingFillMode = {};
        for (const [name, field] of Object.entries(inputSchema)) {
            existingFillMode[name] = field?.fillMode?.default ?? 'manual_static';
        }
        ...
        delete filled._fieldModes;
        // ─────────────────────────────────────────────────────────────────────
        return filled;
    }
}
exports.IntelligentConfigFiller = IntelligentConfigFiller;
exports.intelligentConfigFiller = new IntelligentConfigFiller();
