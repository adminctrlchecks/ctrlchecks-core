...
exports.compileSummaryV2FromWorkflow = compileSummaryV2FromWorkflow;
...
function nodeLabelOf(node) {
    const registryLabel = unified_node_registry_1.unifiedNodeRegistry.get(nodeTypeOf(node))?.label;
    return registryLabel || String(node.data?.label || nodeTypeOf(node) || node.id);
}
function edgeCaseKey(edge) {
    return String(edge.branchName || edge.sourceHandle || (edge.isDefault ? 'default' : `case_${edge.sourceIndex ?? 0}`));
}
function compileSummaryV2FromWorkflow(workflow, userPrompt) {
    ...
    const executionBackbone = nodes.map((node, index) => ({
        ...
        responsibility: unified_node_registry_1.unifiedNodeRegistry.get(nodeTypeOf(node))?.description || 'Processes workflow data',
    }));
    const branches = nodes
        .filter((n) => unified_node_registry_1.unifiedNodeRegistry.get(nodeTypeOf(n))?.isBranching === true || (outgoing.get(n.id) || []).length > 1)
        .map((branchNode) => {
    ...
    const stack = triggerNodeIds.map((id) => ({
        path: [id],
        current: id,
        condition: 'default',
    }));
    while (stack.length > 0) {
        ...
        if (nextEdges.length === 0) {
            pathOutcomes.push({
                ...
                outcome: `Terminates at ${nodeLabelOf(nodeById.get(item.current) || { id: item.current, type: 'unknown', data: { label: item.current, type: 'unknown', category: 'utility', config: {} } })}`,
            });
            continue;
        }
    ...
    return {
        ...
        nodes: nodes.map((node) => ({
            ...
            nodeType: nodeTypeOf(node),
            label: nodeLabelOf(node),
            purpose: unified_node_registry_1.unifiedNodeRegistry.get(nodeTypeOf(node))?.description || 'Workflow step',
