...
exports.structuralPromptGenerator = exports.StructuralPromptGenerator = void 0;
 ...
 */
function buildBranchTree(steps) {
    const result = [];
    let i = 0;
    while (i < steps.length) {
        const step = steps[i];
        const def = unified_node_registry_1.unifiedNodeRegistry.get(step.nodeType);
        if (!def?.isBranching) {
        ...
        while (i < steps.length) {
            ...
            const childDef = unified_node_registry_1.unifiedNodeRegistry.get(childStep.nodeType);
 ...
 */
function renderBranchTree(tree, depth, lines) {
...
class StructuralPromptGenerator {
    ...
    deduplicateLinearNodes(nodes) {
        ...
        return nodes.filter((n) => {
            const canonical = unified_node_registry_1.unifiedNodeRegistry.resolveAlias(n.type) || n.type;
    ...
    buildSteps(nodes, intent) {
        ...
        const displayNameCount = new Map();
    ...
    buildConditions(nodes, intent) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            const nextBranchIdx = nodes.findIndex((n, idx) => {
                if (idx <= i)
                    return false;
                return unified_node_registry_1.unifiedNodeRegistry.get(n.type)?.isBranching === true;
            });
            ...
            const relevantFlows = intent.dataFlows.filter((f) => f.from.toLowerCase().includes(node.type.toLowerCase()) ||
    ...
    composeText(steps, conditions, triggerDescription, terminalAction, intentSummary) {
        const lines = [];
        // WORKFLOW line — AI-generated description, not the raw user prompt
        const triggerNode = steps.find(s => {
            const def = unified_node_registry_1.unifiedNodeRegistry.get(s.nodeType);
            return def?.category === 'trigger' || unified_node_registry_1.unifiedNodeRegistry.isTrigger(s.nodeType);
        });
        ...
        for (const step of steps) {
            const def = unified_node_registry_1.unifiedNodeRegistry.get(step.nodeType);
            if (def?.isBranching)
                branchingStepNumbers.add(step.stepNumber);
        }
    ...
    getDisplayName(nodeType) {
        const canonical = unified_node_registry_1.unifiedNodeRegistry.resolveAlias(nodeType) || nodeType;
...
exports.StructuralPromptGenerator = StructuralPromptGenerator;
exports.structuralPromptGenerator = new StructuralPromptGenerator();
