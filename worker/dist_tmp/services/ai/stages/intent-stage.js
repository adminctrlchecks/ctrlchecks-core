...
const intent_stage_client_1 = require("./intent-stage-client");
// ─── Intent Stage ─────────────────────────────────────────────────────────────
async function runIntentStage(userPrompt, nodeCatalog, correlationId) {
    const catalog = nodeCatalog ?? (0, node_catalog_builder_1.buildNodeCatalogText)();
    const startedAt = Date.now();
    let catalogNodeCount = 0;
    try {
        catalogNodeCount = JSON.parse(catalog).length;
    }
    catch { /* catalog may be plain text */ }
    const inputSummary = `prompt_len=${userPrompt.length}, catalog_nodes=${catalogNodeCount}`;
    logger_1.logger.info({ event: 'ai_pipeline_stage_start', stage: 'intent', correlationId, inputSummary });
    // ── Try remote ai-generator first ────────────────────────────────────────────
    const remote = await (0, intent_stage_client_1.runIntentStageRemote)(userPrompt, catalog, correlationId);
    if (remote?.ok) {
        logger_1.logger.info({ event: 'ai_pipeline_stage_end', stage: 'intent', correlationId, source: 'remote', durationMs: Date.now() - startedAt });
        return {
            ...remote,
    ...
    logger_1.logger.info({ event: 'ai_pipeline_llm_call', stage: 'intent', correlationId, model, temperature });
    let text;
    try {
        const raw = await gemini_orchestrator_1.geminiOrchestrator.processRequest('intent-analysis', { system: systemPrompt, message: userPrompt }, { model, temperature, cache: false });
        text = typeof raw === 'string' ? raw : JSON.stringify(raw);
    }
    catch (err) {
        logger_1.logger.error({ event: 'ai_pipeline_stage_error', stage: 'intent', correlationId, error: 'LLM_CALL_FAILED', message: String(err) });
    ...
    if (parsed) {
        ...
        logger_1.logger.info({ event: 'ai_pipeline_stage_end', stage: 'intent', correlationId, outputSummary: `actions=${parsed.actions.length}, dataFlows=${parsed.dataFlows.length}`, durationMs });
    ...
    logger_1.logger.warn({ event: 'ai_pipeline_stage_retry', stage: 'intent', correlationId, reason: 'JSON parse failed on first attempt' });
    let text2;
    try {
        const retryPrompt = systemPrompt + '\n\nCRITICAL: Your previous response was not valid JSON. Return ONLY the JSON object, nothing else.';
        const raw2 = await gemini_orchestrator_1.geminiOrchestrator.processRequest('intent-analysis', { system: retryPrompt, message: userPrompt }, { model, temperature, cache: false });
        text2 = typeof raw2 === 'string' ? raw2 : JSON.stringify(raw2);
    }
    catch (err) {
        logger_1.logger.error({ event: 'ai_pipeline_stage_error', stage: 'intent', correlationId, error: 'LLM_RETRY_FAILED', message: String(err) });
    ...
    if (parsed2) {
        ...
        logger_1.logger.info({ event: 'ai_pipeline_stage_end', stage: 'intent', correlationId, outputSummary: `actions=${parsed2.actions.length} (retry)`, durationMs: Date.now() - startedAt });
    ...
    logger_1.logger.error({ event: 'ai_pipeline_stage_error', stage: 'intent', correlationId, error: 'INVALID_LLM_RESPONSE', llmResponse: text2 });
    return buildFallbackIntentStageResult({
        userPrompt,
        systemPrompt,
        rawResponse: text2,
...
function tryParseIntent(text) {
    try {
        ...
        if (!obj.intent || !obj.triggerType || !Array.isArray(obj.actions))
            return null;
        return {
 ...
 */
function tryParsePartialIntent(partial) {
    try {
        ...
        if (!intentMatch || !triggerMatch || !actionsMatch)
            return null;
        ...
        const validTriggers = ['schedule', 'webhook', 'form', 'chat_trigger', 'manual_trigger'];
...
function buildDeterministicIntent(userPrompt) {
...
function inferTriggerType(prompt) {
    ...
    if (INBOUND_RECEIVE_RE.test(text))
        return 'webhook';
    if (/\bwebhook\b/.test(text) && !OUTBOUND_FETCH_RE.test(text))
        return 'webhook';
    if (/\b(form|submission|submitted)\b/.test(text))
        return 'form';
    if (/\b(chat|message from user|conversation)\b/.test(text))
        return 'chat_trigger';
    if (/\b(schedule|scheduled|every|daily|weekly|monthly|cron)\b/.test(text))
        return 'schedule';
 ...
 */
function correctIntentTriggerType(intent, originalPrompt) {
    if (intent.triggerType !== 'webhook')
        return intent;
    const text = originalPrompt.toLowerCase();
    if (INBOUND_RECEIVE_RE.test(text))
        return intent; // genuinely inbound — keep webhook
...
function extractActionPhrases(prompt) {
    ...
    for (const segment of segments) {
        ...
        if (conditional) {
            ...
            if (condition)
                actions.push(`check if ${condition}`);
...
function containsConditionalLanguage(value) {
