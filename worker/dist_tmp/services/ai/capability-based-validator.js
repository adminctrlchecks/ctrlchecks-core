...
exports.RequiredCapability = void 0;
exports.extractCapabilityRequirements = extractCapabilityRequirements;
exports.extractCapabilityProviders = extractCapabilityProviders;
exports.validateIntentCoverageByCapabilities = validateIntentCoverageByCapabilities;
...
(function (RequiredCapability) {
    RequiredCapability["READ"] = "read";
    RequiredCapability["TRANSFORM"] = "transform";
    RequiredCapability["WRITE"] = "write";
})(RequiredCapability || (exports.RequiredCapability = RequiredCapability = {}));
/**
 * Operation to capability mapping
 */
const OPERATION_TO_CAPABILITY = {
    // Read operations
 ...
 */
function mapOperationToCapabilities(operation, intentType) {
    ...
    const isTransformationNode = node_capability_registry_dsl_1.nodeCapabilityRegistryDSL.isTransformation(type);
 ...
 */
function extractCapabilityRequirements(intent) {
    const requirements = [];
 ...
 */
function extractCapabilityProviders(dsl) {
    const providers = [];
 ...
 */
function checkCapabilityCoverage(requirement, providers) {
    ...
    for (const requiredCap of requirement.requiredCapabilities) {
        ...
        for (const provider of providers) {
            ...
            if (requirement.capability === RequiredCapability.TRANSFORM && provider.category === 'transformation') {
                ...
                const hasTransformationCap = providerCaps.some(cap => ['transformation', 'ai_processing', 'transform', 'llm', 'summarize', 'analyze'].includes(cap));
            ...
            else if (providerCaps.some(cap => allPossibleCaps.some(reqCap => cap.includes(reqCap) || reqCap.includes(cap)))) {
                if (!matchingProviders.some(p => p.nodeType === provider.nodeType)) {
                    matchingProviders.push(provider);
                }
                satisfiedCapabilities.add(requiredCap);
 ...
 */
function validateIntentCoverageByCapabilities(intent, dsl) {
    const errors = [];
    ...
    for (const requirement of requirements) {
        ...
        if (!coverage.satisfied) {
            ...
            errors.push(`Intent action "${requirement.intentAction.type}" (operation: "${requirement.intentAction.operation}") ` +
