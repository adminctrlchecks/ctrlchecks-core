 ...
 */
async function generateTemplates({ upstreamNode, targetNode, structuredIntent, sampleLimit = 3, llmAdapter, }) {
    // ✅ STEP 1: Get ACTUAL output schema from upstream node
    const upstreamSchema = input_field_mapper_1.inputFieldMapper.getNodeOutputFields(upstreamNode);
    // ✅ STEP 2: Get target node input requirements
    const targetNodeType = targetNode.data?.type || targetNode.type;
    const targetNodeSchema = (0, node_output_types_1.getNodeOutputSchema)(targetNodeType);
    ...
    const mappings = validateAndNormalizeMappings(parsedResponse.mappings || [], upstreamSchema, targetInputFields);
 ...
 */
function buildMappingPrompt({ upstreamSchema, targetInputFields, targetNodeType, structuredIntent, sampleLimit, }) {
 ...
 */
function getTargetInputFields(targetNode) {
 ...
 */
function validateAndNormalizeMappings(rawMappings, upstreamSchema, targetInputFields) {
    const validMappings = [];
 ...
 */
function calculateOverallConfidence(mappings) {
 ...
 */
function generateNotes(mappings, upstreamSchema, targetInputFields) {
    const notes = [];
