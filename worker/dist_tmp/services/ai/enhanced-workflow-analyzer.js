...
exports.enhancedWorkflowAnalyzer = exports.EnhancedWorkflowAnalyzer = void 0;
const workflow_analyzer_1 = require("./workflow-analyzer");
const node_equivalence_mapper_1 = require("./node-equivalence-mapper");
const question_formatter_1 = require("./question-formatter");
/**
...
class EnhancedWorkflowAnalyzer {
    constructor() {
        this.baseAnalyzer = new workflow_analyzer_1.WorkflowAnalyzer();
    }
    /**
     ...
     */
    fastAnalyzePromptWithNodeOptions(userPrompt, context) {
        ...
        const nodeOptions = node_equivalence_mapper_1.nodeEquivalenceMapper.detectMultiNodeOptions(userPrompt);
     ...
     */
    async analyzePromptWithNodeOptions(userPrompt, context) {
    ...
    deduplicateQuestions(questions) {
        ...
        return questions.filter((q) => {
            ...
            const isTriggerQuestion = (normalizedText.includes('trigger') || normalizedText.includes('triggered')) &&
                (normalizedText.includes('workflow') || normalizedText.includes('this'));
            const isScheduleQuestion = (normalizedText.includes('when') || normalizedText.includes('run') || normalizedText.includes('schedule')) &&
                (normalizedText.includes('workflow') || normalizedText.includes('this'));
     ...
     */
    filterRelevantNodeOptions(nodeOptions, userPrompt, baseQuestions) {
     ...
     */
    filterDuplicateQuestions(preferenceQuestions, baseQuestions) {
        return preferenceQuestions.filter(prefQ => {
            ...
            const baseQuestionCovers = baseQuestions.some(baseQ => {
                ...
                const isPrefTriggerQuestion = (prefText.includes('trigger') || prefText.includes('triggered') || prefText.includes('when') || prefText.includes('run') || prefText.includes('schedule')) &&
                    (prefText.includes('workflow') || prefText.includes('this'));
                const isBaseTriggerQuestion = (baseText.includes('trigger') || baseText.includes('triggered') || baseText.includes('when') || baseText.includes('run') || baseText.includes('schedule')) &&
     ...
     */
    generatePreferenceQuestions(nodeOptions, userPrompt) {
        return nodeOptions.map(option => {
            const formatted = question_formatter_1.questionFormatter.formatNodeChoiceQuestion(option.category, option.options, userPrompt);
            // Map category to QuestionCategory
            const questionCategory = option.category === 'notification' ? 'destination' :
     ...
     */
    formatNodePreferenceQuestion(option, formatted) {
     ...
     */
    getFollowUpQuestionsForNode(nodeType) {
        const followUpMap = {
            slack: [
                {
                    id: 'slack_channel',
     ...
     */
    extractNodePreferences(answers) {
        const preferences = {};
     ...
     */
    extractNodeIdFromAnswer(answer) {
...
exports.EnhancedWorkflowAnalyzer = EnhancedWorkflowAnalyzer;
// Export singleton instance
exports.enhancedWorkflowAnalyzer = new EnhancedWorkflowAnalyzer();
