...
exports.requirementsExtractor = exports.RequirementsExtractor = void 0;
...
class RequirementsExtractor {
    ...
    isGeminiConfigured() {
        return !!(config_1.config.geminiApiKey && config_1.config.geminiApiKey.trim().length > 0);
    }
    /**
     * Extract workflow requirements from refined prompt
     */
    async extractRequirements(userPrompt, systemPrompt, answers, constraints) {
        console.log(`📋 Extracting requirements from: "${systemPrompt}"`);
        const extractionPrompt = this.buildExtractionPrompt(userPrompt, systemPrompt, answers, constraints);
        try {
            const response = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-analysis', {
...
Return structured JSON with all extracted requirements.`;
     ...
     */
    buildExtractionPrompt(userPrompt, systemPrompt, answers, constraints) {
...
Return JSON in this exact format:
{
  ...
  "platforms": ["platform1", "platform2"]
     ...
     */
    parseRequirementsResponse(response, systemPrompt, answers) {
        try {
            ...
            const hasAIFunctionality = systemPromptLower.includes('ai agent') ||
     ...
     */
    enhanceWithAnswers(parsed, answers) {
        ...
        if (hasExplicitSlack && !enhanced.platforms.some((p) => p.toLowerCase().includes('slack'))) {
        ...
        if (hasExplicitEmail && !enhanced.platforms.some((p) => p.toLowerCase().includes('email'))) {
     ...
     */
    extractWithRules(systemPrompt, answers) {
     ...
     */
    getMinimalRequirements(systemPrompt) {
        return {
    ...
    normalizeComplexity(value) {
        if (typeof value === 'string') {
            ...
            if (lower === 'simple' || lower === 'easy' || lower === 'basic')
                return 'simple';
            if (lower === 'complex' || lower === 'advanced' || lower === 'hard')
                return 'complex';
        }
        return 'medium';
    }
     ...
     */
    extractPrimaryGoal(systemPrompt) {
     ...
     */
    generateFallbackRequirements(systemPrompt) {
        const lowerPrompt = systemPrompt.toLowerCase();
        ...
        const hasAIFunctionality = lowerPrompt.includes('ai agent') ||
     ...
     */
    extractDataTransformRequirements(userPrompt, systemPrompt) {
     ...
     */
    extractInputOutputMappings(userPrompt, systemPrompt) {
     ...
     */
    extractAggregationOperations(prompt) {
        const operations = [];
        const promptLower = prompt.toLowerCase();
...
exports.RequirementsExtractor = RequirementsExtractor;
// Export singleton instance
exports.requirementsExtractor = new RequirementsExtractor();
