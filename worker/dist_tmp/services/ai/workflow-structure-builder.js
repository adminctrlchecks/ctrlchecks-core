...
exports.workflowStructureBuilder = exports.WorkflowStructureBuilder = void 0;
...
const embeddings_1 = require("../../memory/utils/embeddings");
const dag_validator_1 = require("../../core/validation/dag-validator");
const universal_category_resolver_1 = require("../../core/utils/universal-category-resolver");
class WorkflowStructureBuilder {
    ...
    validateNodeTypesInIntent(intent) {
        ...
        if (intent.trigger) {
            ...
            try {
                ...
                const triggerSchema = node_library_1.nodeLibrary.getSchema(resolvedTrigger);
                if (!triggerSchema) {
        ...
        if (intent.actions && intent.actions.length > 0) {
            ...
            for (const action of intent.actions) {
                try {
                    ...
                    const schema = node_library_1.nodeLibrary.getSchema(resolvedType);
                    if (!schema) {
     ...
     */
    validateOutputToDataSourceConnection(sourceNodeType, targetNodeType) {
    ...
    async getBestSimilarityScore(intent, userPrompt) {
        ...
        for (const priority of priorities) {
            ...
            const priorityScores = await Promise.all(workflows.map(async (workflow) => {
     ...
     */
    async buildStructure(intent, userPrompt) {
        ...
        const { mapIntentToCapabilities } = await Promise.resolve().then(() => __importStar(require('./intent-capability-mapper')));
        ...
        const disallowedActions = intent.actions?.filter(action => {
            const normalized = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(action.type);
        ...
        const dagValidation = dag_validator_1.dagValidator.validateAndFix(structure);
        if (!dagValidation.result.valid) {
            ...
            if (dagValidation.result.errors.some(e => e.includes('Trigger must have') ||
                ...
                const revalidation = dag_validator_1.dagValidator.validateAndFix(structure);
        ...
        const originalConnectionsMap = new Map(structure.connections.map(conn => [`${conn.source}→${conn.target}`, conn]));
        return {
            ...validatedStructure,
            trigger: validatedStructure.trigger || structure.trigger || 'manual_trigger',
            connections: validatedStructure.connections.map(conn => {
     ...
     */
    async findMatchingSampleWorkflow(intent, userPrompt) {
        const allWorkflows = workflow_training_service_1.workflowTrainingService.getAllWorkflows();
        ...
        const priority1Count = allWorkflows.filter(w => w._priority === 1).length;
        const priority2Count = allWorkflows.filter(w => w._priority === 2).length;
        const priority3Count = allWorkflows.filter(w => w._priority === 3).length;
        const priority4Count = allWorkflows.filter(w => w._priority === 4).length;
        ...
        for (const priority of priorities) {
            ...
            const structuralScores = await Promise.all(workflows.map(async (workflow) => {
            ...
            const topScores = await Promise.all(topCandidates.map(async ({ workflow, structuralScore, priority }) => {
                const source = workflow._source || 'unknown';
     ...
     */
    async preFilterWithEmbeddings(workflows, userPrompt) {
     ...
     */
    async calculateStructuralSimilarity(workflow, intent) {
     ...
     */
    async calculateAISimilarityCached(workflow, userPrompt) {
        const workflowDescription = this.extractWorkflowDescription(workflow);
        if (!workflowDescription || workflowDescription.trim().length === 0) {
            return 0;
        }
     ...
     */
    quickKeywordFilter(userPrompt, workflowDescription) {
        if (!userPrompt || !workflowDescription)
            return true; // If missing data, check anyway
     ...
     */
    async calculateSimilarity(workflow, intent, userPrompt) {
        const structuralScore = await this.calculateStructuralSimilarity(workflow, intent);
        if (!userPrompt || userPrompt.trim().length === 0) {
    ...
    extractWorkflowDescription(workflow) {
        ...
        if (workflow.use_case)
            parts.push(workflow.use_case);
        if (workflow.phase1?.step1?.userPrompt)
            parts.push(workflow.phase1.step1.userPrompt);
        if (workflow.category)
            parts.push(workflow.category);
        return parts.join('. ').trim();
    }
    /**
     * Use AI to calculate semantic similarity between user prompt and workflow description
     */
    async calculateAISimilarity(userPrompt, workflowDescription) {
        try {
...
Return ONLY valid JSON, no markdown, no explanations.`;
     ...
     */
    extractTriggerFromWorkflow(workflow) {
     ...
     */
    workflowHasActionType(workflow, actionType) {
        const selectedNodes = workflow?.phase1?.step5?.selectedNodes || [];
        return selectedNodes.some((node) => (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(node) === actionType);
    }
    /**
     ...
     */
    buildFromSampleWorkflow(sampleWorkflow, intent) {
        ...
        const actionNodes = selectedNodes.filter((nodeType) => !['webhook', 'form', 'schedule', 'manual_trigger', 'chat_trigger'].includes(nodeType));
        ...
        if (intent.actions && intent.actions.length > 0) {
            ...
            nodes = actionNodes.map((nodeType, index) => ({
                id: `step${index + 1}`,
                type: nodeType,
                config: {},
            }));
     ...
     */
    categorizeNodes(actions) {
        const dataSources = [];
        const processors = [];
        const outputs = [];
        const other = [];
        actions.forEach(action => {
            const normalizedType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(action.type);
            ...
            const category = universal_category_resolver_1.universalCategoryResolver.getNodeCategoryWithContext(tempNode);
     ...
     */
    parseConnections(connections, actionNodes) {
     ...
     * Add missing required nodes
     */
    addMissingNodes(structure, intent, allowedNodeTypes) {
     ...
     */
    addConditionalLogic(structure, conditions) {
        ...
        conditions.forEach((condition, index) => {
            if (condition.type === 'if_else') {
                ...
                const truePathNodes = structure.nodes.filter(n => condition.true_path?.includes(n.type));
                const falsePathNodes = structure.nodes.filter(n => condition.false_path?.includes(n.type));
                // Find insertion point (before first conditional action)
                const firstConditionalActionIndex = structure.nodes.findIndex(n => condition.true_path?.includes(n.type) || condition.false_path?.includes(n.type));
     ...
     */
    mapOutputToInput(sourceType, targetType) {
        // Default mapping
     ...
     */
    rebuildAsLinearChain(intent, originalStructure) {
        ...
        const normalizedLastType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeTypeString)(lastNode.type);
     ...
     */
    validateAcyclicGraph(structure) {
        ...
        if (validation.hasCycle) {
            ...
            const validConnections = structure.connections.filter(conn => {
                const isCycleEdge = validation.cycleEdges.some(cycleEdge => cycleEdge.source === conn.source && cycleEdge.target === conn.target);
                if (isCycleEdge) {
...
exports.WorkflowStructureBuilder = WorkflowStructureBuilder;
exports.workflowStructureBuilder = new WorkflowStructureBuilder();
