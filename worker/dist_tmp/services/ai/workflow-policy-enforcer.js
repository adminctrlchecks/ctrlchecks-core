...
exports.workflowPolicyEnforcer = exports.WorkflowPolicyEnforcer = void 0;
...
class WorkflowPolicyEnforcer {
     ...
     */
    enforcePolicies(structure, nodes, edges, userPrompt) {
        const violations = [];
     ...
     */
    enforceCRMPolicy(structure, nodes, userPrompt) {
        const violations = [];
        const promptLower = userPrompt.toLowerCase();
        // Detect CRM nodes
        const crmNodeTypes = ['hubspot', 'zoho_crm', 'salesforce', 'pipedrive'];
        const crmNodes = nodes.filter(n => {
            const type = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(n) || n.type || '';
     ...
     */
    enforceGraphPolicy(structure, nodes, edges) {
        const violations = [];
    ...
    enforceEdgePolicy(nodes, edges) {
        ...
        edges.forEach(edge => {
            ...
            if (edge.sourceHandle) {
                const sourceType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(sourceNode) || sourceNode.type || '';
            ...
            if (edge.targetHandle) {
                const targetType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(targetNode) || targetNode.type || '';
     ...
     */
    normalizeStructure(structure, nodes, edges, violations) {
        ...
        if (crmViolations.length > 0) {
            ...
            if (crmNodes.length > 1) {
                ...
                normalizedEdges = normalizedEdges.filter(e => !otherCrms.some(crm => crm.id === e.source || crm.id === e.target));
            }
        }
        ...
        if (triggerNode) {
            ...
            if (triggerOutgoing.length === 0) {
                ...
                if (firstActionNode) {
                    ...
                    const triggerType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(triggerNode) || triggerNode.type || '';
                    const targetType = (0, unified_node_type_normalizer_1.unifiedNormalizeNodeType)(firstActionNode) || firstActionNode.type || '';
        ...
        if (selfLoopsRemoved.length > 0) {
            console.warn(`⚠️  [Policy Enforcer] Removed ${selfLoopsRemoved.length} self-loop edge(s):`, selfLoopsRemoved.map(e => `${e.source} → ${e.target}`).join(', '));
        }
...
exports.WorkflowPolicyEnforcer = WorkflowPolicyEnforcer;
exports.workflowPolicyEnforcer = new WorkflowPolicyEnforcer();
