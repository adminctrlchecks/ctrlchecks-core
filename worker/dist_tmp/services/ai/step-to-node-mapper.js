...
exports.mapActionToNodeType = mapActionToNodeType;
exports.mapTriggerType = mapTriggerType;
exports.mapPlanStepsToWorkflowSteps = mapPlanStepsToWorkflowSteps;
exports.convertPlanToStructure = convertPlanToStructure;
 ...
 */
function mapActionToNodeType(action, context) {
    ...
    switch (action) {
        ...
        case 'summarise':
            ...
            const summarizerSchema = node_library_1.nodeLibrary.getSchema('text_summarizer');
            if (summarizerSchema) {
                return ['text_summarizer'];
            }
            // Fallback to AI service - use canonical transformation node
            return [(0, transformation_node_config_1.getTransformationNodeType)('summarize'), 'openai_gpt', 'ai_agent'];
        case 'analyze_data':
        case 'analyze':
        case 'analyse':
        ...
        case 'ai_processing':
            ...
            const textSummarizerSchema = node_library_1.nodeLibrary.getSchema('text_summarizer');
            if (textSummarizerSchema) {
                return ['text_summarizer'];
            }
            // Fallback to canonical transformation node
            return [(0, transformation_node_config_1.getTransformationNodeType)('process'), 'openai_gpt', 'ai_agent'];
        case 'send_email':
 ...
 */
function mapPlanStepsToWorkflowSteps(plan, userPrompt) {
    const steps = [];
    ...
    plan.steps.forEach((plannerStep, index) => {
        ...
        if (plannerStep.node_type) {
            ...
            const nodeTypes = mapActionToNodeType(plannerStep.action, {
                userPrompt,
        ...
        if (!isIntegrationNode) {
            ...
            if (stepText.includes('summarize') || stepText.includes('summarise') || stepText.includes('summary') ||
                ...
                if (summarizerSchema) {
                    ...
                    // Fallback to canonical transformation node
                    const aiChatModelSchema = node_library_1.nodeLibrary.getSchema('ai_chat_model');
                ...
                const aiAgentSchema = node_library_1.nodeLibrary.getSchema('ai_agent');
                const aiChatModelSchema = node_library_1.nodeLibrary.getSchema('ai_chat_model');
        ...
        if (!schema) {
            ...
            if (stepText.includes('summarize') || stepText.includes('analyze') || stepText.includes('process')) {
                ...
                const fallbackSchema = node_library_1.nodeLibrary.getSchema('ai_agent');
                if (fallbackSchema) {
                    nodeType = 'ai_agent';
                }
                else {
        ...
        const finalSchema = node_library_1.nodeLibrary.getSchema(nodeType);
        steps.push({
            id: `step_${index + 1}`,
 ...
 */
function convertPlanToStructure(plan, userPrompt) {
