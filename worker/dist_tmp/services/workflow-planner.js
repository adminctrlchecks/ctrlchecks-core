...
exports.workflowPlanner = exports.WorkflowPlanner = exports.ALLOWED_NODE_TYPES = void 0;
const gemini_orchestrator_1 = require("./ai/gemini-orchestrator");
 ...
 * These are canonical node types that exist in the node library
 */
exports.ALLOWED_NODE_TYPES = getAllowedNodeTypes();
/**
...
class WorkflowPlanner {
    constructor() {
        this.maxRetries = 3;
        this.baseTemperature = 0.1;
        this.retryDelay = 200; // ms — reduced from 1000ms
        // ✅ FIXED: Execution guard to prevent recursion
        this.isPlanning = false;
        this.planningPromises = new Map();
    }
    /**
     ...
     */
    async planWorkflow(userPrompt, constraints) {
     ...
     */
    async executePlanning(userPrompt, constraints) {
        let lastError = null;
     ...
     */
    async planWorkflowGraph(userPrompt, constraints) {
        let lastError = null;
        let rawResponse = null;
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                const planningPrompt = this.buildGraphPlanningPrompt(userPrompt, isRetry, constraints?.mandatoryNodes);
                const response = await gemini_orchestrator_1.geminiOrchestrator.processRequest('workflow-generation', {
                    prompt: planningPrompt,
                    system: this.getSystemPrompt(isRetry),
                }, {
     ...
     */
    buildPlanningPrompt(userPrompt, isRetry = false, mandatoryNodes) {
        ...
        const triggerNodes = availableNodeTypes.filter(type => type.includes('trigger') || type === 'schedule' || type === 'webhook' || type === 'form');
        const actionNodes = availableNodeTypes.filter(type => !triggerNodes.includes(type));
...
RESPOND WITH JSON ONLY - NO EXPLANATIONS, NO PROSE, NO MARKDOWN`;
     ...
     * }
     */
    buildGraphPlanningPrompt(userPrompt, isRetry = false, mandatoryNodes) {
        const strictJsonDirective = isRetry
...
RESPOND WITH JSON ONLY - NO EXPLANATIONS, NO PROSE, NO MARKDOWN.`;
     ...
     */
    parseGraphPlanningResponse(raw) {
        ...
        try {
            ...
            throw new Error(`Failed to parse WorkflowPlan JSON: ${err.message}`);
        }
     ...
     */
    formatNodeTypesForPrompt(nodeTypesByCategory) {
        const lines = [];
        nodeTypesByCategory.forEach((types, category) => {
            if (category === 'triggers')
                return; // Triggers are shown separately
...
Return JSON only with node_type fields matching the exact node types from the list provided in the prompt.`;
     ...
     */
    parsePlanningResponse(response, attempt = 1) {
        try {
     ...
     */
    resolvePlanNodeTypes(plan) {
        const resolvedPlan = {
            ...plan,
            steps: plan.steps.map((step, index) => {
                // Resolve trigger type if it's a node type
                if (index === 0 && plan.trigger_type) {
                    const resolvedTrigger = (0, node_type_resolver_util_1.resolveNodeType)(plan.trigger_type, false);
                ...
                if (step.node_type) {
                    ...
                    const resolvedType = (0, node_type_resolver_util_1.resolveNodeType)(originalType, debugLogging);
                ...
                if (step.action && !step.node_type) {
                    const debugLogging = process.env.DEBUG_NODE_LOOKUPS === 'true';
                    const resolvedType = (0, node_type_resolver_util_1.resolveNodeType)(step.action, debugLogging);
                    if (debugLogging) {
    ...
    validatePlan(plan) {
        ...
        if (!plan.trigger_type) {
            ...
            const triggerSchema = node_library_1.nodeLibrary.getSchema(plan.trigger_type);
            if (!triggerSchema) {
        ...
        if (!Array.isArray(plan.steps) || plan.steps.length === 0) {
            ...
            plan.steps.forEach((step, index) => {
                ...
                if ('action' in step && step.action && !step.node_type) {
                    ...
                    const resolvedType = (0, node_type_resolver_util_1.resolveNodeType)(step.action, false);
                ...
                if (!step.node_type) {
                    ...
                    const nodeSchema = node_library_1.nodeLibrary.getSchema(step.node_type);
                    if (!nodeSchema) {
                        // Try resolving again in case it wasn't resolved properly
                        const reResolved = (0, node_type_resolver_util_1.resolveNodeType)(step.node_type, false);
                        const reResolvedSchema = node_library_1.nodeLibrary.getSchema(reResolved);
                        if (!reResolvedSchema) {
                        ...
                        const resolved = (0, node_type_resolver_util_1.resolveNodeType)(step.node_type, false);
        ...
        const triggerNodesInSteps = plan.steps.filter(step => {
            ...
            return nodeType && (nodeType.includes('trigger') ||
                nodeType === 'schedule' ||
                nodeType === 'webhook' ||
                nodeType === 'form');
        });
     ...
     */
    enforceMandatoryNodes(plan, mandatoryNodes) {
        ...
        for (const mandatoryNode of mandatoryNodes) {
            ...
            const isSatisfied = unified_node_type_matcher_1.unifiedNodeTypeMatcher.isRequirementSatisfied(mandatoryNode, existingNodeTypes, { strict: false } // Use semantic equivalence
            );
    ...
    enforceMinimalWorkflow(plan) {
        ...
        const normalizedPlanSteps = plan.steps.map((step, idx) => {
            const rawType = (step.node_type || step.action || '').trim();
            if (!rawType)
                return step;
            const resolved = (0, node_type_resolver_util_1.resolveNodeType)(rawType);
        ...
        const filteredSteps = normalizedPlanSteps.filter(step => {
            const nodeType = step.node_type || step.action || '';
            const isTrigger = triggerNodeTypes.some(triggerType => nodeType.includes(triggerType) || nodeType === triggerType);
            if (isTrigger) {
     ...
     */
    validateOutputContribution(steps) {
        if (steps.length === 0) {
            return steps;
        }
        ...
        steps.forEach((step, index) => {
            ...
            const isGoogleSheetsWrite = isGoogleSheets &&
            ...
            const isHttpRequestOutput = isHttpRequest &&
        ...
        const contributingIndices = new Set();
    ...
    fallbackPlan(userPrompt) {
        ...
        if (promptLower.includes('google sheet') || promptLower.includes('sheets') || promptLower.includes('spreadsheet')) {
            ...
            if (nodeType)
                steps.push({ node_type: nodeType, order: steps.length + 1 });
        }
        ...
        if (steps.length === 0) {
            ...
            if (httpNode)
                steps.push({ node_type: httpNode, order: 1 });
            if (jsNode)
                steps.push({ node_type: jsNode, order: 2 });
        }
        return {
            trigger_type: triggerType, // Type assertion needed for backward compatibility
