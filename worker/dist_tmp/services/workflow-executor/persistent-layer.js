...
exports.PersistentLayer = void 0;
...
function toJsonSafe(val) {
    ...
    if (typeof val === 'object')
        return val; // objects and arrays are already JSONB-safe
...
class PersistentLayer {
     ...
     */
    async checkpointNodeExecution(executionId, nodeId, nodeName, nodeType, input, output, status, sequence, error) {
        try {
     ...
     */
    async restoreExecutionState(executionId) {
        try {
            ...
            const nodeOutputs = {};
            const completedSteps = (steps || []).filter((step) => step.status === 'success' || step.status === 'completed');
    ...
    async getNodeOutput(executionId, nodeId) {
        try {
            const { data, error } = await this.db
                ...
                .limit(1);
            const row = Array.isArray(data) ? data.find((step) => step.status === 'success' || step.status === 'completed' || step.output_json !== undefined) : data;
            if (error || !row) {
                return null;
            }
     ...
     */
    async getAllNodeOutputs(executionId) {
        try {
            const { data, error } = await this.db
                .from('execution_steps')
     ...
     */
    async updateExecutionStatus(executionId, status, output, error, meta) {
        try {
            const updateData = {
                status: status,
            ...
            if (status === 'success' || status === 'failed') {
                (0, redisGetCache_1.getCacheRedisClient)(process.env.REDIS_URL || 'redis://redis:6379')
                    .then(client => client ? (0, redisGetCache_1.invalidateExecutionStatusCache)(executionId, client) : null)
...
exports.PersistentLayer = PersistentLayer;
