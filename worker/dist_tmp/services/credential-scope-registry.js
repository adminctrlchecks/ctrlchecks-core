...
exports.PROVIDER_REQUIRED_SCOPES = void 0;
exports.normalizeProvider = normalizeProvider;
exports.scopeSet = scopeSet;
exports.splitScopeSet = splitScopeSet;
exports.scopesCover = scopesCover;
exports.requiredScopesForProvider = requiredScopesForProvider;
exports.credentialRequirementForNode = credentialRequirementForNode;
exports.PROVIDER_REQUIRED_SCOPES = {
    google: [
        'openid',
        'email',
        'profile',
...
function splitScopeSet(value) {
    if (!value || value === 'default')
        return [];
    return value.split('+').map((scope) => scope.trim()).filter(Boolean);
}
function scopesCover(available, required) {
    const have = new Set(available);
    return required.every((scope) => have.has(scope));
}
function requiredScopesForProvider(provider, explicitScopes = []) {
    if (explicitScopes.length > 0)
        return Array.from(new Set(explicitScopes));
    return exports.PROVIDER_REQUIRED_SCOPES[normalizeProvider(provider)] || [];
}
...
function credentialRequirementForNode(nodeType) {
