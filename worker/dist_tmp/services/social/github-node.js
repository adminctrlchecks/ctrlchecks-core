...
class GitHubNode {
    ...
    // ---------------------------------------------------------------------------
    async handleRepository(operation, p) {
        switch (operation) {
            case 'getRepo': {
            ...
            case 'listRepos': {
                ...
                if (targetUserOrOrg) {
                    ...
                    items = await this.octokit.paginate(this.octokit.repos.listForAuthenticatedUser, {
                        visibility: 'all',
            ...
            case 'listContributors': {
                required(p, ['owner', 'repo']);
                const { owner, repo } = p;
                const params = cleanUndefined({
                    owner,
                    repo,
                    per_page: 100,
                });
                const data = await this.octokit.paginate(this.octokit.repos.listContributors, params);
                return this.ok('repository', operation, data);
            }
            case 'createRepo': {
                required(p, ['name']);
    ...
    async handleIssue(operation, p) {
        switch (operation) {
            ...
            case 'listIssues': {
                ...
                const data = await this.octokit.paginate(this.octokit.issues.listForRepo, params);
                return this.ok('issue', operation, data);
            }
            case 'createIssue': {
    ...
    // ---------------------------------------------------------------------------
    async handlePullRequest(operation, p) {
        switch (operation) {
            case 'getPullRequest': {
            ...
            case 'listPullRequests': {
                ...
                const data = await this.octokit.paginate(this.octokit.pulls.list, params);
                return this.ok('pullRequest', operation, data);
            }
    ...
    // ---------------------------------------------------------------------------
    async handleFile(operation, p) {
        switch (operation) {
            case 'getContents': {
                required(p, ['owner', 'repo', 'path']);
                const { owner, repo, path, ref } = p;
                const { data } = await this.octokit.repos.getContent(cleanUndefined({ owner, repo, path, ref }));
    ...
    // Commit operations
    // ---------------------------------------------------------------------------
    async handleCommit(operation, p) {
        switch (operation) {
            case 'listCommits': {
                required(p, ['owner', 'repo']);
                ...
                const data = await this.octokit.paginate(this.octokit.repos.listCommits, params);
    ...
    // Branch operations
    // ---------------------------------------------------------------------------
    async handleBranch(operation, p) {
        switch (operation) {
            case 'listBranches': {
                required(p, ['owner', 'repo']);
                const { owner, repo } = p;
                ...
                const data = await this.octokit.paginate(this.octokit.repos.listBranches, params);
    ...
    // Release operations
    // ---------------------------------------------------------------------------
    async handleRelease(operation, p) {
        switch (operation) {
            case 'getRelease': {
            ...
            case 'listReleases': {
                ...
                const data = await this.octokit.paginate(this.octokit.repos.listReleases, params);
                return this.ok('release', operation, data);
            }
            case 'createRelease': {
    ...
    // ---------------------------------------------------------------------------
    async handleWorkflow(operation, p) {
        switch (operation) {
            case 'getWorkflowRuns': {
                required(p, ['owner', 'repo']);
                const { owner, repo, workflowId } = p;
                ...
                if (workflowId) {
                    ...
                    const data = await this.octokit.paginate(this.octokit.actions.listWorkflowRuns, params);
                    ...
                    const data = await this.octokit.paginate(this.octokit.actions.listWorkflowRunsForRepo, params);
                    return this.ok('workflow', operation, data);
                }
            }
            case 'triggerWorkflow': {
                required(p, ['owner', 'repo', 'workflowId', 'ref']);
                const { owner, repo, workflowId, ref } = p;
    ...
    error(resource, operation, message, statusCode, details) {
        return {
            success: false,
            provider: 'github',
            resource,
    ...
    handleError(resource, operation, err) {
        const e = err;
        const statusCode = typeof e.status === 'number'
            ? e.status
            : e.statusCode ?? undefined;
        const documentationUrl = e.documentation_url || e.response?.data?.documentation_url;
        console.error(`[GitHubNode] Error in ${resource}.${operation}:`, statusCode, e.message, e.response?.data);
        return this.error(resource, operation, e.message || 'Unknown GitHub error', statusCode, e.response?.data);
    }
}
