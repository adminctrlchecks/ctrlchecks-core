...
exports.runFirebaseNode = runFirebaseNode;
...
// Operation handlers
// ---------------------------------------------------------------------------
async function handleGet(db, inputs) {
...
async function handleAdd(db, inputs) {
...
async function handleUpdate(db, inputs) {
    const { collection, documentId } = inputs;
    const data = parseData(inputs.data);
    if (!collection) {
        return { success: false, error: 'collection is required' };
...
async function handleDelete(db, inputs) {
    const { collection, documentId } = inputs;
    if (!collection) {
        return { success: false, error: 'collection is required' };
    }
...
async function handleQuery(db, inputs) {
    const { collection } = inputs;
...
async function handleRealtimeGet(rtdb, inputs) {
...
async function handleRealtimeSet(rtdb, inputs) {
    const { collection } = inputs; // `collection` is used as the path for Realtime DB
    const data = parseData(inputs.data);
    if (!collection) {
        return { success: false, error: 'collection is required' };
 ...
 */
async function runFirebaseNode(context) {
