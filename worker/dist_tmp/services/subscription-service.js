...
const gemini_wallet_service_1 = require("./ai/gemini-wallet-service");
/**
...
class SubscriptionService {
    ...
    async buildFreeSubscriptionFallback(userId) {
    ...
    async getActualWorkflowCount(userId) {
        const client = await (0, db_pool_1.getDbPool)().connect();
        try {
            const result = await client.query(`SELECT COUNT(*)::int AS count
         FROM public.workflows
         WHERE user_id = $1
           AND COALESCE(setup_completed, true) = true
           AND COALESCE(quota_source, 'subscription') = 'subscription'`, [userId]);
    ...
    async syncWorkflowCount(userId, count) {
    ...
    async getEffectiveWorkflowLimit(userId) {
        const client = await (0, db_pool_1.getDbPool)().connect();
        try {
    ...
    async addWorkflowCredits(client, userId, credits) {
        if (credits <= 0)
            return;
        await client.query(`
        UPDATE public.users
    ...
    async getAvailablePlans() {
        try {
            ...
            const transformedPlans = plans.map((plan) => ({
                ...
                price: config_1.config.developmentPricing ?
                ...
                developmentMode: config_1.config.developmentPricing
            }));
            // Update cache
     ...
     */
    async upgradeSubscription(userId, planName, paymentId) {
        const client = await (0, db_pool_1.getDbPool)().connect();
        try {
    ...
    async getSubscriptionUsage(userId) {
        try {
            ...
            const utilizationPercentage = Math.min(100, Math.round((workflowsUsed / workflowLimit) * 100));
            return {
                workflowsUsed,
                workflowLimit,
                remainingWorkflows,
     ...
     */
    async canCreateWorkflow(userId) {
        try {
            if (await gemini_wallet_service_1.geminiWalletService.isActive(userId)) {
                return true;
            }
     ...
     */
    async incrementWorkflowCount(userId) {
        try {
     ...
     */
    async decrementWorkflowCount(userId) {
        try {
            await this.syncWorkflowCount(userId);
            return true;
        }
     ...
     */
    transformSubscriptionData(data) {
