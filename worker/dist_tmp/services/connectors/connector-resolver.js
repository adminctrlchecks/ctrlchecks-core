...
exports.connectorResolver = exports.ConnectorResolver = void 0;
...
class ConnectorResolver {
    ...
    resolveIntent(intent) {
        ...
        if (intent.provider && connectors.length > 1) {
            const providerConnectors = connectors.filter(c => c.provider === intent.provider);
     ...
     */
    assertGmailIntegrity(prompt, resolvedConnectors) {
        const promptLower = prompt.toLowerCase();
        ...
        if (!hasGmailConnector) {
            throw new Error(`🚨 CRITICAL: Prompt mentions Gmail but google_gmail connector was not resolved. ` +
        ...
        if (resolvedConnectors.includes('smtp_email')) {
            throw new Error(`🚨 CRITICAL: Prompt mentions Gmail but smtp_email connector was also resolved. ` +
                `Gmail must use google_gmail connector with OAuth, not SMTP.`);
        }
    }
}
exports.ConnectorResolver = ConnectorResolver;
// Singleton instance
exports.connectorResolver = new ConnectorResolver(connector_registry_1.connectorRegistry);
