...
class ConnectorRegistry {
     ...
     */
    getConnectorByNodeType(nodeType) {
     ...
     */
    getConnectorsByCapability(capability) {
        return Array.from(this.connectors.values()).filter(connector => connector.capabilities.includes(capability));
    }
    /**
     * Get connectors by provider
     */
    getConnectorsByProvider(provider) {
        return Array.from(this.connectors.values()).filter(connector => connector.provider === provider);
    }
    /**
     * Find connectors matching keywords
     */
    findConnectorsByKeywords(keywords) {
        const keywordSet = new Set(keywords.map(k => k.toLowerCase()));
        return Array.from(this.connectors.values()).filter(connector => {
            return connector.keywords.some(keyword => keywordSet.has(keyword.toLowerCase()));
        });
    }
    /**
    ...
    validateIsolation() {
        const errors = [];
        const credentialMap = new Map(); // credential key -> connector IDs
        for (const connector of this.connectors.values()) {
        ...
        for (const [key, connectorIds] of credentialMap.entries()) {
            if (connectorIds.length > 1) {
                ...
                if (providers.size > 1) {
                    errors.push(`Credential ${key} is shared across different providers: ${connectorIds.join(', ')}`);
                }
            }
        }
        return {
...
// Validate on load
const validation = exports.connectorRegistry.validateIsolation();
if (!validation.valid) {
