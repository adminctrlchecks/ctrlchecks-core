...
exports.executeGoogleCalendarOperation = executeGoogleCalendarOperation;
const googleapis_1 = require("googleapis");
const google_sheets_1 = require("./google-sheets");
/**
 ...
 */
async function executeGoogleCalendarOperation(db, userIds, params) {
    // Get access token
    const accessToken = await (0, google_sheets_1.getGoogleAccessToken)(db, userIds);
    if (!accessToken) {
    ...
    try {
        ...
        if (resource === 'calendar') {
            if (operation === 'get') {
                ...
                return params.returnAll ? await getAllPages(calendar.calendarList.list.bind(calendar.calendarList), response) : response.data;
            }
                ...
                if (!params.calendarId || !params.summary)
                    throw new Error('calendarId and summary are required');
        ...
        // ==================== EVENT OPERATIONS ====================
            ...
            if (operation === 'get') {
                ...
                return params.returnAll ? await getAllPages(calendar.events.list.bind(calendar.events), response) : response.data;
            }
            else if (operation === 'create') {
                ...
                const eventData = params.eventData || {};
                const requestBody = {};
                if (params.summary || eventData.summary)
                    requestBody.summary = params.summary || eventData.summary;
                ...
                if (eventData.recurrence)
                    requestBody.recurrence = eventData.recurrence;
                if (eventData.colorId)
                    requestBody.colorId = eventData.colorId;
                if (eventData.transparency)
                    requestBody.transparency = eventData.transparency;
                if (eventData.visibility)
                    requestBody.visibility = eventData.visibility;
                if (eventData.timeZone)
                    requestBody.timeZone = eventData.timeZone;
                ...
                if (!params.calendarId || !params.icalUID)
                    throw new Error('calendarId and icalUID are required');
        ...
        // ==================== EVENT INSTANCE OPERATIONS ====================
            ...
            if (operation === 'list') {
                ...
                return params.returnAll ? await getAllPages(calendar.events.instances.bind(calendar.events), response) : response.data;
            }
        }
        // ==================== ACL OPERATIONS ====================
            ...
            if (operation === 'get') {
                ...
                return params.returnAll ? await getAllPages(calendar.acl.list.bind(calendar.acl), response) : response.data;
            }
            else if (operation === 'create') {
        ...
        // ==================== SETTINGS OPERATIONS ====================
            ...
            if (operation === 'get') {
                ...
                return params.returnAll ? await getAllPages(calendar.settings.list.bind(calendar.settings), response) : response.data;
            }
        }
        // ==================== COLORS OPERATIONS ====================
 ...
 */
async function getAllPages(listFn, firstResponse) {
    const allItems = [];
    ...
    do {
        ...
        if (pageToken) {
            ...
            currentResponse = await listFn({ pageToken });
        }
    } while (pageToken);
