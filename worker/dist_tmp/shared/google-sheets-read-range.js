...
exports.normalizeSheetsValuesToRowObjects = normalizeSheetsValuesToRowObjects;
exports.fetchGoogleSheetReadRange = fetchGoogleSheetReadRange;
const toHeaderStrings = (row) => row.map((c, idx) => {
...
const isProbablyHeaderRow = (row) => {
    if (!Array.isArray(row) || row.length === 0)
        return false;
    if (!row.every((c) => typeof c === 'string' && c.trim().length > 0))
        return false;
 ...
 */
function normalizeSheetsValuesToRowObjects(values) {
    const safeValues = Array.isArray(values) ? values : [];
    let headers = [];
    let dataRows = safeValues;
    if (safeValues.length > 0 && isProbablyHeaderRow(safeValues[0])) {
        headers = toHeaderStrings(safeValues[0]);
    ...
    const itemsObjects = dataRows.map((row, idx) => {
        ...
        const obj = {
            row_number: headers.length > 0 && safeValues.length > 0 && isProbablyHeaderRow(safeValues[0])
                ? idx + 2
                : idx + 1,
        };
        headers.forEach((h, i) => {
 ...
 */
async function fetchGoogleSheetReadRange(params) {
